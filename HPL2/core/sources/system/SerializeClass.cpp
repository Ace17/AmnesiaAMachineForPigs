...
cSerializeSavedClass::cSerializeSavedClass(const char* asName, const char* asParent, cSerializeMemberField* apMemberFields, size_t alSize, iSerializable* (*apCreateFunc)())
{
    msName = asName;
...
{
    if (mpSavedClass == NULL || mpSavedClass->mpMemberFields[mlFieldNum].mType == eSerializeType_NULL)
    {
        return false;
    }
...
cSerializeClass::cSerializeClass(const char* asName, const char* asParent, cSerializeMemberField* apMemberFields, size_t alSize, iSerializable* (*apCreateFunc)())
{
    gvSerializeTempClasses[glSerializeTempClassesNum] = cSerializeSavedClass(asName, asParent, apMemberFields, alSize, apCreateFunc);
...
void cSerializeClass::PrintMembers(iSerializable* apData)
{
    ...
    while (classIt.HasNext())
    {
        ...
        Log(" '%s':'%s' type: %d offset: %d\n", pField->msName.c_str(), ValueToString(apData, pField->mlOffset, pField->mType), pField->mType, pField->mlOffset);
    }
}
...
void cSerializeClass::SaveToElement(iSerializable* apData, const tString& asName, TiXmlElement* apParent, bool abIsPointer)
{
    SetUpData();
...
bool cSerializeClass::LoadFromFile(iSerializable* apData, const tWString& asFile, bool abCompressedAndCrc)
{
    ...
    if (abCompressedAndCrc == false)
    {
        ...
        {
            Error("Couldn't load saved class file '%s' from %s!\n", cString::To8Char(asFile).c_str(), pXmlDoc->ErrorDesc());
            if (pFile)
                fclose(pFile);
            hplDelete(pXmlDoc);
            return false;
...
void cSerializeClass::LoadFromElement(iSerializable* apData, TiXmlElement* apElement, bool abIsPointer)
{
    SetUpData();
...
void cSerializeClass::StringToValue(void* apData, size_t alOffset, eSerializeType aType, const char* asVal)
{
    void* pVal = ValuePointer(apData, alOffset);
...
void cSerializeClass::SaveVariable(TiXmlElement* apElement, cSerializeMemberField* apField, iSerializable* apData)
{
    // CLASS
...
void cSerializeClass::SaveArray(TiXmlElement* apElement, cSerializeMemberField* apField, iSerializable* apData)
{
...
void cSerializeClass::SaveContainer(TiXmlElement* apElement, cSerializeMemberField* apField, iSerializable* apData)
{
...
void cSerializeClass::LoadVariable(TiXmlElement* apElement, iSerializable* apData, cSerializeSavedClass* apClass)
{
...
void cSerializeClass::SetUpData()
{
    ...
    {
        m_mapSavedClasses.insert(tSerializeSavedClassMap::value_type(tString(gvSerializeTempClasses[i].msName), gvSerializeTempClasses[i]));
    }
}
