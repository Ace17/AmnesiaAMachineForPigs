...
tWString cString::UTF8ToWChar(const tString& asString)
{
    ...
    while (pCur < pEnd)
    {
        ...
        {
            uTemp = (pCur[0] & 0x1f) << 6 | (pCur[1] & 0x3f);
            pCur += 2;
        }
        else if ((*pCur & 0xf0) == 0xe0)
        {
            uTemp = (pCur[0] & 0x0f) << 12 | (pCur[1] & 0x3f) << 6 | (pCur[2] & 0x3f);
            pCur += 3;
        }
#if SIZEOF_WCHAR == 4
        else if ((*pCur & 0xf8) == 0xf0)
        {
            uTemp = (pCur[0] & 0x07) << 18 | (pCur[1] & 0x3f) << 12 | (pCur[2] & 0x3f) << 6 | (pCur[3] & 0x3f);
            pCur += 4;
        }
        else if ((*pCur & 0xfc) == 0xf8)
        {
            uTemp = (pCur[0] & 0x03) << 24 | (pCur[1] & 0x3f) << 18 | (pCur[2] & 0x3f) << 12 | (pCur[3] & 0x3f) << 6 | (pCur[4] & 0x3f);
            pCur += 5;
        }
        else if ((*pCur & 0xfe) == 0xfc)
        {
            uTemp = (pCur[0] & 0x01) << 30 | (pCur[1] & 0x3f) << 24 | (pCur[2] & 0x3f) << 18 | (pCur[3] & 0x3f) << 12 | (pCur[4] & 0x3f) << 6 | (pCur[5] & 0x3f);
            pCur += 6;
        }
#endif
...
tString cString::RemoveSlashAtEnd(const tString& asPath)
{
    ...
    char lLastChar = asPath[asPath.size() - 1];
    if (lLastChar == '/' || lLastChar == '\\')
        return cString::Sub(asPath, 0, asPath.size() - 1);
...
tWString cString::RemoveSlashAtEndW(const tWString& asPath)
{
    ...
    wchar_t lLastChar = asPath[asPath.size() - 1];
    if (lLastChar == _W('/') || lLastChar == _W('\\'))
        return cString::SubW(asPath, 0, asPath.size() - 1);
...
cMatrixf cString::ToMatrixf(const char* asString, const cMatrixf& a_mtxDefault)
{
    ...
    return cMatrixf(vValues[0], vValues[1], vValues[2], vValues[3], vValues[4], vValues[5], vValues[6], vValues[7], vValues[8], vValues[9], vValues[10], vValues[11], vValues[12],
          vValues[13], vValues[14], vValues[15]);
}
