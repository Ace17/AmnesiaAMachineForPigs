...
bool cPreprocessParser::ParseKeyword(cParserSymbolKeyword* apKeyword)
{
    ...
    if (lKeyword == eParserKeyword_Ifdef)
    {
        ...
        while (currentKeyword != eParserKeyword_Endif && EndOfSymbols() == false)
        {
            ...
            if (currentKeyword == eParserKeyword_Ifdef || currentKeyword == eParserKeyword_Elseif)
            {
                GetNextSymbol();
            ...
            if (bHasFoundCorrectPassage == false && (bStatementWasTrue || currentKeyword == eParserKeyword_Else))
            {
                bHasFoundCorrectPassage = true;
                while (EndOfSymbols() == false && KeyWordIsOfType(mpCurrentSymbol, vKeywords, lKeywordNum) == false)
                {
            ...
            else
            {
                ...
                while (EndOfSymbols() == false && (KeyWordIsOfType(mpCurrentSymbol, vKeywords, lKeywordNum) == false || lIfDefCount > 0))
                {
