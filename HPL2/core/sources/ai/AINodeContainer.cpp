...
bool cAINodeRayCallback::BeforeIntersect(iPhysicsBody* pBody)
{
    ...
    if ((mFlags & eAIFreePathFlag_SkipDynamic) && (pBody->GetMass() > 0 || pBody->IsCharacter()))
        return false;
...
cAINodeContainer::cAINodeContainer(const tString& asName, const tString& asNodeName, cWorld* apWorld, const cVector3f& avCollideSize)
{
    mpWorld = apWorld;
...
{
public:
    bool operator()(const cAINodeEdge& aEndA, const cAINodeEdge& aEndB) { return aEndA.mfDistance < aEndB.mfDistance; }
};
...
void cAINodeContainer::Compile()
{
    ...
    for (; CurrentNodeIt != mvNodes.end(); ++CurrentNodeIt)
    {
        ...
        while (nodeIt.HasNext())
        {
            ...
            if (fHeight <= mfMaxHeight && FreePath(pNode->mvPosition, pEndNode->mvPosition, -1, flag))
            {
...
void cAINodeContainer::BuildNodeGridMap()
{
    ...
    for (size_t i = 0; i < mvNodes.size(); ++i)
    {
        ...
        if (false)
            Log("Adding node %d, world: (%s) local (%s), at %d : %d\n", i, pNode->GetPosition().ToString().c_str(), vLocalPos.ToString().c_str(), vGridPos.x, vGridPos.y);
...
static const cVector2f gvPosAdds[] = { cVector2f(0, 0), cVector2f(1, 0), cVector2f(-1, 0), cVector2f(0, 1), cVector2f(0, -1), cVector2f(0.5, 0.5), cVector2f(0.5, -0.5),
    cVector2f(-0.5, 0.5), cVector2f(-0.5, -0.5) };

bool cAINodeContainer::FreePath(const cVector3f& avStart, const cVector3f& avEnd, int alRayNum, tAIFreePathFlag aFlags, iAIFreePathCallback* apCallback)
{
