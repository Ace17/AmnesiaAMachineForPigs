...
void cAINodeGenerator::Generate(cWorld* apWorld, cAINodeGeneratorParams* apParams)
{
    ...
    cVector3f vEnds[4] = { cVector3f(mpParams->mfMinWallDist, 0, 0), cVector3f(-mpParams->mfMinWallDist, 0, 0), cVector3f(0, 0, mpParams->mfMinWallDist),
        cVector3f(0, 0, -mpParams->mfMinWallDist) };

    cVector3f vPushBackDirs[4] = { cVector3f(-1, 0, 0), cVector3f(1, 0, 0), cVector3f(0, 0, -1), cVector3f(0, 0, 1) };
...
void cAINodeGenerator::SaveToFile()
{
    ...
    for (; nodeIt != mpNodeList->end(); ++nodeIt)
    {
        ...
        tString sPos = cString::ToString(Node.mvPos.x) + " " + cString::ToString(Node.mvPos.y) + " " + cString::ToString(Node.mvPos.z);
