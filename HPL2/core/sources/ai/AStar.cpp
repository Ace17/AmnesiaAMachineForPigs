...
void cAStarHandler::AddOpenNode(cAINode* apAINode, cAStarNode* apParent, float afDistance)
{
    ...
    if (pNode->mlAStarCount == mlAStarCount)
    {
        ...
        if ((pNode->mFlags & eAStarNodeFlag_Open) == false)
            return; // Best match already found
    ...
    if (apParent == NULL)
        pNode->mFlags |= eAStarNodeFlag_CastRay; // Requires ray cast to start if it doesnt have a parent
...
{
    if (m_setGoalNodes.empty() || m_setOpenList.empty())
        return;
