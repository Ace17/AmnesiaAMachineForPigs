...
void cSurfaceData::OnSlide(float afSpeed, const cVector3f& avPos, int alContacts, iPhysicsBody* apBody, iPhysicsBody* apSlideAgainstBody)
{
        ...
        else
        {
            ...
            if (fAbsSpeed >= mfMiddleScrapeSpeed)
            {
                ...
                else
                {
                    ...
                    float fT = (fAbsSpeed - mfMiddleScrapeSpeed) / (mfMaxScrapeFreqSpeed - mfMiddleScrapeSpeed);
            ...
            else
            {
                ...
                {
                    // Calculate how close the speed is to max.
                    float fT = (mfMiddleScrapeSpeed - fAbsSpeed) / (mfMiddleScrapeSpeed - mfMinScrapeFreqSpeed);
    ...
    else
    {
        if (mfMinScrapeSpeed <= std::abs(afSpeed) && msScrapeSoundName != "")
        {
            ...
            if (pEntity)
            {
                ...
                //											(size_t)pEntity,
                //pEntity->GetSoundEntry(eSoundEntityType_Main)->GetChannel(), 											apBody->GetName().c_str(), apBody);
            }
        }
    }
...
void cSurfaceData::CreateImpactEffect(float afSpeed, const cVector3f& avPos, int alContacts, cSurfaceData* apSecondSurface, iPhysicsWorld* apPhysicsWorld)
{
    if (afSpeed == 0)
        return;
...
void cSurfaceData::UpdateRollEffect(iPhysicsBody* apBody)
{
    ...
    cVector3f vAngularSpeed = cMath::MatrixMul(apBody->GetLocalMatrix().GetRotation(), apBody->GetAngularVelocity());
    ...
    if (apBody->GetRollSoundEntity() != NULL)
    {
        ...
        else
        {
            ...
            if (fAbsSpeed >= mfMiddleRollSpeed)
            {
                ...
                {
                    // Calculate how close the speed is to max.
                    float fT = (fAbsSpeed - mfMiddleRollSpeed) / (mfMaxRollFreqSpeed - mfMiddleRollSpeed);
            ...
            else
            {
                ...
                {
                    // Calculate how close the speed is to max.
                    float fT = (mfMiddleRollSpeed - fAbsSpeed) / (mfMiddleRollSpeed - mfMinRollFreqSpeed);
