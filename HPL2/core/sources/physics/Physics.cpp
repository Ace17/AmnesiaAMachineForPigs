...
bool cPhysics::LoadSurfaceData(const tString& asFile, cHaptic* apHaptic)
{
    ...
    for (; pChildElem != NULL; pChildElem = pChildElem->NextSiblingElement("Material"))
    {
        ...
        if (cHaptic::GetIsUsed() && apHaptic)
        {
            ...
            {
                pSurface = apHaptic->GetLowLevel()->CreateFrictionalSurface(sName, fDamping, fStiffness, fDynamicFriction, fStartingFriction, fTangentStiffness, fStoppingFriction);
            }
            else if (surfaceType == eHapticSurfaceType_Rough)
            {
                pSurface = apHaptic->GetLowLevel()->CreateRoughSurface(
                      sName, fStickyStiffness, fMean, fDamping, fStiffness, fDynamicFriction, fStartingFriction, fTangentStiffness, fStoppingFriction);
            }
            else if (surfaceType == eHapticSurfaceType_Sticky)
            {
                pSurface = apHaptic->GetLowLevel()->CreateStickySurface(
                      sName, fDeadHeight, fStickyStiffness, fDamping, fStiffness, fDynamicFriction, fStartingFriction, fTangentStiffness, fStoppingFriction);
            }
