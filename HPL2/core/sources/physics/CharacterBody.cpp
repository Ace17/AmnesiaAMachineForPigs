...
bool cCharacterBodyRay::OnIntersect(iPhysicsBody* pBody, cPhysicsRayParams* apParams)
{
    ...
    if (pBody->IsCharacter() == false && pBody->GetCollideCharacter() && (bVolatile == false || mbCollideVolatile) && apParams->mfDist < mfMinDist)
    {
...
void iCharacterBody::ConnectToBody(iPhysicsBody* apBody, const cVector3f& avConnectionPos, tPhysicsConnectionFlag alFlags)
{
    if (apBody == NULL)
        return;
...
void iCharacterBody::UpdateForcePushing(float afTimeStep)
{
    ...
    for (size_t i = 0; i < mvTempBodies.size(); ++i)
    {
        ...
        collideData.SetMaxSize(32);
        bool bRet = mpWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), mpCurrentShape, mtxTransform, collideData, 32, true);
        ...
        if (bRet)
        {
            ...
            for (int i = 0; i < collideData.mlNumOfPoints; i++)
            {
                ...
                cVector3f vForce = point.mvNormal * point.mfDepth * fC - point.mvNormal * cMath::Vector3Dot(vBodyVelocityAtPos, point.mvNormal) * fV;
...
void iCharacterBody::UpdateGravityAttachment()
{
    ...
    if (mpGravityLastAttachedBody == mpGravityAttachedBody)
    {
        ...
        if (mpGravityAttachedBody->GetGravityAttachmentRotation())
        {
            ...
            {
                cMatrixf mtxRotDelta = cMath::MatrixMul(mpGravityAttachedBody->GetWorldMatrix().GetRotation(), mtxInvPrevWorld.GetRotation());
        ...
        {
            mvPosition += mpGravityAttachedBody->GetWorldMatrix().GetTranslation() - m_mtxGravityAttachedPrevMatrix.GetTranslation();
        }
    }
...
bool iCharacterBody::CheckCollision(cVector3f* apPushBackVector, const cVector3f& avPos, iPhysicsWorldCollisionCallback* apCallback, int alShapeIdx)
{
    if (alShapeIdx < 0)
        alShapeIdx = mlCurrentShapeIdx;
    iCollideShape* pShape = mvShapes[alShapeIdx];

    return mpWorld->CheckShapeWorldCollision(apPushBackVector, pShape, cMath::MatrixTranslate(avPos), mpCurrentBody, false, true, apCallback, true, mlMinBodyPushStrength,
