    ...
    : iEntity3D(asName)
{
    ...
    mBoundingVolume.SetLocalMinMax(apShape->GetBoundingVolume().GetMin(), apShape->GetBoundingVolume().GetMax());
...
bool iPhysicsBody::UpdateBeforeSimulate(float afTimeStep)
{
    ...
    if (mbStaticMovement)
    {
        if (mvTotalStaticLinearVel == cVector3f(0) && mvTotalStaticAngularVel == cVector3f(0))
        {
            mbStaticMovement = false;
        }
