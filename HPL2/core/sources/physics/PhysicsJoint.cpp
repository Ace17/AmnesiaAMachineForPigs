...
iPhysicsJoint::iPhysicsJoint(const tString& asName,
...
void iPhysicsJoint::CalcSoundFreq(float afSpeed, float* apFreq, float* apVol)
{
    ...
    if (fAbsSpeed >= mfMiddleMoveSpeed)
    {
        ...
        else
        {
            ...
            float fT = (fAbsSpeed - mfMiddleMoveSpeed) / (mfMaxMoveFreqSpeed - mfMiddleMoveSpeed);
    ...
    else
    {
        ...
        {
            // Calculate how close the speed is to max.
            float fT = (mfMiddleMoveSpeed - fAbsSpeed) / (mfMiddleMoveSpeed - mfMinMoveFreqSpeed);
...
bool iPhysicsJoint::OnPhysicsUpdate()
{
    ...
    if (mpParentBody)
    {
        if (m_mtxPrevChild == mpChildBody->GetLocalMatrix() && m_mtxPrevParent == mpParentBody->GetLocalMatrix())
        {
            vVel = 0;
        }
...
void iPhysicsJoint::CheckLimitAutoSleep(iPhysicsJoint* apJoint, const float afMin, const float afMax, const float afDist)
{
    if (apJoint->mbLimitAutoSleep)
    {
        ...
        if (fMaxDiff < apJoint->mfLimitAutoSleepDist || fMinDiff < apJoint->mfLimitAutoSleepDist)
        {
