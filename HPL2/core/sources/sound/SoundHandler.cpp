...
cSoundEntry::cSoundEntry(const tString& asName, iSoundChannel* apSound, float afVolume, eSoundEntryType aType, bool ab3D, bool abStream, int alId, cSoundHandler* apSoundHandler)
{
...
bool cSoundEntry::Update(float afTimeStep)
{
    ...
    if (mpSound->IsPlaying() == false && mpSound->GetPaused() == false)
    {
        ...
        if (mpCallback && mpSound->GetStopUsed() == false && mpSound->GetLooping() && mfVolumeFadeDest != 0)
        {
            if (gbLogEntry)
...
void cSoundEntry::Update3DSpecifics(float afTimeStep)
{
    ...
    mpSound->SetVolume(mfNormalVolume * mfVolumeMul * fBlock * fDistVolumeMul * mpSoundHandler->GetGlobalVolume(mType));
}
...
cSoundEntry* cSoundHandler::Play(const tString& asName,
      bool abLoop,
      float afVolume,
...
cSoundEntry* cSoundHandler::Play3D(const tString& asName,
      bool abLoop,
      float afVolume,
      const cVector3f& avPos,
      float afMinDist,
      ...
      bool* apNotEnoughChannels)
{
    return Play(asName, abLoop, afVolume, avPos, afMinDist, afMaxDist, aEntryType, abRelative, true, alPriorityModifier, abStream, apNotEnoughChannels);
}
...
cSoundEntry* cSoundHandler::PlaySoundEntityGui(const tString& asName, bool abLoop, float afVolume, eSoundEntryType aEntryType, const cVector3f& avPos, bool* apNotEnoughChannels)
{
...
bool cSoundHandler::CheckSoundIsBlocked(const cVector3f& avSoundPosition)
{
    ...
    pPhysicsWorld->CastRay(&mSoundRayCallback, avSoundPosition, mpLowLevelSound->GetListenerPosition(), false, false, false, true);
