...
cVector2f iLowLevelHaptic::GetVirtualMousePos()
{
    ...
    cVector2f vPos = (cVector2f(mvCurrentHardwarePos.x, mvCurrentHardwarePos.y) - mvMinMousePos) / (mvMaxMousePos - mvMinMousePos);
    vPos.y = 1 - vPos.y;
    vPos = vPos * mvScreenSize;
    return vPos;
}
void iLowLevelHaptic::SetVirtualMousePosBounds(const cVector2f& avMin, const cVector2f& avMax, const cVector2f& avScreenSize)
{
    mvMinMousePos = avMin;
    mvMaxMousePos = avMax;
