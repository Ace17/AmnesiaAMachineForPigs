...
cFrustum* cCamera::GetFrustum()
{
    if (mbFrustumUpdated)
    {
        ...
        {
            mFrustum.SetupPerspectiveProj(GetProjectionMatrix(), GetViewMatrix(), GetFarClipPlane(), GetNearClipPlane(), GetFOV(), GetAspect(), GetPosition(), mbInfFarPlane);
        }
        //////////////////////////////////////////////
        // Orthographic projection
        else
        {
            mFrustum.SetupOrthoProj(GetProjectionMatrix(), GetViewMatrix(), GetFarClipPlane(), GetNearClipPlane(), mvViewSize, GetPosition(), mbInfFarPlane);
        }
...
{
    UnProjectHelper(apPosition, apDirection, avScreenPos, avVirtualScreenSize, GetViewMatrix().GetRotation(), GetPosition(), GetForward(), GetUp(), GetRight());
}

void cCamera::UnProjectHelper(cVector3f* apPosition,
      cVector3f* apDirection,
      const cVector2f& avScreenPos,
      const cVector2f& avVirtualScreenSize,
      const cMatrixf& amtxCameraRotation,
      const cVector3f& avCameraPos,
      const cVector3f& avCameraFwd,
      const cVector3f& avCameraUp,
      const cVector3f& avCameraRgt)
{
    cVector2f vNormalizedScreen((avScreenPos.x / avVirtualScreenSize.x) - 0.5f, 0.5f - (avScreenPos.y / avVirtualScreenSize.y));
    ...
    if (mProjectionType == eProjectionType_Perspective)
    {
        ...
        cVector3f vDir(vNormalizedScreen.x * avViewportToWorld.x, vNormalizedScreen.y * avViewportToWorld.y, -mfNearClipPlane);
