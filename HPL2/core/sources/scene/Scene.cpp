...
cScene::cScene(cGraphics* apGraphics, cResources* apResources, cSound* apSound, cPhysics* apPhysics, cSystem* apSystem, cAI* apAI, cGui* apGui, cHaptic* apHaptic)
    ...
    for (cViewport* pViewPort : mlstViewports)
    {
        ...
        if (alFlags & tSceneRenderFlag_World)
        {
            ...
            if (pRenderer && pViewPort->GetWorld() && pFrustum)
            {
                ...
                pRenderer->Render(afFrameTime, pFrustum, pViewPort->GetWorld(), pViewPort->GetRenderSettings(), pViewPort->GetRenderTarget(), bPostEffects,
            ...
            else
            {
                ...
                mpGraphics->GetLowLevel()->SetCurrentFrameBuffer(pRenderTarget->mpFrameBuffer, pRenderTarget->mvPos, pRenderTarget->mvSize);
            }
...
void cScene::PostUpdate(float afTimeStep)
{
    ...
    if (mpCurrentListener && mpCurrentListener->GetCamera())
    {
        ...
        mpSound->GetLowLevel()->SetListenerAttributes(pCamera3D->GetPosition(), cVector3f(0, 0, 0), pCamera3D->GetForward() * -1.0f, pCamera3D->GetUp());
    }
}
