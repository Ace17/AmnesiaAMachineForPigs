...
iParticleEmitter::iParticleEmitter(tString asName, tMaterialVec* avMaterials, unsigned int alMaxParticles, cVector3f avSize, cGraphics* apGraphics, cResources* apResources)
    : iRenderable(asName)
{
    ...
    mpVtxBuffer = apGraphics->GetLowLevel()->CreateVertexBuffer(
          eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Stream, alMaxParticles * 4, alMaxParticles * 6);
...
bool iParticleEmitter::UpdateGraphicsForViewport(cFrustum* apFrustum, float afFrameTime)
{
    ...
    else
    {
        ...
        {
            cVector3f vAdd[4] = { cVector3f(mvDrawSize.x, -mvDrawSize.y, 0), cVector3f(-mvDrawSize.x, -mvDrawSize.y, 0), cVector3f(-mvDrawSize.x, mvDrawSize.y, 0),
