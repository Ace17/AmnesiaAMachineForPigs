...
cSoundEntity::cSoundEntity(const tString& asName,
...
void cSoundEntity::Play(bool abPlayStart)
{
    ...
    if (mvSoundEntries[eSoundEntityType_Main] == NULL && mvSoundEntries[eSoundEntityType_Start] == NULL && (mpData->GetLoop() == false || mpData->GetInterval() == 0))
    {
...
void cSoundEntity::UpdateLogic(float afTimeStep)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (mvSoundEntries[i])
        {
            if (mpSoundHandler->IsValid(mvSoundEntries[i], mvSoundEntryID[i]) == false)
            {
                ...
                else if ((mpData->GetInterval() == 0 || mpData->GetLoop() == false) && mbStarted)
                {
                    mbStopped = true;
                }
            }
    ...
    if (mbStopped == false)
    {
        ...
        if (mpData->GetLoop() && mpData->GetInterval() > 0 && mvSoundEntries[eSoundEntityType_Start] == NULL && mvSoundEntries[eSoundEntityType_Main] == NULL)
        {
            ...
            if (mfIntervalCount >= mpData->GetInterval())
            {
                ...
                if (cMath::RandRectf(0, 1) <= mpData->GetRandom() || mpData->GetRandom() == 0)
                {
...
bool cSoundEntity::PlaySound(eSoundEntityType aType, bool abLoop)
{
    ...
    {
        mvSoundEntries[aType] = mpSoundHandler->Play3D(sSoundName, abLoop, mfVolume, cVector3f(0, 0, 1), 100.0f, 200.0f, // Just set something so that sound is always inside min.
              eSoundEntryType_Gui, true, mpData->GetPriority(), mpData->GetStream(), &bNotEnoughChannels);
    }
    else
    {
        if (mpData->GetUse3D())
        {
            mvSoundEntries[aType] = mpSoundHandler->Play3D(sSoundName, abLoop, mfVolume, GetWorldPosition(), mfMinDistance, mfMaxDistance, eSoundEntryType_World, false,
                  mpData->GetPriority(), mpData->GetStream(), &bNotEnoughChannels);
        }
        else
        {
            mvSoundEntries[aType] = mpSoundHandler->Play3D(sSoundName, abLoop, mfVolume, cVector3f(0, 0, 1), 100.0f,
                  200.0f, // Just set something so that sound is always inside min.
                  eSoundEntryType_World, true, mpData->GetPriority(), mpData->GetStream(), &bNotEnoughChannels);
        }
    }
    ...
    if (mvSoundEntries[aType] == NULL)
    {
        ...
        if ((aType == eSoundEntityType_Main && bNotEnoughChannels == false) || (bNotEnoughChannels && abLoop == false))
        {
            mbStopped = true;
