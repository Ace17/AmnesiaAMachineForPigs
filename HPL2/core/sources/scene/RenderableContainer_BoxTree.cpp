...
void cRenderableContainer_BoxTree::CompileTempNode(cBoxTreeTempNode* apNode, int alLevel, int alSplitAxis)
{
    ...
    if (bLog)
    {
        Log("%sNode %d Parent: %d level %d. Axis: %d Size: (%s) Objects: %d\n", GetSpaces(alLevel), apNode, apNode->mpParent, alLevel, lAxis, vNodeSize.ToString().c_str(),
              lNumOfObjects);
        Log("%s==================================\n", GetSpaces(alLevel));
    }
    ...
    for (size_t i = 0; i < vSortedObjects.size(); ++i)
    {
        ...
        if (bLog)
            Log("%s object: '%s', pos: (%s)", GetSpaces(alLevel), pObject->GetName().c_str(), pObject->GetBoundingVolume()->GetWorldCenter().ToString().c_str());
