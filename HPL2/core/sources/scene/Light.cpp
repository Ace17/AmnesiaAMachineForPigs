...
iLight::iLight(tString asName, cResources* apResources)
    : iRenderable(asName)
{
    ///////////////////////////////
    ...
    if (mbFlickering && mfFadeTime <= 0)
    {
        ...
        if (mbFlickerOn)
        {
            if (mfFlickerTime >= mfFlickerStateLength)
            {
                ...
                {
                    cSoundEntity* pSound = mpWorld->CreateSoundEntity("FlickerOff", msFlickerOffSound, true);
                    if (pSound)
                    {
...
void iLight::SetFlicker(const cColor& aOffCol,
      float afOffRadius,
      float afOnMinLength,
      float afOnMaxLength,
...
void iLight::RenderShadow(iRenderable* apObject, cRenderSettings* apRenderSettings, iLowLevelGraphics* apLowLevelGraphics)
{
}
