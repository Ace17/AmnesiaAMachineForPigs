...
static bool CheckObjectIsAtEdgeOfAABB(iRenderable* apObject, const cVector3f& avNodeMin, const cVector3f& avNodeMax)
{
    ...
    if (cMath::Abs(vMin.x - avNodeMin.x) < kEpsilonf || cMath::Abs(vMin.y - avNodeMin.y) < kEpsilonf || cMath::Abs(vMin.z - avNodeMin.z) < kEpsilonf ||

          cMath::Abs(vMax.x - avNodeMax.x) < kEpsilonf || cMath::Abs(vMax.y - avNodeMax.y) < kEpsilonf || cMath::Abs(vMax.z - avNodeMax.z) < kEpsilonf)
    {
        return true;
    }
...
cDynBoxTreeObjectCallback::cDynBoxTreeObjectCallback(cRenderableContainer_DynBoxTree* apContainer)
...
mfSplitPlane = mvMeanPosition.v[mlSplitAxis];*/
    ...
    if ((int)mlstObjects.size() >= mpContainer->mlSplitThreshold && HasChildNodes() == false)
    {
        /////////////
...
void cRenderableContainer_DynBoxTree::AddObjectToNodeIterative(cRCNode_DynBoxTree* apNode, iRenderable* apObject)
{
    if (gbLog || HasDebug(apObject))
    {
        ...
        Log(" Testing node %d. Inside: %d Intersect: %d (%s)-(%s) vs (%s)-(%s)\n", apNode,
              ...
              cMath::CheckAABBIntersection(pBV->GetMin(), pBV->GetMax(), apNode->mvMin, apNode->mvMax) ? 1 : 0, pBV->GetMin().ToString().c_str(), pBV->GetMax().ToString().c_str(),
...
void cRenderableContainer_DynBoxTree::CheckForFitIterative(cRCNode_DynBoxTree* apNode, cBoundingVolume* apBV)
{
    ...
    if (apNode->GetParent() == NULL || cMath::CheckAABBInside(apBV->GetMin(), apBV->GetMax(), apNode->GetMin(), apNode->GetMax()))
    {
