...
enum eDeferredProgramMode
{
    eDeferredProgramMode_Lights,
    eDeferredProgramMode_Misc,
    eDeferredProgramMode_LastEnum
};
...
cRendererDeferred::cRendererDeferred(cGraphics* apGraphics, cResources* apResources)
    : iRenderer("Deferred", apGraphics, apResources, eDeferredProgramMode_LastEnum)
{
    ////////////////////////////////////
...
bool cRendererDeferred::LoadData()
{
    ...
    cVector3l vShadowSize[] = { cVector3l(128, 128, 1), cVector3l(256, 256, 1), cVector3l(256, 256, 1), cVector3l(512, 512, 1), cVector3l(1024, 1024, 1) };
    int lStartSize = 2;
    ...
    // Create Fog program
    {
        ...
        if (GetGBufferType() == eDeferredGBuffer_32Bit)
            mpFogProgramManager->AddUniform(0, "afNegFarPlane", kVar_afNegFarPlane);
        mpFogProgramManager->AddUniform(0, "avFogStartAndLength", kVar_avFogStartAndLength);
        mpFogProgramManager->AddUniform(0, "avFogColor", kVar_avFogColor);
        mpFogProgramManager->AddUniform(0, "avRayCastStart", kVar_avRayCastStart);
        mpFogProgramManager->AddUniform(0, "a_mtxBoxInvViewModelRotation", kVar_a_mtxBoxInvViewModelRotation);
        mpFogProgramManager->AddUniform(0, "avNegPlaneDistNeg", kVar_avNegPlaneDistNeg);
        mpFogProgramManager->AddUniform(0, "avNegPlaneDistPos", kVar_avNegPlaneDistPos);
        mpFogProgramManager->AddUniform(0, "afFalloffExp", kVar_afFalloffExp);
        ...
        // if(GetGBufferType() == eDeferredGBuffer_32Bit)
        //		mpModulatedFogProgram->AddUniform("afNegFarPlane", kVar_afNegFarPlane,0);
        // mpModulatedFogProgram->AddUniform("avFogStartAndLength", kVar_avFogStartAndLength,0);
        // mpModulatedFogProgram->AddUniform("avFogColor", kVar_avFogColor,0);
        // mpModulatedFogProgram->AddUniform("avRayCastStart", kVar_avRayCastStart,0);
        // mpModulatedFogProgram->AddUniform("a_mtxBoxInvViewModelRotation", kVar_a_mtxBoxInvViewModelRotation,0);
        // mpModulatedFogProgram->AddUniform("avNegPlaneDistNeg",kVar_avNegPlaneDistNeg,0);
        // mpModulatedFogProgram->AddUniform("avNegPlaneDistPos",kVar_avNegPlaneDistPos,0);
        // mpModulatedFogProgram->AddUniform("afFalloffExp",kVar_afFalloffExp,0);
    }
    ...
    // Create Light programs
    {
        ...
        // Misc
        {
            ...
            //////////////
            // Light Stencil
            mpLightStencilProgram = mpProgramManager->CreateProgramFromShaders("LightStencil", "deferred_base_vtx.glsl", "deferred_base_frag.glsl", &vars, true);
            ...
            for (int i = 0; i < 2; ++i)
            {
                if (i == 1)
                    vars.Add("UseSSAO");
                mpLightBoxProgram[i] = mpProgramManager->CreateProgramFromShaders("LightBoxNormal", "deferred_base_vtx.glsl", "deferred_light_box_frag.glsl", &vars, true);
            ...
            mpLightBoxProgram[2] = mpProgramManager->CreateProgramFromShaders("LightBoxNormal", "deferred_base_vtx.glsl", "deferred_light_box_frag.glsl", &vars, true);
            ...
            mpOverdrawProgram = mpProgramManager->CreateProgramFromShaders("Debug Light Complexity", "deferred_base_vtx.glsl", "debug_overdraw_frag.glsl", &vars, true);
            mpHeatMapProgram = mpProgramManager->CreateProgramFromShaders("Debug Heat map", "deferred_base_vtx.glsl", "debug_heat_map_frag.glsl", &vars, true);
        }
        ...
        // Lights
        {
            ...
            mpProgramManager->SetupGenerateProgramData(
                  eDeferredProgramMode_Lights, "Lights", "deferred_base_vtx.glsl", "deferred_light_frag.glsl", gvLightFeatureVec, kLightFeatureNum, defaultVars);
            // 1, defaultVars);

            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "avLightPos", kVar_avLightPos);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "avLightColor", kVar_avLightColor);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "afInvLightRadius", kVar_afInvLightRadius);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "afNegFarPlane", kVar_afNegFarPlane);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "afOneMinusCosHalfSpotFOV", kVar_afOneMinusCosHalfSpotFOV);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "avLightForward", kVar_avLightForward);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "a_mtxSpotViewProj", kVar_a_mtxSpotViewProj);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "a_mtxInvViewRotation", kVar_a_mtxInvViewRotation);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "avShadowMapOffsetMul", kVar_avShadowMapOffsetMul);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "afFalloff", kVar_afFalloff);
            mpProgramManager->AddUniform(eDeferredProgramMode_Lights, "afComplexity", kVar_afComplexity);
        }

        //////////////////////////////
        // Generate some light programs
        // for(i=0; i<128; ++i)
        //	mpProgramManager->GenerateProgram(eDeferredProgramMode_Lights, i);
    }
    ...
    if (mbSSAOLoaded)
    {
        ...
        mpSSAORenderProgram = mpGraphics->CreateGpuProgramFromShaders("SSAORender", "deferred_ssao_render_vtx.glsl", "deferred_ssao_render_frag.glsl", &programVars);
    ...
    if (mbEdgeSmoothLoaded)
    {
        ...
        if (mGBufferType == eDeferredGBuffer_32Bit)
            ...
            programVars.Add("Deferred_64bit");
        programVars.Add("UseUv");
        mpEdgeSmooth_UnpackDepthProgram = mpGraphics->CreateGpuProgramFromShaders(
              "EdgeSmoothUnpackDepth", "deferred_base_vtx.glsl", "deferred_unpack_depth_frag.glsl", &programVars);
    ...
    mpBatchBuffer = mpLowLevelGraphics->CreateVertexBuffer(
          eVertexBufferType_Software, eVertexBufferDrawType_Quad, eVertexBufferUsageType_Stream, mlMaxBatchVertices, mlMaxBatchIndices);
...
{
    if (alID < mvTempDeferredLights.size())
...
void cRendererDeferred::CopyToFrameBuffer()
{
    ...
    DrawQuad(cVector2f(0, 0), 1, cVector2f(vViewportPos.x, (mvScreenSizeFloat.y - vViewportSize.y) - vViewportPos.y),
          cVector2f(vViewportPos.x + vViewportSize.x, mvScreenSizeFloat.y - vViewportPos.y), true);
...
void cRendererDeferred::RenderObjects()
{
    ...
    {
        CheckForVisibleObjectsAddToListAndRenderZ(mpCurrentSettings->mpVisibleNodeTracker, eObjectVariabilityFlag_All, lVisibleFlags, true, NULL);

        mpCurrentRenderList->Compile(eRenderListCompileFlag_Z_Dissolve | eRenderListCompileFlag_Diffuse | eRenderListCompileFlag_Translucent | eRenderListCompileFlag_Decal |
    ...
    else
    {
        ...
        mpCurrentRenderList->Compile(eRenderListCompileFlag_Z_Dissolve | eRenderListCompileFlag_Z | eRenderListCompileFlag_Diffuse | eRenderListCompileFlag_Translucent |
...
void cRendererDeferred::RenderSSAO()
{
    ...
    {
        if (mvSortedLights[eDeferredLightList_Box_StencilFront_RenderBack].empty() && mvSortedLights[eDeferredLightList_Box_RenderBack].empty())
        {
            return;
        }
    }
...
void cRendererDeferred::SetupLightProgramVariables(iGpuProgram* apProgram, cDeferredLight* apLightData)
{
    ...
    else if (pLight->GetLightType() == eLightType_Spot)
    {
        ...
        {
            cVector2f vInvShadowMapSize(1.0f / (float)apLightData->mpShadowTexture->GetWidth(), 1.0f / (float)apLightData->mpShadowTexture->GetHeight());
...
#define RenderSingleLightDebug()                                                                                                                                                   \
    ...
    SetProgram(NULL);                                                                                                                                                              \
    SetTextureRange(NULL, 0);                                                                                                                                                      \
    mpLowLevelGraphics->SetColor(cColor(0.25, 0, 0, 0));                                                                                                                           \
    DrawCurrent();                                                                                                                                                                 \
    mpLowLevelGraphics->SetColor(cColor(1, 1));                                                                                                                                    \
    for (int i = 0; i < mlNumOfGBufferTextures; ++i)                                                                                                                               \
        SetTexture(i, mpGBufferTexture[i]);                                                                                                                                        \
...
iGpuProgram* cRendererDeferred::SetupProgramAndTextures(cDeferredLight* apLightData, tFlag alExtraFlags)
{
    ...
    /////////////////////////
    // Program
    iGpuProgram* pProgram = mpProgramManager->GenerateProgram(eDeferredProgramMode_Lights, lFlags);
    SetProgram(pProgram);
    if (pProgram)
    {
...
static tSortDeferredLightFunc vLightSortFunctions[eDeferredLightList_LastEnum] = { SortFunc_Default, SortFunc_Default, SortFunc_Default,

    SortFunc_Default, //<- Batches, not used!

    SortFunc_Box, SortFunc_Box };
...
void cRendererDeferred::SetupLightsAndRenderQueries()
{
    ...
    for (int i = 0; i < mpCurrentRenderList->GetLightNum(); ++i)
    {
        ...
        if (lightType == eLightType_Point)
        {
            pLightData->mbInsideNearPlane = mpCurrentFrustum->CheckSphereNearPlaneIntersection(pLight->GetWorldPosition(), pLight->GetRadius() * kLightRadiusMul_Low);
        }
        // Spot
        else if (lightType == eLightType_Spot)
        {
        ...
        ////////////////////////////////////
        // Setup Light matrix
        SetupLightMatrix(pLightData->m_mtxViewSpaceRender, pLightData->m_mtxViewSpaceTransform, pLight, mpCurrentFrustum, kLightRadiusMul_Medium);
        ...
        if (lightType == eLightType_Point)
        {
            pLightData->mClipRect = cMath::GetClipRectFromSphere(
                  pLightData->m_mtxViewSpaceRender.GetTranslation(), pLight->GetRadius(), mpCurrentFrustum, mvRenderTargetSize, true, mfScissorLastTanHalfFov);
        }
        // Spot
        else if (lightType == eLightType_Spot)
        {
            cMath::GetClipRectFromBV(pLightData->mClipRect, *pLight->GetBoundingVolume(), mpCurrentFrustum, mvScreenSize, mfScissorLastTanHalfFov);
        }
...
void cRendererDeferred::InitLightRendering()
{
    ...
    {
        UpdateqQuadVertexPostion(mpFullscreenLightQuad, cVector3f(mfFarLeft, mfFarBottom, -mfFarPlane), cVector2f(mfFarRight * 2, mfFarTop * 2), true);
    }
...
void cRendererDeferred::RenderLights_StencilBack_ScreenQuad()
{
    ...
    while (lNumOfNearPlaneLights > 0)
    {
        ...
        for (int i = 0; i < lIterations; ++i)
        {
            ...
            // Stencil
            SetStencil(eStencilFunc_Equal, 0xFF, cMath::GetFlagBit(kStartStencilBit + i), eStencilOp_Keep, eStencilOp_Keep, eStencilOp_Keep);
    ...
    while (transIt.HasNext())
    {
        ...
        if (pMaterial->HasRefraction())
        {
            ...
            if (pMaterial->UseRefractionEdgeCheck())
            {
                ...
                DrawQuad(cVector2f((float)clipRect.x, (float)clipRect.y), cVector2f((float)clipRect.w, (float)clipRect.h), 0, 1, false, cColor(1, 0));
            ...
            CopyFrameBufferToTexure(mpRefractionTexture, cVector2l(clipRect.x, clipRect.y), cVector2l(clipRect.w, clipRect.h), cVector2l(clipRect.x, clipRect.y), true);
        }
...
void cRendererDeferred::RenderSubMeshEntityReflection(cSubMeshEntity* pReflectionObject)
{
    ...
    reflectFrustum.SetupPerspectiveProj(mtxReflProj, mtxReflView, mpCurrentFrustum->GetFarPlane(), mpCurrentFrustum->GetNearPlane(), mpCurrentFrustum->GetFOV(),
          mpCurrentFrustum->GetAspect(), vReflOrigin, false, &mtxProj, true);
    ...
    ///////////////////////////
    // Set back to order!
    BeginRendering(mfCurrentFrameTime, pSaved_Frustum, mpCurrentWorld, pSaved_Settings, pSaved_RenderTarget, bSaved_SendFrameBufferToPostEffects, mpCallbackList, false);
