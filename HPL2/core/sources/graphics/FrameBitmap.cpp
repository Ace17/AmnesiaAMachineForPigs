...
cFrameBitmap::cFrameBitmap(cBitmap* apBitmap, cFrameTexture* apFrmTex, int alHandle)
    : iFrameBase()
{
    mpBitmap = apBitmap;
...
cFrameSubImage* cFrameBitmap::AddBitmap(cBitmap* apSrc, const tWString& asFullPath, cFrameSubImage* apSubImageCreated, bool* apFoundNode)
{
    ...
    for (it = lstNodes.begin(); it != lstNodes.end(); ++it)
    {
        ...
        if (pData->mlHandle < 0)
        {
            ...
            if (DEBUG_BTREE)
                Log("Fit: [%d:%d:%d:%d] in [%d:%d:%d:%d]\n", NewRect.x, NewRect.y, NewRect.w, NewRect.h, pData->mRect.x, pData->mRect.y, pData->mRect.w, pData->mRect.h);
            ...
            if (cMath::CheckRectFit(NewRect, pData->mRect))
            {
                ...
                else
                {
                    ...
                    // Upper
                    UpperNode = mRects.InsertAt(cFBitmapRect(NewRect.x, NewRect.y, pData->mRect.w, NewRect.h, -2), TopNode, eBinTreeNode_Left);

                    // Lower
                    mRects.InsertAt(cFBitmapRect(NewRect.x, NewRect.y + NewRect.h, pData->mRect.w, pData->mRect.h - NewRect.h, -3), TopNode, eBinTreeNode_Right);
                    ...
                    mRects.InsertAt(cFBitmapRect(NewRect.x, NewRect.y, NewRect.w, NewRect.h, 2), UpperNode, eBinTreeNode_Left);

                    // Lower split, this is empty
                    mRects.InsertAt(cFBitmapRect(NewRect.x + NewRect.w, NewRect.y, pData->mRect.w - NewRect.w, NewRect.h, -4), UpperNode, eBinTreeNode_Right);
                }

                vPos = cVector2l(NewRect.x + 1, NewRect.y + 1); //+1 for the right pos

                // Draw corners for border
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x, NewRect.y, 0), cVector3l(1, 1, 1), 0);
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x + apSrc->GetWidth() + 1, NewRect.y, 0), cVector3l(1, 1, 1), cVector3l(apSrc->GetWidth() - 1, 0, 0));
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x + apSrc->GetWidth() + 1, NewRect.y + apSrc->GetHeight() + 1, 0), cVector3l(1, 1, 1),
                      cVector3l(apSrc->GetWidth() - 1, apSrc->GetHeight() - 1, 0));
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x, NewRect.y + apSrc->GetHeight() + 1, 0), cVector3l(1, 1, 1), cVector3l(0, apSrc->GetHeight() - 1, 0));

                // Draw sides for border
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x + 1, NewRect.y, 0), cVector3l(apSrc->GetWidth(), 1, 1), 0);
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x + 1, NewRect.y + apSrc->GetHeight() + 1, 0), cVector3l(apSrc->GetWidth(), 1, 1), cVector3l(0, apSrc->GetHeight() - 1, 0));
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x, NewRect.y + 1, 0), cVector3l(1, apSrc->GetHeight(), 1), 0);
                mpBitmap->Blit(apSrc, cVector3l(NewRect.x + apSrc->GetWidth() + 1, NewRect.y + 1, 0), cVector3l(1, apSrc->GetHeight(), 1), cVector3l(apSrc->GetWidth() - 1, 0, 0));
    ...
    if (bFoundNode && apSubImageCreated == NULL)
    {
        ...
        pImage = hplNew(cFrameSubImage,
              (cString::To8Char(apSrc->GetFileName()), asFullPath, mpFrameTexture, this, cRect2l(vPos, cVector2l(lSW - 2, lSH - 2)), //-2 to get the correct size.
                    cVector2l(mpBitmap->GetWidth(), mpBitmap->GetHeight()), mlHandle, pBitmapImage));
...
public:
    ...
    ~cBitmapSubImageComb() { hplDelete(mpBitmap); }
