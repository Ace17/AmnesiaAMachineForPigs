...
void cRenderList::PrintAllObjects()
{
    ...
    for (size_t i = 0; i < mvTransObjects.size(); ++i)
        Log(" '%s' ViewspaceZ: %f LargeSurfacePlacement: %d Mat: '%s' RenderCount: %d\n", mvTransObjects[i]->GetName().c_str(), mvTransObjects[i]->GetViewSpaceZ(),
              mvTransObjects[i]->GetLargePlaneSurfacePlacement(), mvTransObjects[i]->GetMaterial()->GetName().c_str(), mvTransObjects[i]->GetRenderFrameCount());

    Log("Solid Objects:\n");
    for (size_t i = 0; i < mvSolidObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvSolidObjects[i]->GetName().c_str(), mvSolidObjects[i]->GetMaterial()->GetName().c_str(),
              mvSolidObjects[i]->GetRenderFrameCount());

    Log("Decal Objects:\n");
    for (size_t i = 0; i < mvDecalObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvDecalObjects[i]->GetName().c_str(), mvDecalObjects[i]->GetMaterial()->GetName().c_str(),
              mvSolidObjects[i]->GetRenderFrameCount());

    Log("Illum Objects:\n");
    for (size_t i = 0; i < mvIllumObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvIllumObjects[i]->GetName().c_str(), mvIllumObjects[i]->GetMaterial()->GetName().c_str(),
              mvSolidObjects[i]->GetRenderFrameCount());
...
void cRenderList::CompileArray(eRenderListType aType)
{
    ...
    if (aType == eRenderListType_Z_Dissolve || aType == eRenderListType_Z)
    {
        ...
        for (size_t i = 0; i < pSourceVec->size(); ++i)
        {
            ...
            {
                if (pObject->IsOccluder() == false && (pObject->GetCoverageAmount() <= 1.0f || pMaterial->GetTexture(eMaterialTexture_Alpha)))
                {
                    /////////////
...
void cRenderList::FindNearestLargeSurfacePlane()
{
    ...
    if (bHasLargeSurfacePlane)
    {
        ...
        for (size_t i = 0; i < mvTransObjects.size(); ++i)
        {
            ...
            // Log(" %d Object: '%s' %d  Mat:'%s'\n",i, pObject->GetName().c_str(), pObject->GetLargePlaneSurfacePlacement(),
            // pLargeSurfaceObject->GetMaterial()->GetName().c_str());
        }
    }
    ////////////////////////////////////
