...
namespace hpl
{
static cMatrixf m_mtxNULL; // used to reset current matrix pointer.
//////////////////////////////////////////////////////////////////////////
// PUBLIC METHODS
//////////////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------
...
void iRenderFunctions::InitAndResetRenderFunctions(cFrustum* apFrustum,
...
void iRenderFunctions::SetStencil(eStencilFunc aFunc, int alRef, unsigned int aMask, eStencilOp aFailOp, eStencilOp aZFailOp, eStencilOp aZPassOp)
{
...
void iRenderFunctions::SetScissorRect(const cVector2l& avPos, const cVector2l& avSize, bool abAutoEnabling)
{
    ...
    /////////////////////////////
    // Set the clip rect
    if (mbUseGlobalScissorRect == false && vFinalPos.x == 0 && vFinalPos.y == 0 && vFinalSize.x == mvRenderTargetSize.x && vFinalSize.y == mvRenderTargetSize.y)
    {
        if (abAutoEnabling)
...
void iRenderFunctions::SetFrameBuffer(iFrameBuffer* apFrameBuffer, bool abUsePosAndSize, bool abUseGlobalScissor)
{
    if (abUsePosAndSize)
    {
        if (mbLog)
        {
            Log(" Setting FrameBuffer %d '%s', pos: %d,%d size: %dx%d\n", apFrameBuffer, apFrameBuffer ? apFrameBuffer->GetName().c_str() : "", mpCurrentRenderTarget->mvPos.x,
                  mpCurrentRenderTarget->mvPos.y, mvRenderTargetSize.x, mvRenderTargetSize.y);
        }
    ...
    if (mbUseGlobalScissorRect)
    {
        ...
        if (abUseGlobalScissor)
        {
            ...
            if (mbGlobalScissorRectActive == false || mvCurrentFrameBufferSize != vPrevFrameBufferSize)
            {
                if (mbLog)
                {
                    Log("  Setting scissor rect: %d, %d, %dx%d\n", mvGlobalScissorRectPos.x, mvGlobalScissorRectPos.y, mvGlobalScissorRectSize.x, mvGlobalScissorRectSize.y);
                }
...
void iRenderFunctions::ClearFrameBuffer(tClearFrameBufferFlag aFlags, bool abUsePosAndSize)
{
    ...
    if (abUsePosAndSize && (mpCurrentRenderTarget->mvPos != cVector2l(0, 0) || mvRenderTargetSize != mvCurrentFrameBufferSize || mbUseGlobalScissorRect))
    {
        /////////////////////////////////
        ...
        if (mbUseGlobalScissorRect)
        {
            if (mbLog)
            {
                Log(" Clearing portion of frame buffer. Pos: %d,%d Size: %dx%d\n", mvGlobalScissorRectPos.x, mvGlobalScissorRectPos.y, mvGlobalScissorRectSize.x,
                      mvGlobalScissorRectSize.y);
            }
        }
        /////////////////////////////////
        // Render target size
        else
        {
            if (mbLog)
            {
                Log(" Clearing portion of frame buffer. Pos: %d,%d Size: %dx%d\n", mpCurrentRenderTarget->mvPos.x, mpCurrentRenderTarget->mvPos.y, mvRenderTargetSize.x,
...
void iRenderFunctions::DrawQuad(const cVector3f& aPos, const cVector2f& avSize, const cVector2f& avMinUV, const cVector2f& avMaxUV, bool abInvertY, const cColor& aColor)
{
    if (mbLog)
    {
        Log("   Drawing quad to: pos (%s) size (%s) uv: (%s)-(%s)\n", aPos.ToString().c_str(), avSize.ToString().c_str(), avMinUV.ToString().c_str(), avMaxUV.ToString().c_str());
    }
    if (abInvertY)
    {
        mpLowLevelGraphics->DrawQuad(aPos, avSize, cVector2f(avMinUV.x, avMaxUV.y), cVector2f(avMaxUV.x, avMinUV.y), aColor);
    }
    else
    {
...
void iRenderFunctions::DrawQuad(const cVector3f& aPos,
      ...
      const cColor& aColor)
{
    if (mbLog)
    {
        Log("   Drawing quad to: pos (%s) size (%s) uv0: (%s)-(%s) uv1: (%s)-(%s)\n", aPos.ToString().c_str(), avSize.ToString().c_str(), avMinUV0.ToString().c_str(),
              avMaxUV0.ToString().c_str(), avMinUV1.ToString().c_str(), avMaxUV0.ToString().c_str());
    }
...
void iRenderFunctions::CopyFrameBufferToTexure(iTexture* apTexture,
      const cVector2l& avPos,
      const cVector2l& avSize,
      const cVector2l& avTextureOffset,
      bool abTextureOffsetUsesRenderTargetPos)
{
    if (mbLog)
    {
        if (abTextureOffsetUsesRenderTargetPos)
            Log(" Copying current to texture '%s' (%d). Pos: %d:%d Size: %dx%d TextureOffset: %d:%d\n", apTexture->GetName().c_str(), apTexture, avPos.x, avPos.y, avSize.x,
                  avSize.y, avTextureOffset.x + mpCurrentRenderTarget->mvPos.x, avTextureOffset.y + mpCurrentRenderTarget->mvPos.y);
        else
            Log(" Copying current to texture '%s' (%d). Pos: %d:%d Size: %dx%d TextureOffset: %d:%d\n", apTexture->GetName().c_str(), apTexture, avPos.x, avPos.y, avSize.x,
                  avSize.y, avTextureOffset.x, avTextureOffset.y);
    }
    if (abTextureOffsetUsesRenderTargetPos)
