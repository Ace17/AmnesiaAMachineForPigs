    ...
    : iPostEffectType("Bloom", apGraphics, apResources)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        mpBlurProgram[i] = mpGraphics->CreateGpuProgramFromShaders("BloomBlur", "posteffect_bloom_blur_vtx.glsl", "posteffect_bloom_blur_frag.glsl", &vars);
    ...
    mpBloomProgram = mpGraphics->CreateGpuProgramFromShaders("BloomBlur", "deferred_base_vtx.glsl", "posteffect_bloom_add_frag.glsl", &vars);
    if (mpBloomProgram)
    {
