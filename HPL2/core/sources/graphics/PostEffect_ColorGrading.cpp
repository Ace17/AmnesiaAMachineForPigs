...
cPostEffectType_ColorGrading::cPostEffectType_ColorGrading(cGraphics* apGraphics, cResources* apResources)
    : iPostEffectType("ColorGrading", apGraphics, apResources)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        mpProgram[i] = mpGraphics->CreateGpuProgramFromShaders("ColorGrading" + cString::ToString(i), "deferred_base_vtx.glsl", "posteffect_color_grading_frag.glsl", &vars);
        if (i == 1)
        {
            if (mpProgram[i])
            {
...
cPostEffect_ColorGrading::cPostEffect_ColorGrading(cGraphics* apGraphics, cResources* apResources, iPostEffectType* apType)
...
iTexture* cPostEffect_ColorGrading::LoadLUT(tString asLUTName)
{
    ...
    else
    {
        ...
        if (cPlatform::FileExists(_W("textures/gradingmaps/") + cString::To16Char(asLUTName)) || cPlatform::FileExists(cString::To16Char(asLUTName)))
        {
