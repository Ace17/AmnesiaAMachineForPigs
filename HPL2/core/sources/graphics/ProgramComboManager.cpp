...
void cProgramComboManager::SetupGenerateProgramData(int alMainMode,
...
void cProgramComboManager::AddUniform(int alMainMode, const tString& asVarName, int alId)
{
...
iGpuProgram* cProgramComboManager::CreateProgramFromShaders(const tString& asProgramName, iGpuShader* apVtxShader, iGpuShader* apFragShader, bool abAddtoList)
{
    iGpuProgram* pProgram;
...
iGpuShader* cProgramComboManager::GetShaderForCombo(int alMainMode, int alBitFlags, const tString& asShaderName, tFlag aShaderType)
{
    ...
    for (size_t i = 0; i < comboSettings.mvFeatures.size(); ++i)
    {
        ...
        if ((alBitFlags & lCurrentBit) && (comboSettings.mvFeatures[i].mlShaders & aShaderType))
        {
    ...
    iGpuShader* pShader = CreateShaderFromFeatures(asShaderName, aShaderType, alBitFlags, &comboSettings.mvFeatures[0], (int)comboSettings.mvFeatures.size(),
...
iGpuShader* cProgramComboManager::CreateShaderFromFeatures(const tString& asShaderFile,
      tFlag aShaderType,
      int alBitFlags,
