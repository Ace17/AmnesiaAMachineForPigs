...
void cBitmap::Clear(const cColor& aColor, int alImage, int alMipMap)
{
    ...
    unsigned char vClearColor[] = { FloatColorToUChar(aColor.r), FloatColorToUChar(aColor.g), FloatColorToUChar(aColor.b), FloatColorToUChar(aColor.a) };
...
void cBitmap::Blit(cBitmap* apSrcBmp,
      ...
      int alSrcMipMap)
{
    ...
    if (avDestPosition.x >= mvSize.x || avDestPosition.y >= mvSize.y || avDestPosition.z >= mvSize.z)
    {
        return;
    }
    if (avSrcPosition.x >= apSrcBmp->GetSize().x || avSrcPosition.y >= apSrcBmp->GetSize().y || avSrcPosition.z >= apSrcBmp->GetSize().z)
    {
        return;
    }
    ...
    /*if(bOutOfBounds)
    {
                    Log(" (%d, %d, %d) (%d, %d, %d) (%dx%dx%d) (%dx%dx%d) (%dx%dx%d)\n",	avDestPosition.x,avDestPosition.y, avDestPosition.z,
                                                                                                                                                            avSrcPosition.x,avSrcPosition.y,
    avSrcPosition.z, avSrcSize.x, avSrcSize.y, avSrcSize.z, apSrcBmp->GetSize().x, apSrcBmp->GetSize().y, apSrcBmp->GetSize().z, GetSize().x, GetSize().y, GetSize().z
                                                                                            );
    }*/
}
...
void cBitmap::CopyPixel(unsigned char* apDest, ePixelFormat aDestFormat, unsigned char* apSrc, ePixelFormat aSrcFormat)
{
