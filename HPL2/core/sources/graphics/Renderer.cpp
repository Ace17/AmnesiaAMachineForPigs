...
cRenderSettings::cRenderSettings(bool abIsReflection)
{
    ...
    mlMinimumObjectsBeforeOcclusionTesting = 0; // 8;//8 should be good default, giving a good amount of colliders, or? Clarifiction: Minium num of object rendered until node
                                                // visibility tests start!
...
iRenderer::iRenderer(const tString& asName, cGraphics* apGraphics, cResources* apResources, int alNumOfProgramComboModes)
{
    ...
    mpDepthOnlyProgram = mpProgramManager->CreateProgramFromShaders("DepthOnly", "deferred_base_vtx.glsl", "deferred_depthonly_frag.glsl", &vars, false);
...
void iRenderer::Render(float afFrameTime,
      cFrustum* apFrustum,
      cWorld* apWorld,
...
void iRenderer::BeginRendering(float afFrameTime,
      cFrustum* apFrustum,
      cWorld* apWorld,
      cRenderSettings* apSettings,
      cRenderTarget* apRenderTarget,
      ...
      bool abAtStartOfRendering)
{
    ...
    InitAndResetRenderFunctions(apFrustum, apRenderTarget, apSettings->mbLog, apSettings->mbUseScissorRect, apSettings->mvScissorRectPos, apSettings->mvScissorRectSize);
...
{
    tString sName = "ShadowMap" + cString::ToString(avSize.x) + "x" + cString::ToString(avSize.y) + "_" + cString::ToString((int)mvShadowMapData[aResolution].size());
...
bool iRenderer::ShadowMapNeedsUpdate(iLight* apLight, cShadowMapData* apShadowData)
{
    ...
    bool bValid = cacheData.mpLight == apLight && cacheData.mlTransformCount == apLight->GetTransformUpdateCount() && cacheData.mfRadius == apLight->GetRadius();
...
void iRenderer::RenderZObject(iRenderable* apObject, cFrustum* apCustomFrustum)
{
    ...
    SetAlphaMode(pMaterial->GetTexture(eMaterialTexture_Alpha) || renderMode == eMaterialRenderMode_Z_Dissolve ? eMaterialAlphaMode_Trans : eMaterialAlphaMode_Solid);
...
void iRenderer::PushNodeChildrenToStack(tRendererSortedNodeSet& a_setNodeStack, iRenderableContainerNode* apNode, int alNeededFlags)
{
    ...
    for (; childIt != apNode->GetChildNodeList()->end(); ++childIt)
    {
        ...
        if (pChildNode->UsesFlagsAndVisibility() && (pChildNode->HasVisibleObjects() == false || (pChildNode->GetRenderFlags() & alNeededFlags) != alNeededFlags))
        {
            continue;
        }
        ...
        eCollision frustumCollision = apNode->GetPrevFrustumCollision() == eCollision_Inside ? eCollision_Inside : mpCurrentFrustum->CollideNode(pChildNode);
        ...
        else
        {
            ...
            cMath::CheckAABBLineIntersection(pChildNode->GetMin(), pChildNode->GetMax(), mpCurrentFrustum->GetOrigin(), pChildNode->GetCenter(), &vIntersection, NULL);
...
void iRenderer::AddAndRenderNodeOcclusionQuery(tNodeOcclusionPairList* apList, iRenderableContainerNode* apNode, bool abObjectsRendered)
{
    ...
    if (mbOnlyRenderPrevVisibleOcclusionObjects && mlOnlyRenderPrevVisibleOcclusionObjectsFrameCount >= 1)
    {
...
static void PrintNodeDebugContents(tRendererSortedNodeSet& a_setNodes)
{
    ...
    for (; it != a_setNodes.end(); ++it)
    {
        ...
        Log("   Node: %d, inside: %d, children %d, objects: %d\n", pNode, pNode->IsInsideView(), pNode->GetChildNodeList()->size(), pNode->GetObjectNum());
    }
}
...
void iRenderer::CheckForVisibleObjectsAddToListAndRenderZ(cVisibleRCNodeTracker* apVisibleNodeTracker,
      tObjectVariabilityFlag alObjectTypes,
      ...
      tRenderCHCObjectCallbackFunc apRenderObjectCallback)
{
    ...
    while (setNodeStack.empty() == false || lstNodeOcclusionPairs.empty() == false)
    {
        ...
        if (setNodeStack.empty() == false)
        {
            ...
            if (bNearPlaneInsideNode == false && lMinRenderedObjects <= 0 && (bWasVisible == false || bNodeIsLeaf))
            {
                ////////////////
                ...
                if (mbOnlyRenderPrevVisibleOcclusionObjects && mlOnlyRenderPrevVisibleOcclusionObjectsFrameCount >= 1 && bRenderObjects && bWasVisible)
                {
...
void iRenderer::GetShadowCastersIterative(iRenderableContainerNode* apNode, eCollision aPrevCollision)
{
    ...
    if (apNode->HasObjects())
    {
        for (tRenderableListIt it = apNode->GetObjectList()->begin(); it != apNode->GetObjectList()->end(); ++it)
        {
            ...
            if (CheckObjectIsVisible(pObject, eRenderableFlag_ShadowCaster) == false || pObject->GetMaterial() == NULL || pObject->GetMaterial()->GetType()->IsTranslucent())
            {
                continue;
            }
            ...
            /////////
            // Check if in frustum
            if (frustumCollision != eCollision_Inside && gpLightFrustum->CollideBoundingVolume(pObject->GetBoundingVolume()) == eCollision_Outside)
            {
                continue;
            }
...
void iRenderer::RenderShadowMap(iLight* apLight, iFrameBuffer* apShadowBuffer)
{
    ...
    mpLowLevelGraphics->SetPolygonOffset(
          mpCurrentSettings->mfShadowMapBias * apLight->GetShadowMapBiasMul(), mpCurrentSettings->mfShadowMapSlopeScaleBias * apLight->GetShadowMapSlopeScaleBiasMul());
    ...
    if (apLight->GetOcclusionCullShadowCasters())
    {
        ...
        CheckForVisibleObjectsAddToListAndRenderZ(
              apLight->GetVisibleNodeTracker(), apLight->GetShadowCastersAffected(), eRenderableFlag_ShadowCaster, false, RenderShadowCasterCHCStaticCallback);
    }
    else
    {
...
bool iRenderer::CheckNodeIsVisible(iRenderableContainerNode* apNode)
{
    ...
    for (size_t i = 0; i < mvCurrentOcclusionPlanes.size(); ++i)
    {
        ...
        if (cMath::CheckPlaneAABBCollision(plane, apNode->GetMin(), apNode->GetMax(), apNode->GetCenter(), apNode->GetRadius()) == eCollision_Outside)
        {
            return false;
        }
    }
...
bool iRenderer::CheckFogAreaRayIntersection(cMatrixf& a_mtxInvBoxModelMatrix, const cVector3f& avBoxSpaceRayStart, const cVector3f& avRayDir, float& afEntryDist, float& afExitDist)
{
...
iVertexBuffer*
iRenderer::CreateQuadVertexBuffer(eVertexBufferType aType, const cVector3f& avPos, const cVector2f& avSize, const cVector2f& avMinUV, const cVector2f& avMaxUV, bool abInvertY)
{
    iVertexBuffer* pVtxBuffer = mpLowLevelGraphics->CreateVertexBuffer(aType, eVertexBufferDrawType_Quad, eVertexBufferUsageType_Dynamic);
