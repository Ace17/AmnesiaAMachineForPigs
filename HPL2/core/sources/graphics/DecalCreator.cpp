...
cDecalCreator::cDecalCreator(iLowLevelGraphics* apLowLevelGraphics, cResources* apResources)
    : mvDecalSize(0)
{
...
bool cDecalCreator::Compile()
{
    if (mbUpdated)
    {
        ...
        mpDecalVB = mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static);
        ...
        for (int i = 0; i < mpDecalVB->GetVertexNum(); ++i)
        {
            ...
            cVector3f vPointRelToCenter = cVector3f(pVertexPositions[lBaseIdx], pVertexPositions[lBaseIdx + 1], pVertexPositions[lBaseIdx + 2]) - mvDecalPosition;
...
bool cDecalCreator::AddPolygon(int alVertexCount,
      const cVector3f* apVertices,
      const cVector3f* apNormals,
      iVertexBuffer* apDecalVB,
      const cMatrixf& amtxWorldMatrix,
      const cMatrixf& amtxWorldNormalRot)
{
...
void cDecalCreator::ClipMesh(cSubMeshEntity* apSubMesh, iVertexBuffer* apDecalVB)
{
    ...
    for (int j = 0; j < pSubMeshVB->GetIndexNum(); j += 3)
    {
        ...
        for (int k = 0; k < 3; ++k)
        {
            ...
            vTriangle[k] = cVector3f(pVertices[lPosBaseIdx], pVertices[lPosBaseIdx + 1], pVertices[lPosBaseIdx + 2]);
            vNormal[k] = cVector3f(pNormals[lNrmBaseIdx], pNormals[lNrmBaseIdx + 1], pNormals[lNrmBaseIdx + 2]);
        }
...
int cDecalCreator::ClipPolygon(int alVertexCount,
      const cVector3f* apVertices,
      const cVector3f* apNormals,
      cVector3f* apNewVertices,
      cVector3f* apNewNormals,
      const std::vector<cPlanef>& avPlanes)
{
...
int cDecalCreator::ClipPolygonAgainstPlane(const cPlanef& aPlane,
      int alVertexCount,
      const cVector3f* apVertices,
      const cVector3f* apNormals,
      cVector3f* apNewVertices,
