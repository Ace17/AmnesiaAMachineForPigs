...
cWidgetGroup::cWidgetGroup(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_Group, apSet, apSkin)
{
    mpHeader = NULL;
...
void cWidgetGroup::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mbHeaderBreaksUpperBorder)
    {
        ...
        if (mpGfxBackground)
        {
            ...
            mpSet->DrawGfx(mpGfxBackground, vOffset, vSize, cColor(1, 1));
        }

        {
            ...
            // Borders
            // Right
            mpSet->DrawGfx(mvGfxBorders[0], cVector3f(mvSize.x - mvGfxBorders[0]->GetActiveSize().x, mvGfxCorners[1]->GetActiveSize().y, 0),
                  cVector2f(mvGfxBorders[0]->GetImageSize().x, mvSize.y - (mvGfxCorners[2]->GetActiveSize().y + mvGfxCorners[1]->GetActiveSize().y)));
            // Left
            mpSet->DrawGfx(mvGfxBorders[1], cVector3f(0, mvGfxCorners[0]->GetActiveSize().y, 0),
                  cVector2f(mvGfxBorders[1]->GetImageSize().x, mvSize.y - (mvGfxCorners[3]->GetActiveSize().y + mvGfxCorners[0]->GetActiveSize().y)));

            // Up
            mpSet->DrawGfx(mvGfxBorders[2], cVector3f(mvGfxCorners[0]->GetActiveSize().x, 0, 0), cVector2f(fPreHeaderWidth - 4, mvGfxBorders[2]->GetActiveSize().y));
            mpSet->DrawGfx(
                  mvGfxBorders[2], vPostHeaderPos + cVector3f(4, 0, 0), cVector2f(fPostHeaderWidth - mvGfxCorners[1]->GetActiveSize().x - 4, mvGfxBorders[2]->GetActiveSize().y));

            // Down
            mpSet->DrawGfx(mvGfxBorders[3], cVector3f(mvGfxCorners[3]->GetActiveSize().x, mvSize.y - mvGfxCorners[3]->GetActiveSize().y, 0),
                  cVector2f(mvSize.x - (mvGfxCorners[2]->GetActiveSize().x + mvGfxCorners[3]->GetActiveSize().x), mvGfxBorders[3]->GetImageSize().y));
            ...
            // Right Down
            mpSet->DrawGfx(mvGfxCorners[2], cVector3f(mvSize.x - mvGfxCorners[2]->GetActiveSize().x, mvSize.y - mvGfxCorners[2]->GetActiveSize().y, 0));
            // Left Down
