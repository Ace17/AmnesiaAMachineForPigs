...
cWidgetLabel::cWidgetLabel(cGuiSet* apSet, cGuiSkin* apSkin)
...
void cWidgetLabel::DrawText(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mbWordWrap)
    {
        ...
        tWStringVec vRows;
        mpDefaultFontType->GetWordWrapRows(mvSize.x, fHeight, mvDefaultFontSize, msText, &vRows);
        ...
        for (size_t i = 0; i < vRows.size(); ++i)
        {
            ...
            {
                DrawDefaultText(vRows[i], GetGlobalPosition() + vOffset - cVector3f(0, mfWordWrapOffset, 0), mTextAlign, cColor(0.5f, 1));
    ...
    else
    {
        if (mlMaxCharacters >= 0 && (int)msText.length() > mlMaxCharacters)
        {
            ...
            {
                DrawDefaultText(cString::SubW(msText, 0, mlMaxCharacters), GetGlobalPosition() + vOffset, mTextAlign, cColor(0.5f, 1));
        ...
        else
        {
            ...
            {
                DrawDefaultText(msText, GetGlobalPosition() + vOffset, mTextAlign, cColor(0.5f, 1));
...
void cWidgetLabel::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mbDrawBackGround)
    {
        mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() - cVector3f(2, 2, 0.001f), mvSize + 4, mBackGroundColor);
    }
