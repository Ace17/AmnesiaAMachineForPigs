...
bool cGuiPopUpFilePicker::Button_Pressed(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    /////////////////////////////
    // Save/Load button
    else if (apWidget == mvButtons[1] || apWidget == mpCurrentFileName)
    {
        if (mpCurrentFileName->GetText() == _W(""))
            mpSet->CreatePopUpMessageBox(_W("Error"), _W("File name must not be empty"), _W("OK"), _W(""), NULL, NULL);
        else
        {
            ...
            case eFilePickerType_Save:
                ...
                if (bFileExists)
                {
                    mpSet->CreatePopUpMessageBox(_W("Warning"), _W("File already exists. Overwrite?"), _W("Yes"), _W("No"), this, kGuiCallback(MessageBoxReturnCallback));
                }
                else
                {
                    BuildSaveFileName();
            ...
            case eFilePickerType_Load:
                ...
                {
                    mpSet->CreatePopUpMessageBox(_W("Error"), _W("File does not exist. Please choose a valid file."), _W("OK"), _W(""), NULL, NULL);
                }
                else
                {
                    BuildLoadFileList();
...
void cGuiPopUpFilePicker::Init()
{
    ...
    mpCurrentDirectory = mpSet->CreateWidgetComboBox(cVector3f(30, 60, 0.1f), cVector2f(400, mpWindow->GetDefaultFontSize().y + 2), _W(""), mpWindow);
    ...
    for (int i = 0; i < 3; ++i)
    {
        mvButtons[i] = mpSet->CreateWidgetButton(0, 0, _W(""), mpWindow);
    ...
    mpCurrentFileName = mpSet->CreateWidgetTextBox(cVector3f(30, 425, 0.5f), cVector2f(400, mpWindow->GetDefaultFontSize().y + 2), msStartFilename, mpWindow);
    ...
    // Filters ComboBox
    mpFilterList = mpSet->CreateWidgetComboBox(cVector3f(30, 450, 0.5f), cVector2f(300, mpWindow->GetDefaultFontSize().y + 2), _W(""), mpWindow);
    mpFilterList->AddCallback(eGuiMessage_SelectionChange, this, kGuiCallback(FilterList_OnSelectionChange));

    // File ListBox
    mpFileList = mpSet->CreateWidgetMultiPropertyListBox(cVector3f(30, 90, 1), cVector2f(540, 300), mpWindow);
