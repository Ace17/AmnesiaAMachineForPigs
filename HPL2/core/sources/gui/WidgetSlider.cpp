...
cWidgetSlider::cWidgetSlider(cGuiSet* apSet, cGuiSkin* apSkin, eWidgetSliderOrientation aOrientation)
...
void cWidgetSlider::UpdateBarProperties()
{
    ...
    if (mOrientation == eWidgetSliderOrientation_Vertical)
    {
        ...
        mvBarSize = cVector2f(mvSize.x, ((float)mlBarValueSize / (float)(mlMaxValue + 1)) * (mfSliderSize));
    ...
    else
    {
        ...
        mvBarSize = cVector2f(((float)mlBarValueSize / (float)(mlMaxValue + 1)) * (mfSliderSize), mvSize.y);
...
void cWidgetSlider::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mOrientation == eWidgetSliderOrientation_Vertical)
    {
        mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() + cVector3f(0, mfButtonSize, 0.1f), cVector2f(mvSize.x, mvSize.y - mfButtonSize * 2));
    }
    // Horizontal
    else
    {
        mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() + cVector3f(mfButtonSize, 0, 0.1f), cVector2f(mvSize.x - mfButtonSize * 2, mvSize.y));
    }

    ////////////////////////////////
    // Borders and button background
    DrawBordersAndCorners(mpGfxButtonBackground, mvGfxBorders, mvGfxCorners, GetGlobalPosition() + mvBarPos, mvBarSize);
}
