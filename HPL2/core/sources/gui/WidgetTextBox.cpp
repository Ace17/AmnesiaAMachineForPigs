...
cWidgetTextBox::cWidgetTextBox(cGuiSet* apSet, cGuiSkin* apSkin, eWidgetTextBoxInputType aType)
    : iWidget(eWidgetType_TextBox, apSet, apSkin)
{
    mvButtons[0] = NULL;
    mvButtons[1] = NULL;
    mInputType = aType;
...
{
    return cVector2f(
          mvSize.x - mvGfxCorners[0]->GetActiveSize().x - mvGfxCorners[1]->GetActiveSize().x, mvSize.y - mvGfxCorners[0]->GetActiveSize().y - mvGfxCorners[2]->GetActiveSize().y);
}
...
{
    float fTextPos = WorldToLocalPosition(avWorldPos).x - mvGfxCorners[0]->GetActiveSize().x + 3;
...
float cWidgetTextBox::CharToLocalPos(int alChar)
{
    ...
    {
        fMarkerPos = mpDefaultFontType->GetLength(mvDefaultFontSize, cString::SubW(msText, mlFirstVisibleChar, alChar - mlFirstVisibleChar).c_str());
    }
    return fMarkerPos;
}
...
{
    mvSize.y = mvGfxBorders[0]->GetActiveSize().y + mvGfxBorders[2]->GetActiveSize().y + mvDefaultFontSize.y + 2 * 2;

    float fButtonWidth = 0;
    if (mInputType == eWidgetTextBoxInputType_Numeric && mvButtons[0])
        fButtonWidth = mvButtons[0]->GetSize().x;

    mfTextMaxSize = mvSize.x - mvGfxBorders[0]->GetActiveSize().x - mvGfxBorders[1]->GetActiveSize().x - 3 * 2 - mfMaxTextSizeNeg - fButtonWidth;
...
void cWidgetTextBox::OnChangeText()
{
    if (msText == _W(""))
        ...
        mlVisibleCharSize = GetLastCharInSize(mlFirstVisibleChar, mfTextMaxSize, 0) - mlFirstVisibleChar;
...
void cWidgetTextBox::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    DrawDefaultText(cString::SubW(msText, mlFirstVisibleChar, mlVisibleCharSize), GetGlobalPosition() + vTextAdd, eFontAlign_Left, col);

    // Marker
    if (mlMarkerCharPos >= 0)
    {
        float fMarkerPos = CharToLocalPos(mlMarkerCharPos);
        mpSet->DrawGfx(mpGfxMarker, GetGlobalPosition() + vTextAdd + cVector3f(fMarkerPos, 0, 0.1f), cVector2f(2, mvDefaultFontSize.y));
        ...
        if (mlSelectedTextEnd >= 0)
        {
            ...
            mpSet->DrawGfx(mpGfxSelectedTextBack, GetGlobalPosition() + vTextAdd + cVector3f(fPos, 0, 0.01f), cVector2f(fSize, mvDefaultFontSize.y));

            DrawDefaultTextHighlight(cString::SubW(msText, lHighlightStart, lHighlightSize), GetGlobalPosition() + vTextAdd + cVector3f(fPos, 0, 0.02f), eFontAlign_Left);
        }
    }
...
bool cWidgetTextBox::OnKeyPress(const cGuiMessageData& aData)
{
    ...
    else if (mod & eKeyModifier_Ctrl)
    {
        ...
        else if (key == eKey_V)
        {
            ...
            if (mlSelectedTextEnd < 0)
            {
                if (mlMaxCharacters == -1 || (int)msText.size() + (int)sExtra.size() <= mlMaxCharacters)
                {
                    SetText(cString::SubW(msText, 0, mlMarkerCharPos) + sExtra + cString::SubW(msText, mlMarkerCharPos));
            ...
            {
                if (mlMaxCharacters < 0 || (int)sExtra.size() <= lSelectSize || (int)sExtra.size() + (int)msText.size() - lSelectSize <= mlMaxCharacters)
                {
                    SetText(cString::SubW(msText, 0, lStart) + sExtra + cString::SubW(msText, lEnd));
    ...
    else if (key == eKey_Delete || key == eKey_BackSpace)
    {
        ...
        else
        {
            if (key == eKey_Delete)
            {
                SetText(cString::SubW(msText, 0, mlMarkerCharPos) + cString::SubW(msText, mlMarkerCharPos + 1));
            ...
            else
            {
                if (mlMarkerCharPos != 0)
                {
                    SetText(cString::SubW(msText, 0, mlMarkerCharPos - 1) + cString::SubW(msText, mlMarkerCharPos));
                    SetMarkerPos(mlMarkerCharPos - 1);
    ...
    else
    {
        ...
        if (mInputType == eWidgetTextBoxInputType_Numeric)
        {
            if (unicode >= lFirstNum && unicode <= lLastNum || (unicode == _W('.') || unicode == _W('-')) && mpDefaultFontType->GetGlyph(unicode - lFirstFontChar))
            {
                if (mlSelectedTextEnd < 0)
                {
                    if (unicode == _W('-'))
                    ...
                    else if (unicode == _W('.'))
                    {
                        ...
                        {
                            SetText(cString::SubW(msText, 0, mlMarkerCharPos) + unicode + cString::SubW(msText, mlMarkerCharPos));
                            SetMarkerPos(mlMarkerCharPos + 1);
                ...
                else
                {
                    ...
                    else if (unicode == _W('.'))
                    {
                        ...
                        if (lNumDots == 0 || lNumDots == 1 && lDotPos >= lStart && lDotPos <= lEnd)
                        {
                            SetText(cString::SubW(msText, 0, lStart) + unicode + cString::SubW(msText, lEnd));
        ...
        {
            if (unicode >= lFirstFontChar && unicode <= lLastFontChar && mpDefaultFontType->GetGlyph(unicode - lFirstFontChar))
            {
                if (mlSelectedTextEnd < 0)
                {
