...
cWidgetTabLabel::cWidgetTabLabel(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_Label, apSet, apSkin)
{
    mpGfxIcon = NULL;
...
void cWidgetTabLabel::OnChangeText()
{
    ...
    float fBordersWidth = pTabFrame->mvGfxTabLabelBorders[0]->GetImageSize().x + pTabFrame->mvGfxTabLabelBorders[1]->GetImageSize().x;
    float fNewWidth = mpDefaultFontType->GetLength(mpDefaultFont->mvSize, msText.c_str()) + pTabFrame->mfTextLeftPadding + pTabFrame->mfTextRightPadding + fBordersWidth;
...
void cWidgetTabLabel::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    DrawBordersAndCorners(pBG, pTabFrame->mvGfxTabLabelBorders, pTabFrame->mvGfxTabLabelCorners, GetGlobalPosition(), mvSize);
}
...
cWidgetTab::cWidgetTab(cWidgetTabFrame* apParent)
    : iWidget(eWidgetType_Tab, apParent->GetSet(), apParent->GetSet()->GetSkin())
{
    LoadGraphics();
...
bool cWidgetTab::PointIsInside(const cVector2f& avPoint, bool abOnlyClipped)
{
    ...
    if (avPoint.x < vFrameGlobalPos.x || avPoint.x > vFrameGlobalPos.x + vFrameSize.x || avPoint.y < vFrameGlobalPos.y || avPoint.y > vFrameGlobalPos.y + vFrameSize.y)
    {
        if (bLabelVisible == false || avPoint.x < vLabelGlobalPos.x || avPoint.x > vLabelGlobalPos.x + vLabelSize.x || avPoint.y < vLabelGlobalPos.y ||
              avPoint.y > vLabelGlobalPos.y + vLabelSize.y)
            return false;
        else
            return true;
    }
...
void cWidgetTab::OnInit()
{
    ...
    mpTabLabel = mpSet->CreateWidgetTabLabel(0, cVector2f(0, fLabelHeight), msText, NULL);
    ...
    cVector2f vCornerSize = cVector2f(mpParentTabFrame->mvGfxTabCorners[0]->GetActiveSize().x + mpParentTabFrame->mvGfxTabCorners[1]->GetActiveSize().x,
          mpParentTabFrame->mvGfxTabCorners[0]->GetActiveSize().y + mpParentTabFrame->mvGfxTabCorners[2]->GetActiveSize().y);

    mpFrame = mpSet->CreateWidgetFrame(0, mvSize - cVector2f(vCornerSize.x, fLabelHeight + fFrameOffset + vCornerSize.y), false, NULL, mpParentTabFrame->mbHoriScrollEnabled,
          mpParentTabFrame->mbVertScrollEnabled);
...
void cWidgetTab::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    DrawBordersAndCorners(
          mpParentTabFrame->mpGfxTabBG, mpParentTabFrame->mvGfxTabBorders, mpParentTabFrame->mvGfxTabCorners, mpFrame->GetGlobalPosition(), mpFrame->GetSize(), true);
}
...
cWidgetTabFrame::cWidgetTabFrame(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_TabFrame, apSet, apSkin)
{
    mfBackgroundZ = 0;
    mfForegroundZ = 0.9f;
