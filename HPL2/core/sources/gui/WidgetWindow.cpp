...
cWidgetWindow::cWidgetWindow(cGuiSet* apSet, cGuiSkin* apSkin, tWidgetWindowButtonFlag alFlags)
    : iWidget(eWidgetType_Window, apSet, apSkin)
{
...
void cWidgetWindow::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mbDrawLabel)
    {
        ...
        mpSet->DrawGfx(mpGfxLabel, GetGlobalPosition() + cVector3f(mvGfxCorners[0]->GetActiveSize().x, mvGfxCorners[0]->GetActiveSize().y, 0.2f), vLabelSize, col);
    }

    ////////////////////////////////
    // Label text
    DrawSkinText(msText, eGuiSkinFont_WindowLabel,
          GetGlobalPosition() + cVector3f(mvGfxCorners[0]->GetActiveSize().x + mvLabelTextOffset.x, mvGfxCorners[0]->GetActiveSize().y + mvLabelTextOffset.y, 0.4f));
