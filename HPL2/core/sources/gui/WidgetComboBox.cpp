...
cWidgetComboBox::cWidgetComboBox(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_ComboBox, apSet, apSkin)
{
...
{
    if (alX >= GetItemNum() || mlSelectedItem == alX)
        return;
...
void cWidgetComboBox::OpenMenu()
{
    ...
    mvMenuPos = cVector3f(0, mvSize.y + mvGfxBorders[1]->GetActiveSize().y, mpSet->GetPopUpZ() + 1);
    ...
    if ((int)mvItems.size() > mlMaxItems)
    {
        ...
        mpSlider->SetSize(cVector2f(20, mfMenuHeight - mvGfxBorders[1]->GetActiveSize().y - mvGfxBorders[2]->GetActiveSize().y));
...
bool cWidgetComboBox::ButtonPress(iWidget* apWidget, const cGuiMessageData& aData)
{
    if (aData.mlVal == eGuiMouseButton_Left)
    {
        if (mbMenuOpen)
        {
            ...
            if (vLocalPos.x >= mvMenuPos.x && vLocalPos.y <= mvMenuPos.x + mvSize.x && vLocalPos.y >= mvMenuPos.y && vLocalPos.y <= mvMenuPos.y + mfMenuHeight)
            {
...
void cWidgetComboBox::OnChangeSize()
{
    if (mpText && mpButton && mpSlider)
    {
        ...
        mpButton->SetPosition(cVector3f(mvSize.x - (mvSize.x - vBackSize.x) / 2 - mfButtonWidth, (mvSize.y - vBackSize.y) / 2, 0.3f));
...
void cWidgetComboBox::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    if (mbMenuOpen)
    {
        ...
        ////////////////////////////
        // Draw background
        mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() + mvMenuPos, cVector2f(mvSize.x, mfMenuHeight));

        ////////////////////////////////
        // Draw borders
        DrawBordersAndCorners(NULL, mvGfxBorders, mvGfxCorners, GetGlobalPosition() + mvMenuPos + cVector3f(0, 0, 0.2f), cVector2f(mvSize.x, mfMenuHeight));
...
bool cWidgetComboBox::DrawText(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    ///////////////////////////////
    // Draw text entries
    cVector3f vPos = GetGlobalPosition() + mvMenuPos + cVector3f(3, 2, 0.2f);
    ...
    for (int i = mlFirstItem; i < (int)mvItems.size(); ++i)
    {
        ...
        {
            mpSet->DrawGfx(mpGfxSelection, vPos - cVector3f(3, 0, 0.1f), cVector2f(mvSize.x, mvDefaultFontSize.y));
...
bool cWidgetComboBox::OnLostFocus(const cGuiMessageData& aData)
{
    ...
    {
        cVector3f vLocal = WorldToLocalPosition(aData.mvPos);
        if (vLocal.x < mvMenuPos.x || vLocal.x > mvMenuPos.x + mvSize.x || vLocal.y < mvMenuPos.y || vLocal.y > mvMenuPos.y + mfMenuHeight)
            CloseMenu();
    }
...
bool cWidgetComboBox::PointIsInside(const cVector2f& avPoint, bool abOnlyClipped)
{
    ...
    if (vMousePos.x < 0 || vMousePos.x > mvSize.x || vMousePos.y < 0 || vMousePos.y > mvSize.y)
    {
        bInside = false;
    }
    else
    ...
    if (bInside == false)
    {
        if (mbMenuOpen)
        {
            if (vMousePos.x >= mvMenuPos.x && vMousePos.x <= mvMenuPos.x + mvSize.x && vMousePos.y >= mvMenuPos.y && vMousePos.y <= mvMenuPos.y + mfMenuHeight)
                bInside = true;
        }
    }
