...
cGuiSet::cGuiSet(const tString& asName, cGui* apGui, cGuiSkin* apSkin, cResources* apResources, cGraphics* apGraphics, cSound* apSound, cScene* apScene)
{
    mpGui = apGui;
    mpSkin = NULL;
...
{
    cGuiGlobalShortcut* pShortcut = hplNew(cGuiGlobalShortcut, (alKeyModifiers, aKey, apWidget, aMessage, abBypassVisibility, abBypassEnabled));
...
void cGuiSet::Render(cFrustum* apFrustum)
{
    ...
    if (mbIs3D)
    {
        ...
        // Also scale to size
        cVector3f vPreScale = cVector3f(mv3DSize.x / mvVirtualSize.x, -mv3DSize.y / mvVirtualSize.y, mv3DSize.z / (mfVirtualMaxZ - mfVirtualMinZ));
...
void cGuiSet::DrawGfx(cGuiGfxElement* apGfx,
...
void cGuiSet::DrawFont(const tWString& asText, iFontData* apFont, const cVector3f& avPos, const cVector2f& avSize, const cColor& aColor, eFontAlign aAlign, eGuiMaterial aMaterial)
{
...
void cGuiSet::DrawFont(iFontData* apFont, const cVector3f& avPos, const cVector2f& avSize, const cColor& aColor, eFontAlign aAlign, eGuiMaterial aMaterial, const wchar_t* fmt, ...)
{
    va_list ap;
    if (fmt == NULL)
        return;
...
void cGuiSet::DrawFont(iFontData* apFont, const cVector3f& avPos, const cVector2f& avSize, const cColor& aColor, const wchar_t* fmt, ...)
{
    va_list ap;
    if (fmt == NULL)
        return;
...
cWidgetWindow*
cGuiSet::CreateWidgetWindow(tWidgetWindowButtonFlag alFlags, const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetFrame*
cGuiSet::CreateWidgetFrame(const cVector3f& avLocalPos, const cVector2f& avSize, bool abDrawFrame, iWidget* apParent, bool abHScrollBar, bool abVScrollBar, const tString& asName)
{
...
cWidgetButton*
cGuiSet::CreateWidgetButton(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, bool abToggleable, const tString& asName)
{
...
cWidgetLabel* cGuiSet::CreateWidgetLabel(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetSlider*
cGuiSet::CreateWidgetSlider(eWidgetSliderOrientation aOrientation, const cVector3f& avLocalPos, const cVector2f& avSize, int alMaxValue, iWidget* apParent, const tString& asName)
{
...
cWidgetCheckBox* cGuiSet::CreateWidgetCheckBox(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetListBox* cGuiSet::CreateWidgetListBox(const cVector3f& avLocalPos, const cVector2f& avSize, iWidget* apParent, const tString& asName)
{
...
cWidgetMultiPropertyListBox* cGuiSet::CreateWidgetMultiPropertyListBox(const cVector3f& avLocalPos, const cVector2f& avSize, iWidget* apParent, const tString& asName)
{
...
cWidgetComboBox* cGuiSet::CreateWidgetComboBox(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetMenuItem* cGuiSet::CreateWidgetMenuItem(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetContextMenu* cGuiSet::CreateWidgetContextMenu(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetTabLabel* cGuiSet::CreateWidgetTabLabel(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetTab* cGuiSet::CreateWidgetTab(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cWidgetGroup* cGuiSet::CreateWidgetGroup(const cVector3f& avLocalPos, const cVector2f& avSize, const tWString& asText, iWidget* apParent, const tString& asName)
{
...
cGuiPopUpMessageBox* cGuiSet::CreatePopUpMessageBox(const tWString& asLabel,
      const tWString& asText,
...
{
    cGuiPopUpMessageBox* pMessageBox = hplNew(cGuiPopUpMessageBox, (this, asLabel, asText, asButton1, asButton2, apCallbackObject, apCallback));
...
cGuiPopUpFilePicker* cGuiSet::CreatePopUpSaveFilePicker(tWString& asFileName,
 ...
 * \return
 */
cGuiPopUpFilePicker* cGuiSet::CreatePopUpLoadFilePicker(tWStringVec& avFileList,
...
void cGuiSet::DrawTextFromCharArry(const wchar_t* apString,
      iFontData* apFont,
      const cVector2f& avSize,
...
static void SetClipArea(iLowLevelGraphics* pLowLevelGraphics, cGuiClipRegion* apRegion)
{
    ...
    if (apRegion->mRect.w > 0)
    {
        ...
        if (kLogRender)
            Log("-- Clip region: %d Clipping: x %f y %f w %f h %f\n", apRegion, apRegion->mRect.x, apRegion->mRect.y, apRegion->mRect.w, apRegion->mRect.h);
    }
    else
    {
        if (kLogRender)
...
void cGuiSet::RenderClipRegion()
{
    ...
    while (it != setRenderObjects.end())
    {
        ...
        do
        {
            ...
            if (object.mbRotated)
            {
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    pLowLevelGraphics->AddVertexToBatch_Raw(cVector3f(vVtxPos.x + vPos.x, vVtxPos.y + vPos.y, vPos.z), vtx.col * object.mColor, vtx.tex);
                }
            }
            else
            {
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    pLowLevelGraphics->AddVertexToBatch_Raw(
                          cVector3f(vVtxPos.x * object.mvSize.x + vPos.x, vVtxPos.y * object.mvSize.y + vPos.y, vPos.z), vtx.col * object.mColor, vtx.tex);
                }
            }
        ...
        } while (pTexture == pLastTexture && pMaterial == pLastMaterial && pClipRegion == pLastClipRegion);

        //////////////////////////////
        // Render batch
        pLowLevelGraphics->FlushQuadBatch(eVtxBatchFlag_Position | eVtxBatchFlag_Texture0 | eVtxBatchFlag_Color0, false);
        ...
        if (pLastMaterial != pMaterial || it == setRenderObjects.end())
        {
            ...
            if (kLogRender)
                Log("Material %d '%s' after. new: %d '%s'\n", pLastMaterial, pLastMaterial->GetName().c_str(), pMaterial, pMaterial->GetName().c_str());
        }
    }
            ...
            if (pWidgetUnderMouse == NULL || pWidgetUnderMouse->IsConnectedTo(pWidget) || pWidget->IsConnectedTo(pWidgetUnderMouse))
            {
                ...
                if (pWidget->GetMouseIsOver() == false)
                {
                    ...
                    if (pWidget->HasFocusNavigation() && pWidget->IsVisible() && pWidget->IsEnabled())
                    {
                        ...
                        if (pOldFocus == NULL || (pOldFocus->GetParent() == pWidget->GetParent() && pOldFocus->GetSet() == pWidget->GetSet()))
                        {
...
bool cGuiSet::OnUIArrowPress(const cGuiMessageData& aData)
{
    ...
    if (bRet == false)
    {
        ...
        if (pNextWidget && pNextWidget->IsEnabled() && pNextWidget->IsVisible() && (mpAttentionWidget == NULL || pNextWidget->IsConnectedTo(mpAttentionWidget)))
        {
            iWidget* pOldFocus = mpFocusedWidget;
...
bool cGuiSet::DrawMouse(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    {
        DrawGfx(mpGfxCurrentPointer, cVector3f(mvMousePos.x, mvMousePos.y, mfMouseZ), mpGfxCurrentPointer->GetImageSize(), cColor(1, 1));
    }
      ...
      bool cGuiSet::DrawFocus(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if (HasFocus() && mpFocusedWidget && mbDrawFocus && (mpFocusedWidget == mpDefaultFocusNavWidget || mpFocusedWidget->HasFocusNavigation()))
    {
