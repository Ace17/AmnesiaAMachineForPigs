...
cWidgetCheckBox::cWidgetCheckBox(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_CheckBox, apSet, apSkin)
{
    mbChecked = false;
    mbPressed = false;
...
void cWidgetCheckBox::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    ////////////////////////////////
    // Box
    mpSet->DrawGfx(mvGfxBox[IsEnabled()][mbChecked], GetGlobalPosition() + cVector3f(0, mvBoxSize.y / 2 - mvGfxBox[0][0]->GetActiveSize().y / 2, 0));
