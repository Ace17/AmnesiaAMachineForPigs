...
cWidgetFrame::cWidgetFrame(cGuiSet* apSet, cGuiSkin* apSkin, bool abHScrollBar, bool abVScrollBar)
    : iWidget(eWidgetType_Frame, apSet, apSkin)
{
...
void cWidgetFrame::OnDrawAfterClip(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mbDrawBackground)
    {
        mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() + cVector3f(0, 0, mfBackgroundZ), mvSize, mBackGroundColor);
    }
}
...
{
    if (aData.mlVal & eGuiMouseButton_Left || aData.mlVal & eGuiMouseButton_Right)
        return true;
