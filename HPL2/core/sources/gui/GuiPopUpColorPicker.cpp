...
iGraphicPickerMode::iGraphicPickerMode(cGuiPopUpColorPicker* apPicker, const tWString& asName, int alSliderParamIndex, const cVector3f& avMaxValues)
    : mpPicker(apPicker)
    , msName(asName)
    , mlSliderParamIndex(alSliderParamIndex)
    , mvMaxValues(avMaxValues)
{
...
void iGraphicPickerMode::OnDrawBox(iWidget* apWidget)
{
    ...
    mpPicker->mpSet->DrawGfx(
          mpPicker->mpGfxHMarker, vPos + cVector3f(0, mvColorMapPos.y * (vSize.y - 1), 0), cVector2f(vSize.x, 1), cColor(mvColorMapPos.y, 1), eGuiMaterial_Diffuse);
...
void iGraphicPickerMode::UpdateSliderMarkerColor()
{
    ...
    mpSliderBmp->GetPixel(0, 0, cVector3l(cMath::FastPositiveFloatToInt(mfColorSliderPos * mpSliderBmp->GetSize().x), 0, 0), pixelData);
...
cHSBMode::cHSBMode(cGuiPopUpColorPicker* apPicker, int alIndex)
    : iGraphicPickerMode(apPicker, _W("HSB"), alIndex, cVector3f(360.0f, 1.0f, 1.0f))
{
}
...
{
    cVector2f vMapValues = cVector2f(mvMaxValues.v[mlRowIndex] * avPos.x, mvMaxValues.v[mlColIndex] * (1 - avPos.y));
...
cVector2f cHSBMode::GetPosInMap()
{
    ...
    return cVector2f(vHSB.v[mlRowIndex] / mvMaxValues.v[mlRowIndex], 1.0f - vHSB.v[mlColIndex] / mvMaxValues.v[mlColIndex]);
}
...
cRGBMode::cRGBMode(cGuiPopUpColorPicker* apPicker, int alSliderParamIndex)
    : iGraphicPickerMode(apPicker, _W("RGB"), alSliderParamIndex, cVector3f(1.0f))
{
}
...
void cRGBMode::OnSetBox(const cVector2f& avPos)
{
    ...
    SetRGB(UCharColorToFloat(byteArray[0]), UCharColorToFloat(byteArray[1]), UCharColorToFloat(byteArray[2]));
...
cGuiPopUpColorPicker::cGuiPopUpColorPicker(cGraphics* apGraphics,
      ...
      void* apUpdateColorCallbackObject,
      tGuiCallbackFunc apUpdateColorCallback)
    : iGuiPopUp(apSet, true, cVector2f(600, 350))
{
...
void cGuiPopUpColorPicker::CreateInputs(iWidget* apParent,
      cVector3f& avPos,
      tGuiCallbackFunc apCallback,
      const tFloatVec& avMin,
      const tFloatVec& avMax,
      std::vector<cWidgetTextBox*>& avContainer,
      tBoolVec& avUpdatedContainer,
      tColorPickerModeVec avModes)
{
...
void cGuiPopUpColorPicker::Init()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        mvButtons[i] = mpSet->CreateWidgetButton(0, 30, _W(""), mpWindow);
...
void cGuiPopUpColorPicker::UpdateColor(bool abForce)
{
    ...
    if (abForce == false && previewColor.r == mColor.r && previewColor.g == mColor.g && previewColor.b == mColor.b && mColor.a == mfOldAlpha)
        return;
