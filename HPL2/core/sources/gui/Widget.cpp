...
{
    if (mbCallbacksDisabled || !abSkipVisCheck && IsVisible() == false || !abSkipEnabledCheck && IsEnabled() == false)
        return false;
...
bool iWidget::PointIsInside(const cVector2f& avPoint, bool abOnlyClipped)
{
    ...
    if (avPoint.x < vGlobalPos.x || avPoint.x > vGlobalPos.x + mvSize.x || avPoint.y < vGlobalPos.y || avPoint.y > vGlobalPos.y + mvSize.y)
    {
        return false;
    }
    else
...
void iWidget::DrawBordersAndCorners(cGuiGfxElement* apBackground,
      cGuiGfxElement** apBorderVec,
      cGuiGfxElement** apCornerVec,
      const cVector3f& avPosition,
      const cVector2f& avSize,
      bool abBordersOutside)
{
    ...
    if (apBackground)
    {
        ...
        mpSet->DrawGfx(apBackground, vPos, vSize, cColor(1, 1));
    ...
    if (abBordersOutside)
    {
        ...
        // Borders
        // Right
        mpSet->DrawGfx(apBorderVec[0], cVector3f(avSize.x, 0, 0), cVector2f(apBorderVec[0]->GetImageSize().x, avSize.y));
        // Left
        mpSet->DrawGfx(apBorderVec[1], cVector3f(-apCornerVec[0]->GetActiveSize().x, 0, 0), cVector2f(apBorderVec[1]->GetImageSize().x, avSize.y));

        // Up
        mpSet->DrawGfx(apBorderVec[2], cVector3f(0, -apCornerVec[0]->GetActiveSize().y, 0), cVector2f(avSize.x, apBorderVec[2]->GetImageSize().y));

        // Down
        mpSet->DrawGfx(apBorderVec[3], cVector3f(0, avSize.y, 0), cVector2f(avSize.x, apBorderVec[3]->GetImageSize().y));
        ...
        // Right Down
        mpSet->DrawGfx(apCornerVec[2], cVector3f(avSize.x, avSize.y, 0));
        // Left Down
    ...
    else
    {
        ...
        // Borders
        // Right
        mpSet->DrawGfx(apBorderVec[0], cVector3f(avSize.x - apBorderVec[0]->GetActiveSize().x, apCornerVec[1]->GetActiveSize().y, 0),
              cVector2f(apBorderVec[0]->GetImageSize().x, avSize.y - (apCornerVec[2]->GetActiveSize().y + apCornerVec[1]->GetActiveSize().y)));
        // Left
        mpSet->DrawGfx(apBorderVec[1], cVector3f(0, apCornerVec[0]->GetActiveSize().y, 0),
              cVector2f(apBorderVec[1]->GetImageSize().x, avSize.y - (apCornerVec[3]->GetActiveSize().y + apCornerVec[0]->GetActiveSize().y)));

        // Up
        mpSet->DrawGfx(apBorderVec[2], cVector3f(apCornerVec[0]->GetActiveSize().x, 0, 0),
              cVector2f(avSize.x - (apCornerVec[0]->GetActiveSize().x + apCornerVec[1]->GetActiveSize().x), apBorderVec[2]->GetImageSize().y));

        // Down
        mpSet->DrawGfx(apBorderVec[3], cVector3f(apCornerVec[3]->GetActiveSize().x, avSize.y - apBorderVec[3]->GetActiveSize().y, 0),
              cVector2f(avSize.x - (apCornerVec[2]->GetActiveSize().x + apCornerVec[3]->GetActiveSize().x), apBorderVec[3]->GetImageSize().y));
        ...
        // Right Down
        mpSet->DrawGfx(apCornerVec[2], cVector3f(avSize.x - apCornerVec[2]->GetActiveSize().x, avSize.y - apCornerVec[2]->GetActiveSize().y, 0));
        // Left Down
...
void iWidget::DrawSkinText(const tWString& asText, eGuiSkinFont aFont, const cVector3f& avPosition, eFontAlign aAlign)
{
    cGuiSkinFont* pFont = mpSkin->GetFont(aFont);
    mpSet->DrawFont(asText, pFont->mpFont, avPosition, pFont->mvSize, pFont->mColor * mColorMul, aAlign);
}

//-----------------------------------------------------------------------

void iWidget::DrawDefaultText(const tWString& asText, const cVector3f& avPosition, eFontAlign aAlign)
{
    if (mpDefaultFontType == NULL)
        return;

    mpSet->DrawFont(asText, mpDefaultFontType, avPosition, mvDefaultFontSize, mDefaultFontColor * mColorMul, aAlign);
}

//-----------------------------------------------------------------------

void iWidget::DrawDefaultText(const tWString& asText, const cVector3f& avPosition, eFontAlign aAlign, const cColor& aCol)
{
    if (mpDefaultFontType == NULL)
        return;

    mpSet->DrawFont(asText, mpDefaultFontType, avPosition, mvDefaultFontSize, aCol * mColorMul, aAlign);
}

//-----------------------------------------------------------------------

void iWidget::DrawDefaultTextHighlight(const tWString& asText, const cVector3f& avPosition, eFontAlign aAlign) //, int alFirstVisibleChar, int alHighlightStart, int alHighlightSize
                                                                                                               //)
{
    if (mpDefaultFontType == NULL)
        return;
    /*
                    if(alHighlightSize!=asText.size())
                            mpSet->DrawFont(asText,mpDefaultFontType,avPosition,mvDefaultFontSize,
                                                    ...
                                                    */
    mpSet->DrawFont(asText, mpDefaultFontType, avPosition, mvDefaultFontSize, mDefaultFontHighlightColor * mColorMul, aAlign);
}
