...
iWidgetListBoxBase::iWidgetListBoxBase(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_ListBox, apSet, apSkin)
{
...
void iWidgetListBoxBase::SetSelectedItem(int alX, bool abMoveList, bool abGenerateCallback, eListBoxSelectType aType, bool abClearPrevious)
{
...
void iWidgetListBoxBase::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ////////////////////////////////
    // Background
    mpSet->DrawGfx(mpGfxBackground, GetGlobalPosition() + cVector3f(0, 0, mfBackgroundZ), mvSize);

    ////////////////////////////////
    // Borders
    DrawBordersAndCorners(NULL, mvGfxBorders, mvGfxCorners, GetGlobalPosition() - cVector3f(mvGfxCorners[0]->GetActiveSize().x, mvGfxCorners[0]->GetActiveSize().y, 0),
          mvSize + mvGfxCorners[0]->GetActiveSize() + mvGfxCorners[2]->GetActiveSize());
}
...
bool iWidgetListBoxBase::OnMouseDown(const cGuiMessageData& aData)
{
    ...
    eListBoxSelectType type = bUsingShift ? eListBoxSelectType_FromAnchorPos : (bUsingCtrl ? eListBoxSelectType_Toggle : eListBoxSelectType_Select);
