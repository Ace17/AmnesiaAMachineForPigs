...
cWidgetButton::cWidgetButton(cGuiSet* apSet, cGuiSkin* apSkin)
    : iWidget(eWidgetType_Button, apSet, apSkin)
{
    mbToggleable = false;
    mbPressed = false;
    mpImage = NULL;
...
void cWidgetButton::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    ...
    if (mpImage)
    {
        ...
        mpSet->DrawGfx(
              mpImage, GetGlobalPosition() + cVector3f(mvSize.x / 2 - mpImage->GetImageSize().x / 2, mvSize.y / 2 - mpImage->GetImageSize().y / 2, 0.2f) + vOffset, -1, color);
    }
    ...
    if (IsEnabled())
    {
        ...
        DrawDefaultText(msText, GetGlobalPosition() + cVector3f(mvSize.x / 2, mvSize.y / 2 - mvDefaultFontSize.y / 2, 0.5f) + vOffset, eFontAlign_Center, mDefaultFontColor);
    }
    else
    {
        DrawDefaultText(msText, GetGlobalPosition() + cVector3f(mvSize.x / 2, mvSize.y / 2 - mvDefaultFontSize.y / 2, 0.5f) + vOffset, eFontAlign_Center, cColor(0.5f, 1));
    ...
    if (mbPressed)
    {
        DrawBordersAndCorners(mpGfxBackgroundDown, mvGfxBordersDown, mvGfxCornersDown, GetGlobalPosition(), mvSize);
    }
    else
    {
        DrawBordersAndCorners(mpGfxBackgroundUp, mvGfxBordersUp, mvGfxCornersUp, GetGlobalPosition(), mvSize);
    }
}
