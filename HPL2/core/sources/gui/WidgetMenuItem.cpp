...
cWidgetMenuItem::cWidgetMenuItem(iWidgetMenu* apParent)
    : iWidget(eWidgetType_MenuItem, apParent->GetSet(), apParent->GetSet()->GetSkin())
{
    mbChecked = false;
...
cGuiGlobalShortcut* cWidgetMenuItem::AddShortcut(int alKeyModifiers, eKey aKey, eGuiMessage aMsg, bool abBypassVisibility, bool abBypassEnabled)
{
...
float cWidgetMenuItem::GetTextLength()
{
    ...
    return mpDefaultFontType->GetLength(vFontSize, msText.c_str()) + mpDefaultFontType->GetLength(vFontSize, msShortcutText.c_str());
}
...
void cWidgetMenuItem::OnDraw(float afTimeStep, cGuiClipRegion* apClipRegion)
{
    if (IsSeparator())
    {
        mpSet->DrawGfx(mpGfxLine, GetGlobalPosition() + cVector3f(mfSeparatorPadding, mvSize.y * 0.5f - 1, 0.2f), cVector2f(mvSize.x - 2 * mfSeparatorPadding, mfSeparatorHeight));
        return;
    }
    ...
    cColor col = bEnabled ? (bHighlight ? mDefaultFontHighlightColor : mDefaultFontColor) : mpSkin->GetFont(eGuiSkinFont_Disabled)->mColor;
...
{
    if (IsVisible() == false || mpSet->GetDrawMouse() == false || (mpSet->GetAttentionWidget() != NULL && IsConnectedTo(mpSet->GetAttentionWidget()) == false))
        return true;
...
{
    if (mpSet->GetAttentionWidget() != NULL && IsConnectedTo(mpSet->GetAttentionWidget()) == false)
        return false;
