      ...
      const cVector3f& avScale)
{
    ...
    else if (jointType == ePhysicsJointType_Ball)
    {
        ...
        pJoint->SetConeLimits(cMath::ToRad(apJointElem->GetAttributeFloat("MaxConeAngle")), cMath::ToRad(apJointElem->GetAttributeFloat("MaxTwistAngle")));
...
iEntity3D* cEntityLoader_Object::Load(const tString& asName,
      int alID,
      bool abActive,
      cXmlElement* apRootElem,
      const cMatrixf& a_mtxTransform,
      const cVector3f& avScale,
      cWorld* apWorld,
      ...
      cResourceVarsObject* apInstanceVars)
{
    ...
    // Load sub meshes
    {
        ...
        while (submeshIt.HasNext())
        {
            ...
            if (bHasSkeleton == false)
            {
                cMatrixf mtxLocalTransform = GetMatrixFromVectors(pSubMeshElem->GetAttributeVector3f("WorldPos") * mvScale, pSubMeshElem->GetAttributeVector3f("Rotation"),
        ...
        if (pEntitiesElem)
        {
            ...
            while (entityIt.HasNext())
            {
                ...
                else if (sEntityType == "Billboard")
                {
                    if (mbLoadBillboards)
                    {
                        cBillboard* pBillboard = cEngineFileLoading::LoadBillboard(
                              pEntityElem, asName + "_", apWorld, apWorld->GetResources(), mbLoadAsStatic, &lstLightBillboardListConnections);
                        if (pBillboard)
    ...
    if (mvBodies.empty() == false)
    {
        ...
        for (tEntity3DListIt it = lstTempEntities.begin(); it != lstTempEntities.end(); ++it)
        {
            ...
            if (pEntity->GetEntityType() == "SubMesh" && !mpMesh->GetSkeleton() && mpMesh->GetAnimationNum() > 0)
            {
...
void cEntityLoader_Object::LoadAndAttachChildren(cXmlElement* apMainElem,
      iEntity3D* apEntityParent,
      cBoneState* apBoneStateParent,
      tEntity3DList& a_lstChildList,
      tNodeStateMap& a_mapBoneStates,
      bool abRemoveAttachedChild,
      bool abIsBody)
{
...
{
    apBody->SetMatrix(GetMatrixFromVectors(apElem->GetAttributeVector3f("WorldPos") * mvScale, apElem->GetAttributeVector3f("Rotation"), 1.0f));
...
void cEntityLoader_Object::SetJointProperties(iPhysicsJoint* pJoint, cXmlElement* apJointElem, cWorld* apWorld)
{
    ...
    pJoint->SetMoveSpeedType(cString::ToLowerCase(apJointElem->GetAttributeString("MoveType", "Linear")) == "angular" ? ePhysicsJointSpeed_Angular : ePhysicsJointSpeed_Linear);
...
void cEntityLoader_Object::LoadController(iPhysicsJoint* apJoint, iPhysicsWorld* apPhysicsWorld, TiXmlElement* apElem)
{
    //////////////////////////////
    // Get the properties
