...
iXmlNode* iXmlNode::GetFirstOfType(eXmlNodeType aType, const tString& asName)
{
    ...
    tXmlNodeListIt it = mlstChildren.begin();
    iXmlNode* pNode = *it;
    while (pNode->GetType() != eXmlNodeType_Element || pNode->GetValue() != asName)
    {
        ++it;
        if (it == mlstChildren.end())
            return NULL;
...
cXmlElement::cXmlElement(const tString& asName, iXmlNode* apParent)
    : iXmlNode(eXmlNodeType_Element, apParent, asName)
{
}
...
bool iXmlDocument::CreateFromFile(const tWString& asPath)
{
    ...
    if (bRet == false)
    {
        Log("Failed parsing of XML document %s in line %d, column %d: %s\n", cString::To8Char(asPath).c_str(), mlErrorRow, mlErrorCol, msErrorDesc.c_str());
    }
