...
bool cBinaryBuffer::Reserve(size_t alSize)
{
    ...
    for (size_t i = 0; i < lChunks; ++i)
    {
        ...
        if (mvDataChunks[i])
            pNewData = (char*)hplRealloc(mvDataChunks[i], lChunkSize);
        else
            pNewData = (char*)hplMalloc(lChunkSize);
...
{
    if (mlDataSize % 8 != 0)
        return; // this data isnt 64 bit aligned, not an encrypted source
