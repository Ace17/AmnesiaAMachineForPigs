...
cWorld* cWorldLoaderHplMap::LoadWorld(const tWString& asFile, tWorldLoadFlag aFlags)
{
    ...
    if (cResources::GetCreateAndLoadCompressedMaps() && bLoadedFromNormalFile)
    {
        ...
        if (cPlatform::FileExists(sCompFile) == false || cPlatform::FileModifiedDate(sCompFile) < cPlatform::FileModifiedDate(asFile))
        {
            tString sData;
...
void cWorldLoaderHplMap::LoadCacheFile(const tWString& asFile)
{
    ...
    for (int mesh = 0; mesh < lStaticMeshEntities; ++mesh)
    {
        ...
        ////////////////////
        // Vertex data
        iVertexBuffer* pVtxBuff = mpGraphics->GetLowLevel()->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static, 0, 0);
        {
...
static bool SortStaticSubMeshesForMeshes(iRenderable* apObjectDataA, iRenderable* apObjectDataB)
{
    ...
    if (apObjectDataA->GetRenderFlagBit(eRenderableFlag_ShadowCaster) != apObjectDataB->GetRenderFlagBit(eRenderableFlag_ShadowCaster))
    {
        return apObjectDataA->GetRenderFlagBit(eRenderableFlag_ShadowCaster) < apObjectDataB->GetRenderFlagBit(eRenderableFlag_ShadowCaster);
    }
...
{
    // Is shadow caster check
    if (aObjectDataA.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster) != aObjectDataB.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster))
    {
        return aObjectDataA.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster) < aObjectDataB.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster);
    }
    // Material check
...
void cWorldLoaderHplMap::CombineAndCreateMeshesAndPhysics(tRenderableList* apObjectList)
{
    ...
    for (size_t i = 0; i < vMeshObjects.size(); ++i)
    {
        ...
        if (gbLog)
            Log("  %d Checking '%s', material: %d, cast shadows: %d\n", i, pMeshObject->GetName().c_str(), pMeshObject->GetMaterial(),
        ...
        if (i < vMeshObjects.size() - 1)
        {
            ...
            if (pNextObject->GetMaterial() != pMeshObject->GetMaterial() ||
                  ...
                  pNextObject->IsOccluder() != pMeshObject->IsOccluder() || pNextObjectUserData->mbCombine == false)
            {
    ...
    for (size_t i = 0; i < vPhysicsObjects.size(); ++i)
    {
        ...
        if (gbLog)
            Log("  %d Checking '%s', physics material: %d\n", i, physicsObject.mpObject->GetName().c_str(), physicsObject.mpPhysicsMaterial);
        ...
        if (i < vMeshObjects.size() - 1)
        {
            ...
            if (lIndexCount > lIndexCountLimit || nextObject.mbCharCollider != physicsObject.mbCharCollider || nextObject.mpPhysicsMaterial != physicsObject.mpPhysicsMaterial ||
...
void cWorldLoaderHplMap::CombineObjectsAndCreateMeshEntity(tRenderableVec& avObjects, int alFirstIdx, int alLastIdx)
{
    ...
    iVertexBuffer* pVtxBuffer = mpGraphics->GetLowLevel()->CreateVertexBuffer(
          eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static, lTotalVtxAmount, lTotalIdxAmount);

    // Set up what data arrays to use
    const int lDataArrayNum = 5;
    cVertexDataArray lDataArrayTypes[lDataArrayNum] = { { eVertexBufferElement_Position, 4 }, { eVertexBufferElement_Normal, 3 }, { eVertexBufferElement_Color0, 4 },
        { eVertexBufferElement_Texture0, 3 }, { eVertexBufferElement_Texture1Tangent, 4 } };
    ...
    for (int vtxbuffer = alFirstIdx; vtxbuffer <= alLastIdx; ++vtxbuffer)
    {
        ...
        iVertexBuffer* pTransformedVtxBuffer = pSubVtxBuffer->CreateCopy(eVertexBufferType_Software, eVertexBufferUsageType_Static, pSubVtxBuffer->GetVertexElementFlags());
...
void cWorldLoaderHplMap::CombineObjectsAndCreatePhysics(std::vector<cHplMapPhysicsObject>& avObjects, int alFirstIdx, int alLastIdx)
{
    ...
    // Create the vertex buffer (skipping color!)
    iVertexBuffer* pVtxBuffer = mpGraphics->GetLowLevel()->CreateVertexBuffer(
          eVertexBufferType_Software, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Dynamic, lTotalVtxAmount, lTotalIdxAmount);
    ...
    for (int vtxbuffer = alFirstIdx; vtxbuffer <= alLastIdx; ++vtxbuffer)
    {
        ...
        iVertexBuffer* pSubVtxBuffer = pObject->GetVertexBuffer();
        iVertexBuffer* pTransformedVtxBuffer = pSubVtxBuffer->CreateCopy(eVertexBufferType_Software, eVertexBufferUsageType_Static, eVertexElementFlag_Position);
        pTransformedVtxBuffer->Transform(pObject->GetWorldMatrix());
...
void cWorldLoaderHplMap::CreateStaticObjectEntity(cXmlElement* apElement, tMeshEntityList& alstMeshEntities, cRenderableContainer_BoxTree* apContainer)
{
    ////////////////////////////////
    // Load properties
    ...
    cMeshEntity* pMeshEntity = hplNew(cMeshEntity, (sName, pMesh, mpResources->GetMaterialManager(), mpResources->GetMeshManager(), mpResources->GetAnimationManager()));
...
void cWorldLoaderHplMap::CreatePrimitive(cXmlElement* apElement, tMeshEntityList& alstMeshEntities, cRenderableContainer_BoxTree* apContainer)
{
    ////////////////////////////////
    ...
    if (sType == "Plane")
    {
        ...
        // Create the mesh
        cMesh* pMesh = mpGraphics->GetMeshCreator()->CreatePlane(sName, vStartCorner, vEndCorner, vUVCorners[0], vUVCorners[1], vUVCorners[2], vUVCorners[3], sMaterial);
        ...
        // Create mesh entity
        pMeshEntity = hplNew(cMeshEntity, (sName, pMesh, mpResources->GetMaterialManager(), mpResources->GetMeshManager(), mpResources->GetAnimationManager()));
        pMeshEntity->SetRenderFlagBit(eRenderableFlag_ShadowCaster, bCastsShadows);
...
void cWorldLoaderHplMap::CreateDecal(cXmlElement* apElement, tMeshEntityList& alstMeshEntities, cRenderableContainer_BoxTree* apDecalContainer)
{
    ////////////////////////////////
    // Load Main Properties
    cMeshEntity* pMeshEntity = NULL;
...
void cWorldLoaderHplMap::CreateStaticObjectCombo(cXmlElement* apElement, tMeshEntityList& alstMeshEntities, cRenderableContainer_BoxTree* apContainer)
{
...
void cWorldLoaderHplMap::LoadEntity(const tString& asName,
      int alID,
      bool abActive,
      const cVector3f& avPos,
      const cVector3f& avRot,
      const cVector3f& avScale,
      cXmlElement* apElement)
{
