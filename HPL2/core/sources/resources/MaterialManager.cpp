...
public:
    ...
    iGpuProgram* GetGpuProgram(cMaterial* apMaterial, eMaterialRenderMode aRenderMode) { return NULL; }
...
cMaterial* cMaterialManager::LoadFromFile(const tString& asName, const tWString& asPath)
{
    ...
    for (int i = 0; i < pMatType->GetUsedTextureNum(); ++i)
    {
        ...
        else
        {
            ...
            {
                pTex = mpResources->GetTextureManager()->Create1D(sFile, bMipMaps, eTextureUsage_Normal, mlTextureSizeDownScaleLevel);
            }
            else if (type == eTextureType_2D)
            {
                pTex = mpResources->GetTextureManager()->Create2D(sFile, bMipMaps, eTextureType_2D, eTextureUsage_Normal, mlTextureSizeDownScaleLevel);
            }
            else if (type == eTextureType_3D)
            {
                pTex = mpResources->GetTextureManager()->Create3D(sFile, bMipMaps, eTextureUsage_Normal, mlTextureSizeDownScaleLevel);
            }
            else if (type == eTextureType_CubeMap)
            {
                // Check for DDS ending and load cubemap as file.
                pTex = mpResources->GetTextureManager()->CreateCubeMap(sFile, bMipMaps, eTextureUsage_Normal, mlTextureSizeDownScaleLevel);
            }
        }
