...
void cFrustum::SetupPerspectiveProj(const cMatrixf& a_mtxProj,
      ...
      bool abInfFarPlane,
      cMatrixf* apCustomFarProjMtx,
      bool abObliqueNearPlane)
{
...
void cFrustum::SetupOrthoProj(const cMatrixf& a_mtxProj,
      const cMatrixf& a_mtxView,
      float afFarPlane,
      float afNearPlane,
...
void cFrustum::Setup(const cMatrixf& a_mtxProj,
      const cMatrixf& a_mtxView,
      float afFarPlane,
      float afNearPlane,
      const cVector3f& avOrigin,
...
bool cFrustum::CheckLineIntersection(const cVector3f& avStart, const cVector3f& avEnd, cVector3f& avIntersection)
{
    ...
    for (int i = 0; i < lPlanes; i++)
    {
        ...
        {
            if ((fT < fClosestT || fClosestT < 0) && CollidePoint(vIntersection + plane.GetNormal() * 0.001f))
            {
...
void cFrustum::UpdateSphere()
{
    ...
    else
    {
        ...
        cVector3f vHalfSize(mvViewSize.x * 0.5f, mvViewSize.y * 0.5f, fViewLen * 0.5f);
...
{
    // Left
    mPlane[eFrustumPlane_Left] = cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[0][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[0][1],
          m_mtxViewProj.m[3][2] + m_mtxViewProj.m[0][2], m_mtxViewProj.m[3][3] + m_mtxViewProj.m[0][3]);

    // Right
    mPlane[eFrustumPlane_Right] = cPlanef(m_mtxViewProj.m[3][0] - m_mtxViewProj.m[0][0], m_mtxViewProj.m[3][1] - m_mtxViewProj.m[0][1],
          m_mtxViewProj.m[3][2] - m_mtxViewProj.m[0][2], m_mtxViewProj.m[3][3] - m_mtxViewProj.m[0][3]);

    // Bottom
    mPlane[eFrustumPlane_Bottom] = cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[1][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[1][1],
          m_mtxViewProj.m[3][2] + m_mtxViewProj.m[1][2], m_mtxViewProj.m[3][3] + m_mtxViewProj.m[1][3]);

    // Top
    mPlane[eFrustumPlane_Top] = cPlanef(m_mtxViewProj.m[3][0] - m_mtxViewProj.m[1][0], m_mtxViewProj.m[3][1] - m_mtxViewProj.m[1][1], m_mtxViewProj.m[3][2] - m_mtxViewProj.m[1][2],
          m_mtxViewProj.m[3][3] - m_mtxViewProj.m[1][3]);

    // Near
    mPlane[eFrustumPlane_Near] = cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[2][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[2][1],
          m_mtxViewProj.m[3][2] + m_mtxViewProj.m[2][2], m_mtxViewProj.m[3][3] + m_mtxViewProj.m[2][3]);
    ...
    if (apCustomFarProjMtx)
    {
        ...
        mPlane[eFrustumPlane_Far] = cPlanef(mtxCustomViewProj.m[3][0] - mtxCustomViewProj.m[2][0], mtxCustomViewProj.m[3][1] - mtxCustomViewProj.m[2][1],
              mtxCustomViewProj.m[3][2] - mtxCustomViewProj.m[2][2], mtxCustomViewProj.m[3][3] - mtxCustomViewProj.m[2][3]);
    }
    else
    {
        mPlane[eFrustumPlane_Far] = cPlanef(m_mtxViewProj.m[3][0] - m_mtxViewProj.m[2][0], m_mtxViewProj.m[3][1] - m_mtxViewProj.m[2][1],
              m_mtxViewProj.m[3][2] - m_mtxViewProj.m[2][2], m_mtxViewProj.m[3][3] - m_mtxViewProj.m[2][3]);
    }
...
void cFrustum::UpdateVertices()
{
    ...
    if (mbObliqueNearPlane)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (cMath::PlaneToPointDist(fakeNearPlane, mvVertices[i]) < 0 || cMath::PlaneToPointDist(farPlane, mvVertices[i]) < 0)
            {
