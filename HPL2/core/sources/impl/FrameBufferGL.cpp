...
cFrameBufferGL::cFrameBufferGL(const tString& asName, iLowLevelGraphics* apLowLevelGraphics)
...
void cFrameBufferGL::SetDepthStencilBuffer(iDepthStencilBuffer* apBuffer)
{
    ...
    {
        glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, pBufferSDL->GetHandle());
    ...
    {
        glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, pBufferSDL->GetHandle());
...
void cFrameBufferGL::AttachTexture(int alAttachmentType, int alAttachmentIdx, iTexture* apTexture, int alMipmapLevel, int alExtra)
{
    ...
    if (texType == eTextureType_2D || texType == eTextureType_Rect || texType == eTextureType_CubeMap)
    {
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, alAttachmentType + alAttachmentIdx, texType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alExtra : GLTarget,
    ...
    {
        glFramebufferTexture3DEXT(GL_FRAMEBUFFER_EXT, alAttachmentType + alAttachmentIdx, GLTarget, pTextureSDL->GetTextureHandle(), alMipmapLevel, alExtra);
    }
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
