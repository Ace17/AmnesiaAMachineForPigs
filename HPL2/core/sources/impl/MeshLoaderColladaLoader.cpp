      ...
      bool abCache)
{
    ...
    if (abCache && cPlatform::FileExists(sCacheFile) && cPlatform::FileExists(asFile))
    {
    ...
    if (bLoadCache)
    {
        return LoadStructures(sCacheFile, apColladaImageVec, apColladaTextureVec, apColladaMaterialVec, apColladaLightVec, apColladaGeometryVec, apColladaControllerVec,
              apColladaAnimVec, apColladaScene);
    }
        ...
        // if(false)
        {
            ...
            for (size_t idx = 0; idx < pGeometry->mvExtraVtxVec.size(); ++idx)
            {
                ...
                for (; it != pList->end(); ++it)
                {
                    ...
                    sData += cString::ToString(extra.mlVtx) + " " + cString::ToString(extra.mlNorm) + " " + cString::ToString(extra.mlTex) + " " +
...
static void LoadControllerVec(TiXmlElement* apRootElem, tColladaControllerVec* apColladaControllerVec)
{
    ...
    for (; pControllerElem != NULL; pControllerElem = pControllerElem->NextSiblingElement(), ++lCount)
    {
        ...
        // Matrices
        {
            ...
            {
                pController->mvMatrices.push_back(cMatrixf(pData[0], pData[1], pData[2], pData[3], pData[4], pData[5], pData[6], pData[7], pData[8], pData[9], pData[10], pData[11],
        ...
        // Pairs
        {
            ...
            for (int i = 0; i < lSize; ++i)
            {
                ...
                {
                    pController->mvPairs[i].push_back(cColladaJointPair(vPairs[lPairCount], vPairs[lPairCount + 1]));
                    lPairCount += 2;
                }
            }
        }
