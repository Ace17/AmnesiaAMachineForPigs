...
cAnimationTrack* cMeshLoaderCollada::CreateAnimTrack(cAnimation* apAnimation, cSkeleton* apSkeleton, cColladaAnimation& aAnim, cColladaScene* apScene)
{
    ...
    for (size_t i = 0; i < aAnim.mvSamplers.size(); i++)
    {
        ...
        if (pTrans->msType == "translate")
        {
            ...
            for (size_t j = 0; j < pTimeVec->size(); j++)
            {
                ...
                if (sExt == "")
                {
                    pTempData->mvTrans = cVector3f((*pValueVec)[j * 3 + 0], (*pValueVec)[j * 3 + 1], (*pValueVec)[j * 3 + 2]);
                }
                // X
                else if (sExt == "x")
                {
        ...
        else if (pTrans->msType == "rotate")
        {
            ...
            for (size_t j = 0; j < pTimeVec->size(); j++)
            {
                ...
                pTempData->mvRot += (cVector3f(pTrans->mvValues[0], pTrans->mvValues[1], pTrans->mvValues[2]) * (*pValueVec)[j]);
            }
            ...
            for (size_t j = 0; j < vTempData.size(); ++j)
            {
                ...
                else if (fTimeBefore >= 0 && fTimeAfter >= 0)
                {
                    ...
                    vTempData[j].mvRot.v[lVecNum] = pBefore->mvRot.v[lVecNum] * (1.0f - fT) + pAfter->mvRot.v[lVecNum] * fT;
    ...
    cAnimationTrack* pTrack = apAnimation->CreateTrack(aAnim.msTargetNode, eAnimTransformFlag_Rotate | eAnimTransformFlag_Translate);
    ////////////////////////////////////
...
iVertexBuffer* cMeshLoaderCollada::CreateVertexBuffer(cColladaGeometry& aGeometry, eVertexBufferUsageType aUsageType)
//,tColladaExtraVtxListVec &vExtraVtxVec)
{
    ...
    iVertexBuffer* pVtxBuff = mpLowLevelGraphics->CreateVertexBuffer(
          eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, aUsageType, (int)aGeometry.mvVertexVec.size(), (int)aGeometry.mvIndexVec.size());
...
static cColladaAnimation& GetAnimationFromTarget(const tString& asTargetNode, tColladaAnimationVec& avAnimations)
{
...
void cMeshLoaderCollada::LoadAnimations(TiXmlElement* apRootElem, tColladaAnimationVec& avAnimations, cColladaScene* apColladaScene)
{
...
void cMeshLoaderCollada::LoadColladaScene(TiXmlElement* apRootElem, cColladaNode* apParentNode, cColladaScene* apScene, tColladaLightVec* apColladaLightVec)
{
    ...
    while (pTransformElem)
    {
        ...
        else if (sVal == "scale")
        {
            ...
            else if (cString::ToLowerCase(cString::Sub(pNode->msName, 1, 8)) == "collider" || cString::ToLowerCase(cString::Sub(pNode->msName, 1, 12)) == "charcollider" ||
...
void cMeshLoaderCollada::LoadControllers(TiXmlElement* apRootElem, tColladaControllerVec& avColladaControllerVec, tColladaGeometryVec* apColladaGeometryVec)
{
...
void cMeshLoaderCollada::LoadGeometry(TiXmlElement* apRootElem, tColladaGeometryVec& avColladaGeometryVec)
{
    ...
    for (; pGeomElem != NULL; pGeomElem = pGeomElem->NextSiblingElement("geometry"))
    {
        ...
        if (cString::Sub(Geometry.msName, 0, 4) == "_ref" || cString::Sub(Geometry.msName, 0, 3) == "_bb" || cString::Sub(Geometry.msName, 0, 4) == "_ps" ||
        ...
        for (size_t i = 0; i < Geometry.mvArrayVec.size(); i++)
        {
            if (Geometry.mvArrayVec[i].mbIsInVertex)
            {
                ...
                {
                    Geometry.mlTexArrayIdx = i; // PUDDLES: starting point, here the dae index list referring to a texture mapping channel starts
...
void cMeshLoaderCollada::SplitVertices(cColladaGeometry& aGeometry, tColladaExtraVtxListVec& avExtraVtxVec, tVertexVec& avVertexVec, tUIntVec& avIndexVec)
{
...
{
    return apA->mVertex.pos == apB->mVertex.pos && apA->mVertex.norm == apB->mVertex.norm && apA->mVertex.tex == apB->mVertex.tex;
}
...
int cMeshLoaderCollada::OptimizeGeometry(tColladaGeometryVec& avColladaGeometryVec)
{
    ...
    for (size_t i = 0; i < avColladaGeometryVec.size(); ++i)
    {
        ...
        for (size_t j = 0; j < vSortVertices.size(); j += lPairs)
        {
            ...
            vSortVertices[j]->mlMergedIndex = -(1 + pGeometry.mvVertexVec.size());
...
void cMeshLoaderCollada::GenerateTangents(cColladaGeometry& aGeometry)
{
    ...
    // Create Tangents
    tFloatVec vPosVec;
    vPosVec.resize(aGeometry.mvVertexVec.size() * 4);
    tFloatVec vNormVec;
    vNormVec.resize(aGeometry.mvVertexVec.size() * 3);
    tFloatVec vTexVec;
    vTexVec.resize(aGeometry.mvVertexVec.size() * 3);
    ...
    cMath::CreateTriTangentVectors(
          &aGeometry.mvTangents[0], &aGeometry.mvIndexVec[0], (int)aGeometry.mvIndexVec.size(), &vPosVec[0], 4, &vTexVec[0], &vNormVec[0], (int)aGeometry.mvVertexVec.size());
}
