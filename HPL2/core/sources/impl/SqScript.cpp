...
cSqScript::cSqScript(const tString& asName, asIScriptEngine* apScriptEngine, cScriptOutput* apScriptOutput, int alHandle)
    : iScript(asName, _W(""))
{
    ...
    msModuleName = "Module_" + cString::ToString(cMath::RandRectl(0, 1000000)) + "_" + cString::ToString(mlHandle);
}
...
bool cSqScript::CreateFromFile(const tWString& asFileName, tString* apCompileMessages)
{
    ...
    if (sExt == _W("hps") && cResources::GetCreateAndLoadCompressedMaps())
    {
        ...
        if (cPlatform::FileExists(sCompFile) == false || cPlatform::FileModifiedDate(sCompFile) < cPlatform::FileModifiedDate(asFileName))
        {
            cBinaryBuffer textBuff;
