...
#define CG_CHECK(type, p)                                                                                                                                                          \
    else if (msForce##type == #p) { mProfile = cgGLIsProfileSupported(CG_PROFILE_##p) ? CG_PROFILE_##p : CG_PROFILE_UNKNOWN; }
...
bool cCGShader::CreateFromFile(const tString& asFile, const tString& asEntry)
{
    ...
    if (mShaderType == eGpuShaderType_Vertex)
    {
        mProgram = cgCreateProgramFromFile(mContext, CG_SOURCE, asFile.c_str(), mProfile, asEntry.c_str(), NULL);
    }
    else
    {
        if (false) // GLEE_ATI_draw_buffers)
        {
            ...
            mProgram = cgCreateProgramFromFile(mContext, CG_SOURCE, asFile.c_str(), mProfile, asEntry.c_str(), &vArgs[0]);
        }
        else
        {
            mProgram = cgCreateProgramFromFile(mContext, CG_SOURCE, asFile.c_str(), mProfile, asEntry.c_str(), 0);
        }
    }
...
bool cCGShader::SetMatrixf(int alVarId, eGpuShaderMatrix mType, eGpuShaderMatrixOp mOp)
{
    if (alVarId < 0 || alVarId >= (int)mvParameters.size())
        return false;
    CGparameter& Param = mvParameters[alVarId];
...
{
    if (mvTexUnitParam[alUnit] == NULL || alUnit >= kMaxTextureUnits)
        return false;
