...
tWString cPlatform::GetSystemSpecialPath(eSystemPath aPathType)
{
    ...
    if (cString::GetLastCharW(sOutput) != _W("/") && cString::GetLastCharW(sOutput) != _W("\\"))
    {
        sOutput += _W("/");
    }
...
void cPlatform::GetAvailableVideoModes(tVideoModeVec& avDestVidModes, int alMinBpp, int alMinRefreshRate)
{
    ...
    {
        if ((bSkipBppCheck || alMinBpp <= (int)mode.dmBitsPerPel) && (bSkipRateCheck || alMinRefreshRate <= (int)mode.dmDisplayFrequency))
        {
            avDestVidModes.push_back(cVideoMode(cVector2l(mode.dmPelsWidth, mode.dmPelsHeight), mode.dmBitsPerPel, mode.dmDisplayFrequency));
        }
