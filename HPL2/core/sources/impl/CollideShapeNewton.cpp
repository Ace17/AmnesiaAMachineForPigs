...
cCollideShapeNewton::cCollideShapeNewton(eCollideShapeType aType, const cVector3f& avSize, cMatrixf* apOffsetMtx, NewtonWorld* apNewtonWorld, iPhysicsWorld* apWorld)
    : iCollideShape(apWorld)
{
    ...
    else if (mType == eCollideShapeType_Cylinder || mType == eCollideShapeType_Capsule)
    {
...
cVector3f cCollideShapeNewton::GetInertia(float afMass)
{
    ...
    if (mType == eCollideShapeType_Box)
    {
        ...
        vInertia = cVector3f(afMass * (mvSize.y * mvSize.y + mvSize.z * mvSize.z) * fInv, afMass * (mvSize.x * mvSize.x + mvSize.z * mvSize.z) * fInv,
    ...
    else if (mType == eCollideShapeType_Compound)
    {
        ...
        float fInv = 1.0f / 12.0f;
        vInertia = cVector3f(afMass * (vBoxSize.y * vBoxSize.y + vBoxSize.z * vBoxSize.z) * fInv, afMass * (vBoxSize.x * vBoxSize.x + vBoxSize.z * vBoxSize.z) * fInv,
...
void cCollideShapeNewton::CreateCompoundFromShapeVec(tCollideShapeVec& avShapes)
{
    ...
    mpNewtonCollision = NewtonCreateCompoundCollision(mpNewtonWorld, (int)vNewtonColliders.size(), &vNewtonColliders[0], 0);
...
void cCollideShapeNewton::CreateFromVertices(const unsigned int* apIndexArray, int alIndexNum, const float* apVertexArray, int alVtxStride, int alVtxNum)
{
    float vTriVec[9];
    ...
    for (int tri = 0; tri < alIndexNum; tri += 3)
    {
        ...
        if (false) // bOptimize==false)
        {
            ...
            else
            {
                ...
                if (std::abs(plane.a - tempPlane.a) > 0.001f || std::abs(plane.b - tempPlane.b) > 0.001f || std::abs(plane.c - tempPlane.c) > 0.001f ||
