...
void cPlatform::CreateMessageBoxBase(eMsgBoxType eType, const wchar_t* asCaption, const wchar_t* fmt, va_list ap)
{
    ...
    SDL_MessageBoxButtonData buttons[] = { { SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT, 1, "Dismiss" } };
    SDL_MessageBoxData data = { type, NULL, caption.c_str(), message.c_str(), 1, buttons, NULL };
    int pressed = -1;
