...
cGamepadSDL::cGamepadSDL(cLowLevelInputSDL* apLowLevelInputSDL, int alIndex)
    : iGamepad("SDL Portable Gamepad", alIndex)
{
...
void cGamepadSDL::Update()
{
    ...
    for (; it != mpLowLevelInputSDL->mlstEvents.end(); ++it)
    {
        ...
        if ((pEvent->type != SDL_JOYAXISMOTION && pEvent->type != SDL_JOYBUTTONUP && pEvent->type != SDL_JOYBUTTONDOWN && pEvent->type != SDL_JOYHATMOTION &&
