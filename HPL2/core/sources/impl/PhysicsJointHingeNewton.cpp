...
float cPhysicsJointHingeNewton::GetAngle()
{
    ...
    float fSinAngle = cMath::Vector3Dot(cMath::Vector3Cross(mtxPinPivot0Inv.GetRight(), mtxPinPivot1Inv.GetRight()), mtxPinPivot0Inv.GetUp());
...
void cPhysicsJointHingeNewton::SubmitConstraints(dFloat afTimestep, int alThreadIndex)
{
    ...
    if (mfMinAngle != 0 || mfMaxAngle != 0)
    {
        ...
        else
        {
            ...
            {
                if (bSkipLimitCheck == false && ((mbStickyMaxLimit && mfPreviousAngle > mfMaxAngle) || (mbStickyMinLimit && mfPreviousAngle < mfMinAngle)))
                {
