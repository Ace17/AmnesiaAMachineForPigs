...
{
    iCollideShape* pShape = hplNew(cCollideShapeNewton, (eCollideShapeType_Null, 0, NULL, mpNewtonWorld, this));
...
{
    iCollideShape* pShape = hplNew(cCollideShapeNewton, (eCollideShapeType_Box, avSize, apOffsetMtx, mpNewtonWorld, this));
    mlstShapes.push_back(pShape);
...
{
    iCollideShape* pShape = hplNew(cCollideShapeNewton, (eCollideShapeType_Sphere, avRadii, apOffsetMtx, mpNewtonWorld, this));
    mlstShapes.push_back(pShape);
...
{
    iCollideShape* pShape = hplNew(cCollideShapeNewton, (eCollideShapeType_Cylinder, cVector3f(afRadius, afHeight, afRadius), apOffsetMtx, mpNewtonWorld, this));
    mlstShapes.push_back(pShape);
...
{
    iCollideShape* pShape = hplNew(cCollideShapeNewton, (eCollideShapeType_Capsule, cVector3f(afRadius, afHeight, afRadius), apOffsetMtx, mpNewtonWorld, this));
    mlstShapes.push_back(pShape);
...
iCollideShape* cPhysicsWorldNewton::CreateMeshShape(iVertexBuffer* apVtxBuffer)
{
    ...
    pShape->CreateFromVertices(apVtxBuffer->GetIndices(), apVtxBuffer->GetIndexNum(), apVtxBuffer->GetFloatArray(eVertexBufferElement_Position),
          apVtxBuffer->GetElementNum(eVertexBufferElement_Position), apVtxBuffer->GetVertexNum());
    mlstShapes.push_back(pShape);
...
{
    iPhysicsJointBall* pJoint = hplNew(cPhysicsJointBallNewton, (asName, apParentBody, apChildBody, this, avPivotPoint, avPinDir));
...
iPhysicsJointHinge* cPhysicsWorldNewton::CreateJointHinge(const tString& asName,
      ...
      iPhysicsBody* apChildBody)
{
    iPhysicsJointHinge* pJoint = hplNew(cPhysicsJointHingeNewton, (asName, apParentBody, apChildBody, this, avPivotPoint, avPinDir));
    mlstJoints.push_back(pJoint);
    return pJoint;
}
...
iPhysicsJointSlider* cPhysicsWorldNewton::CreateJointSlider(const tString& asName,
      ...
      iPhysicsBody* apChildBody)
{
    iPhysicsJointSlider* pJoint = hplNew(cPhysicsJointSliderNewton, (asName, apParentBody, apChildBody, this, avPivotPoint, avPinDir));
    mlstJoints.push_back(pJoint);
    return pJoint;
}
...
iPhysicsJointScrew* cPhysicsWorldNewton::CreateJointScrew(const tString& asName,
      ...
      iPhysicsBody* apChildBody)
{
    iPhysicsJointScrew* pJoint = hplNew(cPhysicsJointScrewNewton, (asName, apParentBody, apChildBody, this, avPivotPoint, avPinDir));
    mlstJoints.push_back(pJoint);
    return pJoint;
}
...
static float RayCastFilterFunc(const NewtonBody* apNewtonBody, const float* apNormalVec, int alCollisionID, void* apUserData, float afIntersetParam)
{
    cPhysicsBodyNewton* pRigidBody = (cPhysicsBodyNewton*)NewtonBodyGetUserData(apNewtonBody);
    if (pRigidBody->IsActive() == false)
        return 1;
...
bool cPhysicsWorldNewton::CheckShapeCollision(iCollideShape* apShapeA,
      ...
      bool abCorrectNormalDirection)
{
    ...
    if (pNewtonShapeA->GetType() == eCollideShapeType_Compound || pNewtonShapeB->GetType() == eCollideShapeType_Compound)
    {
        ...
        for (int a = 0; a < lACount; a++)
        {
            for (int b = 0; b < lBCount; b++)
            {
                ...
                int lNum = NewtonCollisionCollide(mpNewtonWorld, alMaxPoints, pSubShapeA->GetNewtonCollision(), &(mtxTransposeA.m[0][0]), pSubShapeB->GetNewtonCollision(),
                      &(mtxTransposeB.m[0][0]), mpTempPoints, mpTempNormals, mpTempDepths, 0);
                if (lNum < 1)
                    continue;
                if (lNum > alMaxPoints)
                    lNum = alMaxPoints;
    ...
    {
        // Log(" 1\n");
        int lNum = NewtonCollisionCollide(mpNewtonWorld, alMaxPoints, pNewtonShapeA->GetNewtonCollision(), &(mtxTransposeA.m[0][0]), pNewtonShapeB->GetNewtonCollision(),
              &(mtxTransposeB.m[0][0]), mpTempPoints, mpTempNormals, mpTempDepths, 0);
...
void cPhysicsWorldNewton::RenderShapeDebugGeometry(iCollideShape* apShape, const cMatrixf& a_mtxTransform, iLowLevelGraphics* apLowLevel, const cColor& aColor)
{
    gpLowLevelGraphics = apLowLevel;
    gDebugColor = aColor;

    cCollideShapeNewton* pNewtonShape = static_cast<cCollideShapeNewton*>(apShape);
    auto mat = a_mtxTransform.GetTranspose();
    NewtonCollisionForEachPolygonDo(pNewtonShape->GetNewtonCollision(), &mat.m[0][0], RenderDebugPolygon, NULL);
}
