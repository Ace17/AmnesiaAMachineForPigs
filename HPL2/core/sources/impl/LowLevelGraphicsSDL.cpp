...
bool cLowLevelGraphicsSDL::Init(int alWidth,
      int alHeight,
      int alDisplay,
      int alBpp,
      int abFullscreen,
      int alMultisampling,
      eGpuProgramFormat aGpuProgramFormat,
      ...
      const cVector2l& avWindowPos)
{
    ...
    mpScreen = SDL_CreateWindow(
          asWindowCaption.c_str(), SDL_WINDOWPOS_CENTERED_DISPLAY(mlDisplay), SDL_WINDOWPOS_CENTERED_DISPLAY(mlDisplay), mvScreenSize.x, mvScreenSize.y, mlFlags);
    if (mpScreen == NULL)
    {
        // try disabling FSAA
...
int cLowLevelGraphicsSDL::GetCaps(eGraphicCaps aType)
{
    ...
    case eGraphicCaps_ShaderModel_2:
        return (GLEW_ARB_fragment_program || GLEW_ARB_fragment_shader) ? 1 : 0; // Mac always support this, so not a good test.
#ifdef __APPLE__
    case eGraphicCaps_ShaderModel_3:
        return 0; // Force return false for OS X as dynamic branching doesn't work well (it's slow)
...
void cLowLevelGraphicsSDL::CopyFrameBufferToTexure(iTexture* apTex, const cVector2l& avPos, const cVector2l& avSize, const cVector2l& avTexOffset)
{
    ...
    SetTexture(0, apTex);
    glCopyTexSubImage2D(GetGLTextureTargetEnum(apTex->GetType()), 0, vTexPos.x, vTexPos.y, vPos.x, vPos.y, vSize.x, vSize.y);
}
...
void cLowLevelGraphicsSDL::SetColorWriteActive(bool abR, bool abG, bool abB, bool abA)
{
    ...
    if (mColorWrite.r == abR && mColorWrite.g == abG && mColorWrite.b == abB && mColorWrite.a == abA)
    {
        return;
    }
    mColorWrite.r = abR;
...
void cLowLevelGraphicsSDL::SetStencil(eStencilFunc aFunc, int alRef, unsigned int aMask, eStencilOp aFailOp, eStencilOp aZFailOp, eStencilOp aZPassOp)
{
    ...
    glStencilOp(GetGLStencilOpEnum(aFailOp), GetGLStencilOpEnum(aZFailOp), GetGLStencilOpEnum(aZPassOp));
}
...
void cLowLevelGraphicsSDL::SetBlendFuncSeparate(eBlendFunc aSrcFactorColor, eBlendFunc aDestFactorColor, eBlendFunc aSrcFactorAlpha, eBlendFunc aDestFactorAlpha)
{

    if (GLEW_EXT_blend_func_separate)
    {
        glBlendFuncSeparateEXT(GetGLBlendEnum(aSrcFactorColor), GetGLBlendEnum(aDestFactorColor), GetGLBlendEnum(aSrcFactorAlpha), GetGLBlendEnum(aDestFactorAlpha));
    }
    else
    {
...
void cLowLevelGraphicsSDL::DrawQuad(const cVector3f& avPos, const cVector2f& avSize, const cVector2f& avMinTexCoord, const cVector2f& avMaxTexCoord, const cColor& aColor)
{
...
void cLowLevelGraphicsSDL::DrawQuadMultiTex(const tVertexVec& avVtx, const tVector3fVec& avExtraUvs)
{
    ...
    glBegin(GL_QUADS);
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                glMultiTexCoord3fARB(GL_TEXTURE0_ARB + unit + 1, avExtraUvs[unit * 4 + i].x, avExtraUvs[unit * 4 + i].y, avExtraUvs[unit * 4 + i].z);
            }
...
void cLowLevelGraphicsSDL::DrawSphere(const cVector3f& avPos, float afRadius, cColor aCol)
{
    ...
    glBegin(GL_LINES);
    {
        ...
        {
            glVertex3f(avPos.x, avPos.y + sin(a) * afRadius, avPos.z + cos(a) * afRadius);

            glVertex3f(avPos.x, avPos.y + sin(a + afAngleStep) * afRadius, avPos.z + cos(a + afAngleStep) * afRadius);
        }

        // Y Circle:
        for (float a = 0; a < k2Pif; a += afAngleStep)
        {
            glVertex3f(avPos.x + cos(a) * afRadius, avPos.y, avPos.z + sin(a) * afRadius);

            glVertex3f(avPos.x + cos(a + afAngleStep) * afRadius, avPos.y, avPos.z + sin(a + afAngleStep) * afRadius);
        }
        ...
        for (float a = 0; a < k2Pif; a += afAngleStep)
        {
            ...
            glVertex3f(avPos.x + cos(a + afAngleStep) * afRadius, avPos.y + sin(a + afAngleStep) * afRadius, avPos.z);
        }
    }
    glEnd();
}
