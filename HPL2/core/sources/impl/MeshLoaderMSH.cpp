...
cMeshLoaderMSH::cMeshLoaderMSH(iLowLevelGraphics* apLowLevelGraphics)
    ...
    for (int sub = 0; sub < lSubMeshNum; ++sub)
    {
        ...
        // Colliders
        {
            ...
            for (int i = 0; i < lColliderNum; ++i)
            {
                ...
                if (gbLogMSHLoad)
                    Log("  Collider%d: %d %s %s %d\n", i, type, pCollider->m_mtxOffset.ToString().c_str(), pCollider->mvSize.ToString().c_str(), pCollider->mbCharCollider);
            }
        }
        ...
        ////////////////////
        // Vertex data
        iVertexBuffer* pVtxBuff = mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static, 0, 0);
        {
    ...
    for (int sub = 0; sub < apMesh->GetSubMeshNum(); sub++)
    {
        ...
        // Colliders
        {
            ...
            for (int i = 0; i < pSubMesh->GetColliderNum(); ++i)
            {
                ...
                if (gbLogMSHLoad)
                    Log("  Collider%d: %d %s %s %d\n", i, pCollider->mType, pCollider->m_mtxOffset.ToString().c_str(), pCollider->mvSize.ToString().c_str(),
                          pCollider->mbCharCollider);
            }
        }
...
void cMeshLoaderMSH::AddAnimation(cAnimation* apAnimation, cBinaryBuffer* apBuffer)
{
    ...
    for (int track = 0; track < apAnimation->GetTrackNum(); ++track)
    {
        ...
        for (int frame = 0; frame < pTrack->GetKeyFrameNum(); ++frame)
        {
            ...
            //														pFrame->rotation.v.ToString().c_str(), pFrame->rotation.w,
            //pFrame->time);
...
void cMeshLoaderMSH::AddNodeToBuffer(cNode3D* apNode, cBinaryBuffer* apBuffer, int alLevel)
{
    ...
    if (gbLogMSHLoad)
        Log("%s Node '%s' %d %d (%s)\n", GetLevelSpaces(alLevel), apNode->GetName().c_str(), apNode->GetCustomFlags(), apNode->GetChildList()->size(),
              apNode->GetLocalMatrix().ToString().c_str());
...
void cMeshLoaderMSH::GetNodeFromBuffer(cNode3D* apParentNode, cMesh* apMesh, cBinaryBuffer* apBuffer, int alLevel)
{
    ...
    if (gbLogMSHLoad)
        Log("%s Node '%s' %d %d (%s)\n", GetLevelSpaces(alLevel), pNode->GetName().c_str(), pNode->GetCustomFlags(), lChildNum, pNode->GetLocalMatrix().ToString().c_str());
...
void cMeshLoaderMSH::AddBoneToBuffer(cBone* apBone, cBinaryBuffer* apBuffer, int alLevel)
{
    ...
    if (gbLogMSHLoad)
        Log("%s Bone '%s' '%s' %d (%s)\n", GetLevelSpaces(alLevel), apBone->GetName().c_str(), apBone->GetSid().c_str(), apBone->GetChildList()->size(),
              apBone->GetLocalTransform().ToString().c_str());
...
void cMeshLoaderMSH::GetBoneFromBuffer(cBone* apParentBone, cBinaryBuffer* apBuffer, int alLevel)
{
    ...
    if (gbLogMSHLoad)
        Log("%s Bone '%s' '%s' %d (%s)\n", GetLevelSpaces(alLevel), pBone->GetName().c_str(), pBone->GetSid().c_str(), lChildNum, pBone->GetLocalTransform().ToString().c_str());
