...
void iVertexBufferOpenGL::CreateElementArray(eVertexBufferElement aType, eVertexBufferElementFormat aFormat, int alElementNum, int alProgramVarIndex)
{
...
bool iVertexBufferOpenGL::Compile(tVertexCompileFlag aFlags)
{
    ...
    if (aFlags & eVertexCompileFlag_CreateTangents)
    {
        ...
        cMath::CreateTriTangentVectors((float*)pTangentElement->GetArrayPtr(), &mvIndexArray[0], GetIndexNum(),

              (float*)pPosElement->GetArrayPtr(), pPosElement->mlElementNum,

              (float*)pTextureElement->GetArrayPtr(), (float*)pNormalElement->GetArrayPtr(), GetVertexNum());
    }
...
iVertexBuffer* iVertexBufferOpenGL::CreateCopy(eVertexBufferType aType, eVertexBufferUsageType aUsageType, tVertexElementFlag alVtxToCopy)
{
    ...
    for (size_t i = 0; i < mvElementArrays.size(); ++i)
    {
        ...
        memcpy(pDestElement->GetArrayPtr(), pSrcElement->GetArrayPtr(), pSrcElement->Size() * GetVertexFormatByteSize(pSrcElement->mFormat));
    }
