    ...
    : iPhysicsBody(asName, apWorld, apShape)
{
    ...
    mpNewtonBody = NewtonCreateBody(pWorldNewton->GetNewtonWorld(), pShapeNewton->GetNewtonCollision());
...
void cPhysicsBodyNewtonCallback::OnTransformUpdate(iEntity3D* apEntity)
{
    ...
    auto mat = apEntity->GetLocalMatrix().GetTranspose();
...
cMatrixf cPhysicsBodyNewton::GetInertiaMatrix()
{
    ...
    cMatrixf mtxI(fIxx, 0, 0, 0, 0, fIyy, 0, 0, 0, 0, fIzz, 0, 0, 0, 0, 1);
...
static int BuoyancyPlaneCallback(const int alCollisionID, void* apContext, const float* afGlobalSpaceMatrix, float* afGlobalSpacePlane)
{
...
void cPhysicsBodyNewton::OnUpdateCallback(const NewtonBody* apBody, dFloat afTimestep, int alThreadIndex)
{
    ...
    if (pRigidBody->mBuoyancy.mbActive && pRigidBody->mfBuoyancyDensityMul > 0)
    {
        ...
        NewtonBodyAddBuoyancyForce(apBody, pRigidBody->mBuoyancy.mfDensity * pRigidBody->mfBuoyancyDensityMul, pRigidBody->mBuoyancy.mfLinearViscosity,
              pRigidBody->mBuoyancy.mfAngularViscosity, vGravity.v, BuoyancyPlaneCallback, pRigidBody);
    }
