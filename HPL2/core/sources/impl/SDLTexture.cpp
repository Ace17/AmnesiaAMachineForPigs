...
bool cSDLTexture::CreateCubeFromBitmapVec(std::vector<cBitmap*>* avBitmaps)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        bool bRet = CreateTexture(mvTextureHandles[0], pBmp->GetData(0, 0), pBmp->GetNumOfMipMaps(), pBmp->GetSize(), pBmp->GetPixelFormat(), i, mbUseMipMaps, true);
        if (bRet == false)
            return false;
    }
...
void cSDLTexture::SetRawData(int alLevel, const cVector3l& avOffset, const cVector3l& avSize, ePixelFormat aPixelFormat, void* apData)
{
    ...
    {
        glTexSubImage1D(GLTarget, alLevel, avOffset.x, avSize.x, GLFormat, GL_UNSIGNED_BYTE, apData);
    }
    else if (mType == eTextureType_2D || mType == eTextureType_Rect)
    {
        glTexSubImage2D(GLTarget, alLevel, avOffset.x, avOffset.y, avSize.x, avSize.y, GLFormat, GL_UNSIGNED_BYTE, apData);
    }
    else if (mType == eTextureType_3D)
    {
        glTexSubImage3D(GLTarget, alLevel, avOffset.x, avOffset.y, avOffset.z, avSize.x, avSize.y, avSize.z, GLFormat, GL_UNSIGNED_BYTE, apData);
    }
...
bool cSDLTexture::CreateFromBitmapToIndex(cBitmap* apBmp, int alIdx)
{
    ...
    if (mType == eTextureType_CubeMap)
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            bool bRet = CreateTexture(mvTextureHandles[alIdx], apBmp->GetData(i, 0), apBmp->GetNumOfMipMaps(), apBmp->GetSize(), apBmp->GetPixelFormat(), i, mbUseMipMaps, true);
            if (bRet == false)
                return false;
        }
    }
    ...
    {
        bool bRet = CreateTexture(mvTextureHandles[alIdx], apBmp->GetData(0, 0), apBmp->GetNumOfMipMaps(), apBmp->GetSize(), apBmp->GetPixelFormat(), 0, mbUseMipMaps, true);
        if (bRet == false)
            return false;
    }
...
bool cSDLTexture::CreateTexture(int alTextureHandle,
      cBitmapData* apBitmapImage,
      int alNumOfMipMaps,
      const cVector3l avSize,
      ePixelFormat aPixelFormat,
      int alFaceNum,
      bool abGenerateMipMaps,
      bool abCheckForResize)
{
    ...
    if (mlSizeDownScaleLevel > 0 && abCheckForResize && (mbIsCompressed == false || alNumOfMipMaps > 1) && PixelFormatIsDepth(aPixelFormat) == false)
    {
        //////////////////////
        ...
        else if ((mType == eTextureType_2D || mType == eTextureType_CubeMap) && mvSize.x > mvMinDownScaleSize.x && mvSize.y > mvMinDownScaleSize.y)
        {
            ...
            gluScaleImage(GLFormat, mvSize.x, mvSize.y, GL_UNSIGNED_BYTE, apBitmapImage->mpData, vNewSize.x, vNewSize.y, GL_UNSIGNED_BYTE, pResizeData);
...
void cSDLTexture::GenerateMipMaps(GLenum aGLTarget, ePixelFormat aPixelFormat, const cVector3l avSize, unsigned char* apData, int alDataSize, int alFaceNum)
{
    GLenum GLFormat = PixelFormatToGLFormat(aPixelFormat);
    ...
    else if (PixelFormatIsCompressed(aPixelFormat) == false)
    {
        if (mType == eTextureType_1D)
        {
            gluBuild1DMipmaps(aGLTarget, GLInternalFormat, avSize.x, GLFormat, GL_UNSIGNED_BYTE, apData);
        }
        else if (mType == eTextureType_2D || mType == eTextureType_CubeMap)
        {
            gluBuild2DMipmaps(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : aGLTarget, GLInternalFormat, avSize.x, avSize.y, GLFormat,
                  GL_UNSIGNED_BYTE, apData);
        }
...
bool cSDLTexture::CopyTextureDataToGL(int alTextureHandle, int alLevel, unsigned char* apData, int alDataSize, const cVector3l avSize, ePixelFormat aPixelFormat, int alFaceNum)
{
    GLenum GLTarget = TextureTypeToGLTarget(mType);
    GLenum GLFormat = PixelFormatToGLFormat(aPixelFormat);
    GLenum GLInternalFormat = PixelFormatToGLInternalFormat(aPixelFormat);
    ...
    if (PixelFormatIsCompressed(aPixelFormat))
    {
        ...
        if (mType == eTextureType_1D)
        {
            glCompressedTexImage1DARB(GLTarget, alLevel, GLCompressionFormat, avSize.x, 0, alDataSize, apData);
        }
        else if (mType == eTextureType_2D || mType == eTextureType_CubeMap)
        {
            glCompressedTexImage2DARB(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : GLTarget, alLevel, GLCompressionFormat, avSize.x, avSize.y,
                  0, alDataSize, apData);
        }
        else if (mType == eTextureType_3D)
        {
            glCompressedTexImage3DARB(GLTarget, alLevel, GLCompressionFormat, avSize.x, avSize.y, avSize.z, 0, alDataSize, apData);
        }
    }
    /////////////////////////////////////////
    ...
    else
    {
        ...
        if (mType == eTextureType_1D)
        {
            glTexImage1D(GLTarget, alLevel, GLInternalFormat, avSize.x, 0, GLFormat, glType, apData);
        }
        else if (mType == eTextureType_2D || mType == eTextureType_Rect || mType == eTextureType_CubeMap)
        {
            glTexImage2D(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : GLTarget, alLevel, GLInternalFormat, avSize.x, avSize.y, 0, GLFormat,
                  glType, apData);
        }
        else if (mType == eTextureType_3D)
        {
            glTexImage3D(GLTarget, alLevel, GLInternalFormat, avSize.x, avSize.y, avSize.z, 0, GLFormat, glType, apData);
        }
    }
...
void cSDLTexture::SetupProperties(int alTextureHandle)
{
    ...
    else
    {
        ...
        if (mpLowLevelGraphics->GetCaps(eGraphicCaps_AnisotropicFiltering) && (float)mpLowLevelGraphics->GetCaps(eGraphicCaps_MaxAnisotropicFiltering) >= mfAnisotropyDegree)
        {
            glTexParameterf(GLTarget, GL_TEXTURE_MAX_ANISOTROPY_EXT, mfAnisotropyDegree);
        }
    }
