...
void cPhysicsMaterialNewton::UpdateMaterials()
{
    ...
    while (MatIt.HasNext())
    {
        ...
        ePhysicsMaterialCombMode frictionMode = (ePhysicsMaterialCombMode)std::max(mFrictionMode, pMat->mFrictionMode);
        ePhysicsMaterialCombMode elasticityMode = (ePhysicsMaterialCombMode)std::max(mElasticityMode, pMat->mElasticityMode);
        ...
        NewtonMaterialSetDefaultElasticity(mpNewtonWorld, mlMaterialId, pMat->mlMaterialId, Combine(elasticityMode, mfElasticity, pMat->mfElasticity));

        NewtonMaterialSetDefaultFriction(mpNewtonWorld, mlMaterialId, pMat->mlMaterialId, Combine(frictionMode, mfStaticFriction, pMat->mfStaticFriction),
        ...
        NewtonMaterialSetCollisionCallback(mpNewtonWorld, mlMaterialId, pMat->mlMaterialId, (void*)NULL, OnAABBOverlapCallback, ContactsProcessCallback);
    }
}
...
int cPhysicsMaterialNewton::OnAABBOverlapCallback(const NewtonMaterial* apMaterial, const NewtonBody* apBody1, const NewtonBody* apBody2, int alThreadIndex)
{
...
void cPhysicsMaterialNewton::ContactsProcessCallback(const NewtonJoint* apContactJoint, dFloat afTimestep, int alThreadIndex)
{
    ...
    if (pMaterial1->GetSurfaceData() && pMaterial2->GetSurfaceData() && pContactBody1->GetUseSurfaceEffects() && pContactBody2->GetUseSurfaceEffects() &&
          pContactBody1->GetBuoyancyActive() == false && pContactBody2->GetBuoyancyActive() == false)
    {
        pMaterial1->GetSurfaceData()->CreateImpactEffect(
              contactData.mfMaxContactNormalSpeed, contactData.mvContactPosition, lContactNum, pMaterial2->GetSurfaceData(), pContactBody1->GetWorld());
        ...
        if (lPrio1 >= lPrio2)
        {
            if (std::abs(contactData.mfMaxContactNormalSpeed) > 0)
                pMaterial1->GetSurfaceData()->OnImpact(contactData.mfMaxContactNormalSpeed, contactData.mvContactPosition, lContactNum, pContactBody1);
            if (std::abs(contactData.mfMaxContactTangentSpeed) > 0)
                pMaterial1->GetSurfaceData()->OnSlide(contactData.mfMaxContactTangentSpeed, contactData.mvContactPosition, lContactNum, pContactBody1, pContactBody2);
        }

        if (lPrio2 >= lPrio1 && pMaterial2 != pMaterial1)
        {
            if (std::abs(contactData.mfMaxContactNormalSpeed) > 0)
                pMaterial2->GetSurfaceData()->OnImpact(contactData.mfMaxContactNormalSpeed, contactData.mvContactPosition, lContactNum, pContactBody2);
            if (std::abs(contactData.mfMaxContactTangentSpeed) > 0)
                pMaterial2->GetSurfaceData()->OnSlide(contactData.mfMaxContactTangentSpeed, contactData.mvContactPosition, lContactNum, pContactBody2, pContactBody1);
        }
    }
