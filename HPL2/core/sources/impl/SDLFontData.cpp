...
bool cSDLFontData::CreateFromBitmapFile(const tWString& asFileName)
{
    ...
    for (; pCharElem != NULL; pCharElem = pCharElem->NextSiblingElement("char"))
    {
        ...
        cGlyph* pGlyph = CreateGlyph(pImage, cVector2l(lXOffset, lYOffset), cVector2l(lW, lH), cVector2l(lBase + lOutline, lLineHeight + lOutline), (lOutline + lAdvance * 2) / 2);
...
bool cSDLFontData::CreateFromFontFile(const tWString& asFileName, int alSize, unsigned short alFirstChar, unsigned short alLastChar)
{
    /*SetFullPath(asFileName);
