...
cSDLEngineSetup::cSDLEngineSetup(tFlag alHplSetupFlags)
{
    ...
    if (alHplSetupFlags & (eHplSetup_Screen | eHplSetup_Video))
    {
        ...
        if (SDL_GetWMInfo(&info))
        {
            ...
            hDevNotify = RegisterDeviceNotification(info.window, &notificationFilter, DEVICE_NOTIFY_WINDOW_HANDLE | DEVICE_NOTIFY_ALL_INTERFACE_CLASSES);
...
cScene*
cSDLEngineSetup::CreateScene(cGraphics* apGraphics, cResources* apResources, cSound* apSound, cPhysics* apPhysics, cSystem* apSystem, cAI* apAI, cGui* apGui, cHaptic* apHaptic)
{
