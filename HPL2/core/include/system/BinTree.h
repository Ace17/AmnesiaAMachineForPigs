...
public:
    ...
    BinTreeNode<T>* GetChild(eBinTreeNode i) { return mChild[i]; }

    BinTreeNode<T>* GetParent() { return mParent; }
...
private:
    ...
    void PopulateLeafList(BinTreeNode<T>* aNode)
    {
        ...
        if (aNode->GetChild(eBinTreeNode_Left) == NULL && aNode->GetChild(eBinTreeNode_Right) == NULL)
        {
            mvNodes.push_back(aNode);
        }
