...
public:
    ...
    cLinearOscillation() {}
    cLinearOscillation(float afMin, float afMax, float afVal, float afUpSpeed, float afDownSpeed) { SetUp(afMin, afMax, afVal, afUpSpeed, afDownSpeed); }
...
template<class T>
class cSmoothInterpolation
{
public:
    cSmoothInterpolation() { mlMaxValues = 1; }

    void SetMaxValues(size_t alMax) { mlMaxValues = alMax; }
...
public:
    ...
    inline bool operator==(const cPlane<T>& aPlane) const { return (a == aPlane.a && b == aPlane.b && c == aPlane.c && d == aPlane.d); }

    inline bool operator!=(const cPlane<T>& aPlane) const { return !(a == aPlane.a && b == aPlane.b && c == aPlane.c && d == aPlane.d); }

    /////////////////

    inline cPlane<T> operator*(const T& aVal) const { return cPlane<T>(a * aVal, b * aVal, c * aVal, d * aVal); }
    ...
    cPlane(const cVector3<T>& avNormal, const cVector3<T>& avPoint) { FromNormalPoint(avNormal, avPoint); }

    /////////////////

    cPlane(const cVector3<T>& avPoint0, const cVector3<T>& avPoint1, const cVector3<T>& avPoint2) { FromPoints(avPoint0, avPoint1, avPoint2); }
    ...
    inline void FromPoints(const cVector3<T>& avPoint0, const cVector3<T>& avPoint1, const cVector3<T>& avPoint2)
    {
    ...
    inline cVector3<T> GetNormal() const { return cVector3<T>(a, b, c); }
...
typedef union tIntOrFloatUnion
{
    ...
    tIntOrFloatUnion() {}
    tIntOrFloatUnion(int lVal)
        : i(lVal)
    {
    }
    tIntOrFloatUnion(float fVal)
        : f(fVal)
    {
    }
} tIntOrFloat;
