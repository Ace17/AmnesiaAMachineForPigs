...
public:
     ...
     */
    static bool CheckSphereLineIntersection(const cVector3f& avSpherePos,
     ...
     * \return true if intersection, else false.
     */
    static bool CheckAABBIntersection(const cVector3f& avMin1, const cVector3f& avMax1, const cVector3f& avMin2, const cVector3f& avMax2);

    /**
     * Check if an AABB is completely inside another.
     */
    static bool CheckAABBInside(const cVector3f& avInsideMin, const cVector3f& avInsideMax, const cVector3f& avOutsideMin, const cVector3f& avOutsideMax);
    ...
    /**
     * Creates a clip rect for a bounding volume that comes in -1 - 1 for points in screen. It can return points above and below 1/-1 though. Returns false if all points are behind
     * frustum,
     */
    static bool GetNormalizedClipRectFromBV(cVector3f& avDestMin, cVector3f& avDestMax, cBoundingVolume& aBV, cFrustum* apFrustum, float afTanHalfFov);
     ...
     */
    static void GetClipRectFromBV(cRect2l& aDestRect, cBoundingVolume& aBV, cFrustum* apFrustum, const cVector2l& avScreenSize, float afTanHalfFov);

    /**
     * Creates a clip rect for a sphere. Can handle even if part of sphere is behind near clip.
     */
    static cRect2l
    GetClipRectFromSphere(const cVector3f& avPosition, float afRadius, cFrustum* apFrustum, const cVector2l& avScreenSize, bool abPositionIsScreenSpace, float afTanHalfFov);
     ...
     */
    static bool
    CheckAABBLineIntersection(const cVector3f& avMin, const cVector3f& avMax, const cVector3f& avLineStart, const cVector3f& avLineEnd, cVector3f* apIntersectionPos, float* apT);
     ...
     * \return
     */
    static bool CheckPointInFlatPolygon(const cVector3f& avRayStart,
          ...
          tVector3fVec* apTriangleWorldSpace);
     ...
     * \return
     */
    static int RoundToInt(float afVal) { return (int)floor(afVal + 0.5); }

    static cVector2l RoundToInt(const cVector2f& avX) { return cVector2l(RoundToInt(avX.x), RoundToInt(avX.y)); }

    static cVector3l RoundToInt(const cVector3f& avX) { return cVector3l(RoundToInt(avX.x), RoundToInt(avX.y), RoundToInt(avX.z)); }
    ...
    static inline cVector2f Vector2Floor(const cVector2f& avVec) { return cVector2f(floor(avVec.x), floor(avVec.y)); }

    static inline cVector2f Vector2Ceil(const cVector2f& avVec) { return cVector2f(ceil(avVec.x), ceil(avVec.y)); }

    static inline cVector2f Vector2Abs(const cVector2f& avVec) { return cVector2f(Abs(avVec.x), Abs(avVec.y)); }

    static inline cVector2f Vector2Min(const cVector2f& avVecA, const cVector2f& avVecB) { return cVector2f(Min(avVecA.x, avVecB.x), Min(avVecA.y, avVecB.y)); }

    static inline cVector2f Vector2Max(const cVector2f& avVecA, const cVector2f& avVecB) { return cVector2f(Max(avVecA.x, avVecB.x), Max(avVecA.y, avVecB.y)); }
    ...
    static inline cVector2f Vector2ToRad(const cVector2f& avVec) { return cVector2f(ToRad(avVec.x), ToRad(avVec.y)); }

    static inline cVector2f Vector2ToDeg(const cVector2f& avVec) { return cVector2f(ToDeg(avVec.x), ToDeg(avVec.y)); }
    ...
    static inline cVector3f Vector3ToRad(const cVector3f& avVec) { return cVector3f(ToRad(avVec.x), ToRad(avVec.y), ToRad(avVec.z)); }

    static inline cVector3f Vector3ToDeg(const cVector3f& avVec) { return cVector3f(ToDeg(avVec.x), ToDeg(avVec.y), ToDeg(avVec.z)); }
    ...
    static inline cVector3f Vector3Floor(const cVector3f& avVec) { return cVector3f(floor(avVec.x), floor(avVec.y), floor(avVec.z)); }

    static inline cVector3f Vector3Ceil(const cVector3f& avVec) { return cVector3f(ceil(avVec.x), ceil(avVec.y), ceil(avVec.z)); }

    static inline cVector3f Vector3Abs(const cVector3f& avVec) { return cVector3f(Abs(avVec.x), Abs(avVec.y), Abs(avVec.z)); }
    ...
    static inline float Vector3Dist(const cVector3f& avStartPos, const cVector3f& avEndPos) { return sqrt(Vector3DistSqr(avStartPos, avEndPos)); }
    ...
    static void Vector3OrthonormalizeBasis(const cVector3f& avSrcRight,
          const cVector3f& avSrcUp,
          const cVector3f& avSrcForward,
          cVector3f& avDstRight,
          cVector3f& avDstUp,
          cVector3f& avDstForward);
     ...
     * \return
     */
    static eCollision CheckPlaneAABBCollision(const cPlanef& aPlane, const cVector3f& avMin, const cVector3f& avMax, const cVector3f& avSphereCenter, float afSphereRadius);
     ...
     */
    static bool CheckPlaneLineIntersection(const cPlanef& aPlane, const cVector3f& avLineStart, const cVector3f& avLineEnd, cVector3f* apIntersectionPos, float* apT);
     ...
     */
    static bool CheckSeparatingAxisIntersection(cVector3f* apVerticesA,
     ...
     */
    static bool CheckLineTriangleIntersection(const cVector3f& avLineStart,
          const cVector3f& avLineEnd,
          const cVector3f& avP0,
          const cVector3f& avP1,
          const cVector3f& avP2,
          float* apT,
          bool abSkipBackfacing = true);

    /**
     * Checks intersection between line and a mesh. For speed reasons the matrix is INVERSE!
     */
    static bool CheckLineTriMeshIntersection(const cVector3f& avLineStart,
          const cVector3f& avLineEnd,
     ...
     * Checks intersection between line and a mesh. For speed reasons the matrix is INVERSE!
     */
    static bool CheckLineTriVertexBufferIntersection(const cVector3f& avLineStart,
          const cVector3f& avLineEnd,
          const cMatrixf& a_mtxInvMeshMtx,
     ...
     */
    static inline float MatrixMinor(const cMatrixf& a_mtxA, const size_t r0, const size_t r1, const size_t r2, const size_t c0, const size_t c1, const size_t c2);
    /**
     ...
     */
    static bool
    CreateEdges(tTriEdgeVec& avEdges, const unsigned int* apIndexArray, int alIndexNum, const float* apVertexArray, int alVtxStride, int alVertexNum, bool* apIsDoubleSided);
};
