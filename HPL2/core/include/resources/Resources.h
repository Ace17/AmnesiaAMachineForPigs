...
{
public:
    iMapDataLoader(const tString& asName)
        : msName(asName)
    {
    }
...
{
public:
    iEntityLoader(const tString& asName)
        : iMapDataLoader(asName)
        , mbCreatesStaticEntity(false)
    {
    }
    ...
    virtual iEntity3D* Load(const tString& asName,
          int alID,
          bool abActive,
          ...
          cResourceVarsObject* apInstanceVars) = 0;
...
{
public:
    iAreaLoader(const tString& asName)
        : iMapDataLoader(asName)
        , mbCreatesStaticArea(false)
    {
    }
    virtual ~iAreaLoader() {}
