...
public:
    ...
    /**
     * Decompresses the data from a source buffer, beginning at the current position. If abSizeDataWritten is false, it stops at EOF and assumes size has not been writen. If true,
     * it will assume that the first 4 bytes is the size of the data. It also updates the current postion in the data for SrcBuffer
     */
     ...
     */
    void EncryptTEA(size_t alKey[4]);

    /**
     * Decrypt this buffer using Tiny Encryption Algorithm
     */
    void DecryptTEA(size_t alKey[4]);
