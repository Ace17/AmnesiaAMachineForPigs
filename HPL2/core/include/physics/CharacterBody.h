...
public:
    ...
    void SetLastPosition(const cVector3f& avPos)
    {
        mvLastPosition = avPos;
    }; // quite hacky, but necessary. Last position is used to determine velocity. If we want to teleport a pig and maintain its velocity, we need to fix up the last position too.
