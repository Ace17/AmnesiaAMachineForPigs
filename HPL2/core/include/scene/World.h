...
{
public:
    cTempAiNode(const cVector3f& avPos, const tString& asName, int alID)
        : mvPos(avPos)
        , msName(asName)
        , mlID(alID)
    {
...
class cStartPosEntity : public iSerializable
{
    ...
    cStartPosEntity(const tString& asName)
        : msName(asName)
    {
    }
...
{
public:
    cWorld(tString asName, cGraphics* apGraphics, cResources* apResources, cSound* apSound, cPhysics* apPhysics, cScene* apScene, cSystem* apSystem, cAI* apAI, cHaptic* apHaptic);
    ~cWorld();
    ...
    iEntity3D* CreateEntity(const tString& asName,
