...
public:
    iTexture(const tString& asName, const tWString& asFullPath, eTextureType aType, eTextureUsage aUsage, iLowLevelGraphics* apLowLevelGraphics)
        ...
        , mWrapS(eTextureWrap_Repeat)
        , mWrapT(eTextureWrap_Repeat)
        , mWrapR(eTextureWrap_Repeat)
        , mfFrameTime(1)
        , mAnimMode(eTextureAnimMode_Loop)
        , mlSizeDownScaleLevel(0)
        , mvMinDownScaleSize(16, 16, 16)
        , mfAnisotropyDegree(1.0f)
        , mFilter(eTextureFilter_Bilinear)
    ...
    virtual void SetRawData(int alLevel, const cVector3l& avOffset, const cVector3l& avSize, ePixelFormat aPixelFormat, void* apData) = 0;
