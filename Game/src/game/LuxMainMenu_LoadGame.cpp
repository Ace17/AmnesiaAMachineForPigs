...
cLuxMainMenu_LoadGame::cLuxMainMenu_LoadGame(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin)
...
bool cLuxMainMenu_LoadGame::PressOK(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if (mpLBSavedGames->GetSelectedItem() < 0)
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("LoadGame", "NoGame"), kTranslate("Global", "OK"), _W(""), NULL, NULL);
    else
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("LoadGame", "LoadGameMessage"), kTranslate("Global", "OK"), kTranslate("Global", "Cancel"),
