...
cLuxMainMenu_Profile::cLuxMainMenu_Profile(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin)
...
void cLuxMainMenu_Profile::CreateMainGui()
{
    ...
    //////////////////////////
    // List box
    mpListProfiles = mpGuiSet->CreateWidgetListBox(
          vPos, cVector2f(mvWindowSize.x - mfInformationWidth - fBorderSize * 2, mvWindowSize.y - vPos.y - 40 - fBorderSize * 2), mpWindow);
...
void cLuxMainMenu_Profile::CreateEnterNameGui()
{
    ...
    // Window
    vPos.y += 5;
    mpWindowEnterName = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_ButtonClose, cVector3f(0, 0, 10), mvEnterNameWindowSize, kTranslate("MainMenu", "Create Profile"));
...
bool cLuxMainMenu_Profile::PressDeleteProfile(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "Sure you want to delete profile"), kTranslate("MainMenu", "Yes"), kTranslate("MainMenu", "No"), this,
          kGuiCallback(PressDeleteProfilePopupClose));
    pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
...
static unsigned char gvEncStr[54] = { 0x2B, 0x15, 0x7, 0x12, 0x5B, 0x47, 0x6C, 0x30, 0x37, 0x26, 0x22, 0x2E, 0x20, 0x2C, 0x2E, 0x2E, 0x36, 0x2D, 0x2A, 0x37, 0x3A, 0x6D, 0x20, 0x2C,
    0x2E, 0x6C, 0x22, 0x30, 0x6C, 0x21, 0x2C, 0x36, 0x2D, 0x20, 0x26, 0x6C, 0x27, 0x22, 0x6C, 0x31, 0x28, 0x27, 0x26, 0x30, 0x20, 0x26, 0x6C, 0x2D, 0x37, 0x6C, 0x7C, 0x28, 0x7E,
    0 };
...
static tString DecryptCryptoStringCode(const tString& asEncStr, cBinaryBuffer& aKeyBuffer)
{
    ...
    for (size_t i = 0; i < asEncStr.size(); ++i)
    {
        ...
        if (++lBuffPos >= aKeyBuffer.GetSize())
            aKeyBuffer.SetPos(0);
    }
