...
cLuxPropLoader_Lever::cLuxPropLoader_Lever(const tString& asName)
...
cLuxProp_Lever::cLuxProp_Lever(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Lever)
{
...
void cLuxProp_Lever::UpdateCheckLimit(float afAngle, float afTimeStep)
{
    ...
    ///////////////////////
    // Middle
    else if (afAngle < (mfDefaultMaxAngle - mfMaxLimitRange) - cMath::ToRad(5) || afAngle > (mfDefaultMinAngle + mfMinLimitRange) + cMath::ToRad(5))
    {
        ChangeState(0, true);
    }
}
