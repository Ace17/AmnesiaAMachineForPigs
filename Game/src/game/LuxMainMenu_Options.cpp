...
cLuxMainMenu_Options::cLuxMainMenu_Options(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin)
...
void cLuxMainMenu_Options::AddGraphicsOptions(cWidgetTab* apTab)
{
    ...
    mpBToggleShowGfxOptions = mpGuiSet->CreateWidgetButton(0, cVector2f(0, 25), vToggleButtonStrings[0], apTab);
    ...
    {
        float fStringLength = mpBToggleShowGfxOptions->GetDefaultFontType()->GetLength(mpBToggleShowGfxOptions->GetDefaultFontSize(), mpBToggleShowGfxOptions->GetText().c_str()) +
              10;
...
void cLuxMainMenu_Options::AddBasicGfxOptions(cWidgetDummy* apDummy)
{
    ...
    cWidgetGroup* pGroup = mpGuiSet->CreateWidgetGroup(vPos, 0, kTranslate("OptionsMenu", "Screen"), apDummy);
    {
        ...
        mpChBAdaptiveVSync = mpGuiSet->CreateWidgetCheckBox(
              vPosInGroup + cVector3f(mpChBVSync->GetSize().x + 10, mpChBFullScreen->GetSize().y + 10, 0), 0, kTranslate("OptionsMenu", "AdaptiveVSync"), pGroup);
...
void cLuxMainMenu_Options::AddAdvancedGfxOptions(cWidgetDummy* apDummy)
{
    ...
    pGroup = mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("KeyConfig", "Misc"), pMainFrame);
    {
        ...
        mpChBWorldReflection = mpGuiSet->CreateWidgetCheckBox(
              vPosInGroup, 0, kTranslate("OptionsMenu", "WorldReflection") + _W(" (") + kTranslate("OptionsMenu", "Water") + _W(")"), pGroup);
...
void cLuxMainMenu_Options::AddInputOptions(cWidgetTab* apTab)
{
    ...
    SetUpSlider(mpSGamepadLookSensitivity, mfGamepadLookSensitivityMin, mfGamepadLookSensitivityMax, mfGamepadLookSensitivityStep,
          kGuiCallback(GamepadLookSensitivitySlider_OnMove), &mpLGamepadLookSensitivity);
...
void cLuxMainMenu_Options::PopulateLanguageList()
{
    ...
    for (; it != lstLangs.end(); ++it)
    {
        ...
        tWString sLangEntry = cString::ToLowerCaseW(cString::SubW(sLang, 0, cString::GetLastStringPosW(sLang, _W("."))));
...
void cLuxMainMenu_Options::SetSliderLabelString(cWidgetLabel* apLabel, float afValue, float afMinValue, float afMaxValue, const tWString& asMin, const tWString& asMax)
{
    tWString sText;
