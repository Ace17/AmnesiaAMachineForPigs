...
cLuxAreaLoader_Sticky::cLuxAreaLoader_Sticky(const tString& asName)
...
cLuxArea_Sticky::cLuxArea_Sticky(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxArea(asName, alID, apMap, eLuxAreaType_Sticky)
{
    mpAttachedBody = NULL;
...
void cLuxArea_Sticky::UpdateAttachBody(float afTimeStep)
{
    ...
    else if (mbMoveBody && !mbRotateBody)
    {
        ...
        cVector3f vNew = mtxAttachedStart.GetTranslation() * (1 - mfSetMtxTime) + vGoal * mfSetMtxTime;
...
void cLuxArea_Sticky::UpdateCollision(float afTimeStep)
{
    ...
    for (size_t i = 0; i < vBodies.size(); ++i)
    {
        ...
        ///////////////////////////////
        // Check for collision
        if (pPhysicsWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), pAreaBody->GetShape(), pAreaBody->GetLocalMatrix(), collideData, 1, false) == false)
        {
            continue;
        }
