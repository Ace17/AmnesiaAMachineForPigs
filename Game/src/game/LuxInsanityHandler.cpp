...
void iLuxInstanityEvent::Start()
{
    ...
    gpBase->mpDebugHandler->AddMessage(_W("Starting Insanity event: '") + cString::To16Char(msName) + _W("'") + _W("set: '") + cString::To16Char(msSet) + _W("'"), false);
...
void cLuxInstanityEvent_Bugs::OnDraw(float afFrameTime)
{
    ...
    for (int i = 0; i < mlNumOfBugs; ++i)
    {
        ...
        pSet->DrawGfx(mpBugImage, pBug->mvPos, mpBugImage->GetActiveSize() * pBug->mfSizeMul, cColor(1, pBug->mfAlpha), eGuiMaterial_LastEnum, pBug->mfAngle);
    }
}
...
{
    if (gpBase->mpMessageHandler->ShowEffectSubtitles() && mvCurrentTextRows.empty() == false && gpBase->mpEffectHandler->GetPlayVoice()->IsActive() == false)
    {
...
cLuxInsanityHandler::cLuxInsanityHandler()
    : iLuxUpdateable("LuxInsanityHandler")
{
