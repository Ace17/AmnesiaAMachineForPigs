...
iLuxPropLoader::iLuxPropLoader(const tString& asName)
...
void iLuxPropLoader::AfterLoad(cXmlElement* apRootElem, const cMatrixf& a_mtxTransform, cWorld* apWorld, cResourceVarsObject* apInstanceVars)
{
    ...
    if (apInstanceVars)
    {
        ...
        pProp->SetPlayerLookAtCallback(apInstanceVars->GetVarString("PlayerLookAtCallback", ""), apInstanceVars->GetVarBool("PlayerLookAtCallbackAutoRemove", false));

        pProp->SetPlayerInteractCallback(apInstanceVars->GetVarString("PlayerInteractCallback", ""), apInstanceVars->GetVarBool("PlayerInteractCallbackAutoRemove", false));
...
iLuxProp::iLuxProp(const tString& asName, int alID, cLuxMap* apMap, eLuxPropType aPropType)
    : iLuxEntity(asName, alID, apMap, eLuxEntityType_Prop)
{
    mPropType = aPropType;
...
void iLuxProp::MoveAngularTo(const cMatrixf& a_mtxGoal,
      float afAcc,
      float afMaxSpeed,
      float afSlowdownDist,
      bool abResetSpeed,
      bool abUseOffset,
      const cVector3f& avWorldOffset,
...
void iLuxProp::RotateAtSpeed(float afAcc,
      float afGoalSpeed,
      const cVector3f& avAxis,
      bool abResetSpeed,
      bool abUseOffset,
      const cVector3f& avWorldOffset,
      const cVector3f& avLocalOffset)
...
void iLuxProp::UpdateAttachedProps(float afTimeStep, bool abForceUpdate)
{
    ...
    for (; it != mlstAttachedProps.end(); ++it)
    {
        ...
        if (pProp->GetBodyNum() > 0)
        {
            ...
            cMatrixf mtxProp = cMath::MatrixMul(GetMainBody()->GetLocalMatrix(), cMath::MatrixMul(pAttachProp->m_mtxOffset, pProp->mvBodyExtraData[0].m_mtxLocalTransform));
...
void iLuxProp::UpdateAngularMoving(float afTimeStep)
{
    ...
    if (mbMoveAngularNoGoal)
    {
        ...
        if ((fAcc < 0 && mfMoveAngularSpeed < mfMoveAngularMaxSpeed) || (fAcc > 0 && mfMoveAngularSpeed > mfMoveAngularMaxSpeed))
        {
...
void iLuxProp::UpdateCheckIfOutsidePlayer(float afTimeStep)
{
    ...
    for (size_t i = 0; i < mvBodies.size(); ++i)
    {
        ...
        bool bCollide = pPhysicsWorld->CheckShapeCollision(
              pBody->GetShape(), pBody->GetLocalMatrix(), pPlayerBody->GetShape(), pPlayerBody->GetLocalMatrix(), collideData, 1, false);
        if (bCollide)
        {
            bNotColliding = false;
...
kBeginSerializeBase(cLuxPropConnectedProp) kSerializeVar(msName, eSerializeType_String)
kEndSerialize()
...
kSerializeClassContainer(mvBodies, cEngineBody_SaveData, eSerializeType_Class) kSerializeClassContainer(mvJoints, cEngineJoint_SaveData, eSerializeType_Class)
      kSerializeClassContainer(mvPS, cEnginePS_SaveData, eSerializeType_Class) kSerializeClassContainer(mvBeams, cEngineBeam_SaveData, eSerializeType_Class)
            kSerializeClassContainer(mvLights, cEngineLight_SaveData, eSerializeType_Class) kSerializeClassContainer(mvSounds, cEngineSound_SaveData, eSerializeType_Class)
...
// CreateEntity seems broken. It does not add the entity to the map, but returns map->GetLatestEntity(). I do not want to interfere with the rest at this point, so creating a
// second method for my means.
...
void iLuxProp::LoadFromSaveData(iLuxEntity_SaveData* apSaveData)
{
    ...
    if (mfBodyMassBackup == 0 || mfBodyMassBackup < 0 || (mfBodyMassBackup == mfBodyMassBackup) == false)
        mfBodyMassBackup = 1;
