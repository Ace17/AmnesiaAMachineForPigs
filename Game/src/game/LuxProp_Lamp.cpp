...
cLuxPropLoader_Lamp::cLuxPropLoader_Lamp(const tString& asName)
...
void cLuxLampLightConnection::Update(float afTimeStep)
{
    ...
    for (; it != mlstLamps.end(); ++it)
    {
        ...
        cColor lightColor = pLampConnection->mbUseLightOnColor ? pLamp->mvEffectLightData[0].mOnColor : pLamp->mvLights[0]->GetDiffuseColor();
...
cLuxProp_Lamp::cLuxProp_Lamp(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Lamp)
{
    mbLit = true;
...
{
    if (mbCanBeLitByPlayer && (mbLit == false || mbCanBeTurnedOff))
        return true;
...
void cLuxProp_Lamp::UpdatePropSpecific(float afTimeStep)
{
    ...
    if (mbFlickerActive && mvLights.size() > 0)
    {
        ...
        for (size_t i = 0; i < mvParticleSystems.size(); ++i)
        {
            ...
            if (pPS && mpWorld->ParticleSystemExists(pPS))
                pPS->SetFlickerMultiplier(flickerOn ? 1.0f : 0.25f);
        }
    }
