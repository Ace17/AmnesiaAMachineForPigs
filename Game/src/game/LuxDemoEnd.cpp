...
void cLuxDemoEnd::OnDraw(float afFrameTime)
{
    ...
    if (mpFontMessage)
    {
        ...
        if (mbIsSteamVersion)
            mpGuiSet->DrawFont(
                  _W("Available now on Steam!"), mpFontMessage, cVector3f(400.0f, mfAvailableAtY, 10), mvAvailableAtFontSize, mAvailableAtFontColor, eFontAlign_Center);
        else
            mpGuiSet->DrawFont(
                  kTranslate("Demo", "AvailableAt"), mpFontMessage, cVector3f(400.0f, mfAvailableAtY, 10), mvAvailableAtFontSize, mAvailableAtFontColor, eFontAlign_Center);
    }
