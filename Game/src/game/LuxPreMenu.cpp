...
void cLuxPreMenu::UpdateState()
{
    ...
    case eLuxPreMenuState_FastFadeOut:
    {
        if (mfAlphaFade == 1)
        {
            ...
            {
                while (mlCurrentSectionIdx < (int)mvSections.size() && mvSections[mlCurrentSectionIdx]->ShowFirstStartOnly())
                {
                    ++mlCurrentSectionIdx;
                }
            }
    ...
    case eLuxPreMenuState_ShowPremenuSection:
    {
        if (mpCurrentSection->HasTextElements() == false && mpCurrentSection->HasGammaSettings() == false && mfTimer <= 0)
        {
            mCurrentState = eLuxPreMenuState_FadeOut;
        }
    }
...
void cLuxPreMenu::LoadPreMenuSections()
{
    ...
    if (gpBase->msUserResourceDir.length() && cPlatform::FileExists(gpBase->msUserResourceDir + sConfigPath))
    {
