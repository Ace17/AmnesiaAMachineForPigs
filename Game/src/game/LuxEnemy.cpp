...
float iLuxEnemy::DrawDebug(cGuiSet* apSet, iFontData* apFont, float afStartY)
{
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  State: '%ls' MoveState: '%ls' Pose: '%ls' PatrolNode: %d"), gsLuxEnemyStates[mCurrentState].c_str(),
          sMoveState.c_str(), gsPoseStates[mCurrentPose].c_str(), mlCurrentPatrolNode);
    afStartY += 14;
...
void iLuxEnemy::UpdateCanSeePlayer(float afTimeStep)
{
    ...
    if (bLanternOn == false && pPlayer->GetHelperLightLevel()->GetNormalLightLevel() < mfPlayerInDarknessLightLevel && mbSkipVisibilityRangeHandicaps == false)
    {
        fMaxRange = mfDarknessSightRange;
        if (pPlayer->GetAvgSpeed() < 0.05f)
            fMaxRange *= 0.5f;
    }
...
{
    if (gpBase->mpPlayer->IsDead() || TriggersDisabled())
        return;
...
bool iLuxEnemy::Attack(const cEnemyAttackSizeData& aSizeData, const cEnemyAttackDamageData& aDamageData, float afDamageMul)
{
    ...
    bool bHit = gpBase->mpMapHelper->ShapeDamage(GetAttackShape(aSizeData.mlShapeIdx), GetDamageShapeMatrix(aSizeData.mvOffset), mpCharBody->GetPosition(),
          aDamageData.mfMinDamage * afDamageMul, aDamageData.mfMaxDamage * afDamageMul, aDamageData.mfForce, aDamageData.mfMaxImpulse, aDamageData.mlStrength,
          aDamageData.mfHitSpeed, aDamageData.mDamageType, aDamageData.mWeaponHitType, false, aDamageData.mbCheckPlayer, aDamageData.mbCheckProps, true, &bHitPlayer);
    if (bHit)
    {
        mlAttackHitCounter++;
