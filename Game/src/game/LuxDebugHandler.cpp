...
cLuxDebugHandler::cLuxDebugHandler()
    : iLuxUpdateable("LuxDebugHandler")
{
    ///////////////////////////////
    // Setup GUI stuff
...
static void PrintContainerNode(iRenderableContainerNode* apNode, int alLevel)
{
    ...
    Log("%sNode %d Radius: %f Min: (%s) Max: (%s) Objects: %d\n", GetSpaces(alLevel), apNode, apNode->GetRadius(), apNode->GetMin().ToString().c_str(),
          apNode->GetMax().ToString().c_str(), apNode->GetObjectNum());
...
void cLuxDebugHandler::OnDraw(float afFrameTime)
{
    ...
    if (mbShowFPS)
    {
        ...
        gpBase->mpGameDebugSet->DrawFont(
              gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("FrameTime: %.1fms FPS: %.1f\n"), fFrameTime, gpBase->mpEngine->GetFPS());
        fY += 13.0f;
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Max GameLogic: %05.2fms Max RenderingLogic: %05.2fms\n"),
    ...
    {
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Draw Calls: %d"), iRenderer::GetDrawCalls());
        fY += 13.0f;
    }
    ...
    if (cRendererDeferred::GetDebugRenderLightComplexity())
    {
        ...
        for (int i = 0; i < mvLightComplexity.size(); ++i)
        {
            ...
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5 + 1, fY + 1, -1), 14, cColor(0, 1), _W("- %ls %g"),
                  cString::To16Char(mvLightComplexity[i].second->GetName()).c_str(), fComplexity);
            gpBase->mpGameDebugSet->DrawFont(
                  gpBase->mpDefaultFont, cVector3f(5, fY, 0), 14, vColor, _W("- %ls %g"), cString::To16Char(mvLightComplexity[i].second->GetName()).c_str(), fComplexity);
    ...
    if (mbShowPlayerInfo)
    {
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Position: %ls OnGround: %d Moved: %d Mass: %f ActiveSize: %d\n"),
              cString::To16Char(pCharBody->GetPosition().ToString()).c_str(), pCharBody->IsOnGround(), pCharBody->GetMovedLastUpdate(), pCharBody->GetMass(),
              pCharBody->GetActiveSize());
        fY += 15.0f;
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("MoveSpeed: %f, %f (%f) Mul: %f AvgSpeed: %f\n"),
              pCharBody->GetMoveSpeed(eCharDir_Forward), pCharBody->GetMoveSpeed(eCharDir_Right), pCharBody->GetVelocity(gpBase->mpEngine->GetStepSize()).Length(),
              pPlayer->GetInteractionMoveSpeedMul(), pPlayer->GetAvgSpeed());
        fY += 15.0f;

        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Climbing: %d\n"), pCharBody->IsClimbing());
        fY += 15.0f;
        ...
        if (pPlayer->GetCurrentMoveState() == eLuxMoveState_Normal)
        {
            ...
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Crouching: %d Running: %d MoveSpeedMul %f RunSpeedMul %f\n"),
                  pMoveNormal->IsCrouching(), pMoveNormal->IsRunning(), pMoveNormal->GetMoveSpeedMul(), pMoveNormal->GetRunSpeedMul());
            fY += 15.0f;
        }

        gpBase->mpGameDebugSet->DrawFont(
              gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("ForceVel: %ls\n"), cString::To16Char(pCharBody->GetForceVelocity().ToString()).c_str());
        fY += 15.0f;
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Health: %f Terror: %f LightLevel: %f\n"), pPlayer->GetHealth(),
              pPlayer->GetTerror(), pPlayer->GetHelperLightLevel()->GetNormalLightLevel());
        fY += 15.0f;

        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("Infection: %.2f InfectionLevel: %d Tinderboxes: %d"),
              pPlayer->GetInfection(), pPlayer->GetInfectionLevel(), pPlayer->GetTinderboxes());
        fY += 15.0f;
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("State: %d\n"), pPlayer->GetCurrentState());
        fY += 15.0f;
        ...
        if (pAnim)
        {
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("HandAnim: '%ls' Time: %f/%f\n"),
        ...
        if (moveState == eLuxMoveState_Normal)
        {
            ...
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("HeadBob: %f cos: %f sin: %f\n"), fBob, cos(fBob), sin(fBob));
            fY += 15.0f;
        }
    ...
    if (mbShowSoundPlaying)
    {
        ...
        if (pMusic)
        {
            ...
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                  _W("Music: '%ls' vol: %.2f playing: %d prio: %d elapsed: %.2f total time: %.2f %ls"), cString::To16Char(pChannel->GetData()->GetName()).c_str(),
                  pChannel->GetVolume(), pChannel->IsPlaying(), pChannel->GetPriority(), pChannel->GetElapsedTime(), pChannel->GetTotalTime(),
        ...
        for (int i = 0; i < (int)vSoundNames.size(); i++)
        {
            ...
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f((float)lCol * 250, fY + (float)lRow * 15, 10), 14, cColor(1, 1),
                  _W("%ls%ls%ls(%.2f)(%d) (%.2f/%.2f)"), cString::To16Char(vSoundNames[i]).c_str(), pChannel->GetData()->IsStream() ? _W("*st*") : _W(""),
                  pChannel->IsBufferUnderrun() ? _W("BUFFER UNDERRUN!!") : _W(""), pChannel->GetVolume(), pChannel->GetPriority(), pChannel->GetElapsedTime(),
    ...
    if (mbInspectionMode && mpInspectMeshEntity)
    {
        ...
        for (int i = 0; i < eMaterialTexture_LastEnum; ++i)
        {
            ...
            pSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W(" %d size: %d x %d x %d mem: %.2fmb type: %ls file: '%ls'"), i, pTex->GetSize().x,
                  pTex->GetSize().y, pTex->GetSize().z, fMemSize, cString::To16Char(PixelFormatToString(pTex->GetPixelFormat())).c_str(), pTex->GetFullPath().c_str());
            fY += 15;
        }
    }
    ...
    if (mbShowAILog)
    {
        ...
        {
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), cString::To16Char(*aiLogIt).c_str());
...
void cLuxDebugHandler::OutputContainerContentsRec(iRenderableContainerNode* apNode, int alLevel)
{
    ...
    if (apNode->HasObjects())
    {
        ...
        for (; it != apNode->GetObjectList()->end(); ++it)
        {
            ...
            Log("%s %s (%s) AABB: (%s)-(%s)\n", GetTab(alLevel).c_str(), pObject->GetName().c_str(), pObject->GetEntityType().c_str(), pBV->GetMin().ToString().c_str(),
                  pBV->GetMax().ToString().c_str());
        }
    }
    ...
    if (apNode->HasObjects())
    {
        ...
        for (; it != apNode->GetObjectList()->end(); ++it)
        {
            ...
            while (pCheckNode && pCheckNode->GetParent())
            {
                ...
                {
                    Log(" Object: '%s' is outside node %d AABB Amount: %s!\n", pObject->GetName().c_str(), lLevel,
                          GetOutSideAmount(pObject, pCheckNode->GetMin(), pCheckNode->GetMax()).ToString().c_str());
                    break;
                }
                ++lLevel;
...
bool cLuxDebugHandler::ChangeDebugText(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    else if (lNum == 15)
    {
        ...
        cRendererDeferred::SetDebugRenderLightComplexity(bActive);
    else if (lNum == 31)
        cRendererDeferred::SetDebugRenderOverdraw(bActive);
    /*else if(lNum == 26)
    {
        ...
        mbPositionAttachedProps = bActive;
    else if (lNum == 17)
    {
