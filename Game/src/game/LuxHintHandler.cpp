...
#if USE_SDL2
    mpGampadAxisIcon[eGamepadAxis_LeftTrigger * 2 + eGamepadAxisRange_Positive - 1] = pGui->CreateGfxTexture(
          "gamepad_lt.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGampadAxisIcon[eGamepadAxis_RightTrigger * 2 + eGamepadAxisRange_Positive - 1] = pGui->CreateGfxTexture(
          "gamepad_rt.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
#else
...
void cLuxHintHandler::DrawHintText(float afFrameTime, cGuiSet* apGuiSet)
{
    ...
    apGuiSet->DrawFont(mpFont, cVector3f(400, mfYPos, 20), mvFontSize, hintCol, eFontAlign_Center, eGuiMaterial_FontNormal, _W(" %ls"), kTranslate("Hints", "HINT:").c_str());
    ...
    if (vRows.size() == 1)
    {
        apGuiSet->DrawFont(mpFont, cVector3f(400, fStartY, 20), mvFontSize, cColor(1, mfAlpha), eFontAlign_Center, eGuiMaterial_FontNormal, _W(" %ls"), msCurrentText.c_str());
    }
    else
    {
        for (size_t i = 0; i < vRows.size(); ++i)
        {
            apGuiSet->DrawFont(mpFont, cVector3f(400 - fMaxWidth * 0.5f, fStartY + (float)i * (mvFontSize.y + 2.0f), 20), mvFontSize, cColor(1, mfAlpha), eFontAlign_Left,
                  eGuiMaterial_FontNormal, _W(" %ls"), vRows[i].c_str());
        }
    }
    ...
    if (mvHintIcons.size() > 0)
    {
        ...
        for (size_t i = 0; i < vRows.size(); ++i)
        {
            ...
            while (sRowText[lCount] != 0)
            {
                ...
                if (lGlyphNum < mpFont->GetFirstChar() || lGlyphNum > mpFont->GetLastChar())
                {
                    lCount++;
                    continue;
                }
    ...
    if (vInputParts[0] == "GamepadButton")
    {
        ...
        if (button < eGamepadButton_LastEnum)
        {
            if (mpGamepadButtonIcon[button])
            {
                ...
                if (button == eGamepadButton_4 || button == eGamepadButton_5)
                    return _W("       ");
                return _W("   ");
            }
            else
