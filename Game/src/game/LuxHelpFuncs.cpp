...
bool cLuxHelpFuncs::PlayGuiSoundData(const tString& asName,
      eSoundEntryType aDestType,
      float afVolMul,
      eSoundEntityType aSoundType,
...
tWString cLuxHelpFuncs::ParseStringCommand(const tWString& asCommand)
{
    ...
    if (sPrefix == _W("But"))
    {
        ...
        for (size_t i = 0; i < pAction->GetSubActionNum(); ++i)
        {
            ...
            ////////////
            //
            if (sType == "GamepadButton" || sType == "GamepadAxis" || sType == "GamepadHat")
            {
                tWString sConverted;
