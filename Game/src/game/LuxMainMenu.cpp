    ...
    : iLuxUpdateable("LuxDebugHandler")
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        mpBlurProgram[i] = mpGraphics->CreateGpuProgramFromShaders(
              "MainMenuBlur" + cString::ToString(i), "mainmenu_screen_blur_vtx.glsl", "mainmenu_screen_blur_frag.glsl", &programVars);
    }
...
void cLuxMainMenu::OnDraw(float afFrameTime)
{
    ...
    if (mpScreenGfx)
    {
        ...
        mpGuiSet->DrawGfx(mpTopBackground, cVector3f(0, vPos.y, 0.5f), cVector2f(mvScreenSize.x, mvScreenSize.y - vPos.y), cColor(1, 0.5f * mfTopMenuAlpha));
    }
    /////////////////////////////////
    // 3D background
    else
...
void cLuxMainMenu::ExitMenu(eLuxMainMenuExit aMessage)
{
    ...
    if (aMessage == eLuxMainMenuExit_ContinueGame || aMessage == eLuxMainMenuExit_StartGame)
    {
        if (msZoomSound != "")
...
void cLuxMainMenu::UpdateBase(float afTimeStep)
{
    ...
    if (mbExiting)
    {
        ...
        if (mExitMessage == eLuxMainMenuExit_StartGame || mExitMessage == eLuxMainMenuExit_ContinueGame)
        {
            bZoomCamera = true;
...
void cLuxMainMenu::CreateTopMenuGui()
{
    ...
    ///////////////
    // Continue
    if (gpBase->mbPTestActivated == false && gpBase->mpMapHandler->MapIsLoaded() == false && gpBase->mpSaveHandler->SaveFileExists())
    {
    ...
    ///////////////
    // Load game
    if (gpBase->mbPTestActivated == false && gpBase->mpSaveHandler->SaveFileExists() && gpBase->mpMapHandler->MapIsLoaded() == false)
    {
...
bool cLuxMainMenu::PressContinue(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    cGuiPopUpMessageBox* pPopUp = mpGuiSet->CreatePopUpMessageBox(
          _W(""), kTranslate("MainMenu", "Continue old game?"), kTranslate("MainMenu", "Yes"), kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedContinuePopup));
      ...
      bool cLuxMainMenu::PressStartGame(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox* pPopUp = mpGuiSet->CreatePopUpMessageBox(
          _W(""), kTranslate("MainMenu", "Start a new game?"), kTranslate("MainMenu", "Yes"), kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedStartGamePopup));
...
bool cLuxMainMenu::PressExit(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox* pPopUp = mpGuiSet->CreatePopUpMessageBox(
          _W(""), kTranslate("MainMenu", "Sure you want to quit?"), kTranslate("MainMenu", "Yes"), kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedExitPopup));
    pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
    pPopUp->SetKillOnEscapeKey(false);
...
bool cLuxMainMenu::PressExitToMainMenu(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox* pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "Sure you want to exit to main menu?"), kTranslate("MainMenu", "Yes"),
          kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedExitToMainMenuPopup));
    pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
    pPopUp->SetKillOnEscapeKey(false);
...
bool cLuxMainMenu::PressExitAndSave(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox* pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "Sure you want to exit and save?"), kTranslate("MainMenu", "Yes"),
          kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedExitAndSavePopup));
    pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
    pPopUp->SetKillOnEscapeKey(false);
