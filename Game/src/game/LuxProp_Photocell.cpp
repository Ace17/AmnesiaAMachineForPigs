...
cLuxPropLoader_Photocell::cLuxPropLoader_Photocell(const tString& asName)
...
cLuxProp_Photocell::cLuxProp_Photocell(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Photocell)
{
    mfLightLevel = 0;
...
void cLuxProp_Photocell::UpdateLightLevel(float afTimeStep)
{
    ...
    cVector3f vSamplePos[6] = { pBV->GetWorldCenter() + cVector3f(vAxisAdd.x, 0, 0), pBV->GetWorldCenter() - cVector3f(vAxisAdd.x, 0, 0),
        pBV->GetWorldCenter() + cVector3f(0, vAxisAdd.y, 0), pBV->GetWorldCenter() - cVector3f(0, vAxisAdd.y, 0), pBV->GetWorldCenter() + cVector3f(0, 0, vAxisAdd.z),
