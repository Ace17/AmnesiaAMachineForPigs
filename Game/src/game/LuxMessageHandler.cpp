...
cLuxMessageHandler::cLuxMessageHandler()
    : iLuxUpdateable("LuxMusicHandler")
{
...
void cLuxMessageHandler::DrawPauseMessage()
{
    ...
    if (mbMessageYesNo)
    {
        ...
        gpBase->mpGameHudSet->DrawFont(
              mpFont, vTextPos, mvFontSize - cVector2f(1), cColor(1, mfPauseMessageAlpha), _W("(%ls)"), gpBase->mpInputHandler->GetInputName("Interact").c_str());
        ...
        vTextPos.x += 80;
        gpBase->mpGameHudSet->DrawFont(
              mpFont, vTextPos, mvFontSize - cVector2f(1), cColor(1, mfPauseMessageAlpha), _W("(%ls)"), gpBase->mpInputHandler->GetInputName("Attack").c_str());
    }
}
