...
cLuxPlayerStamina::cLuxPlayerStamina(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerStamina")
{
    mfStaminaTimer = 0.0f;
...
cLuxPlayerVoiceFlashback::cLuxPlayerVoiceFlashback(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerVoiceFlashback")
{
    mbIsDelaying = false;
    mbIsPlaying = false;
    mbUseEffects = false;
...
void cLuxPlayerVoiceFlashback::Start(const tString& asVoiceFile,
      const tString& asTextEntry1,
      float afDelay1,
      const tString& asTextEntry2,
      float afDelay2,
      const tString& asTextEntry3,
      float afDelay3,
      const tString& asTextEntry4,
      float afDelay4,
      const tString& asTextEntry5,
      float afDelay5,
      const tString& asTextEntry6,
      float afDelay6,
      const tString& asTextEntry7,
      float afDelay7,
      bool abUseEffects,
      bool abObstructMovement)
{
    ...
    gpBase->mpEffectHandler->GetPlayVoice()->AddMultiSubbedVoice(asVoiceFile, "", "Voice", asTextEntry1, afDelay1, asTextEntry2, afDelay2, asTextEntry3, afDelay3, asTextEntry4,
          afDelay4, asTextEntry5, afDelay5, asTextEntry6, afDelay6, asTextEntry7, afDelay7, false, cVector3f(), 0, 0);
...
cLuxPlayerInfectionCollapse::cLuxPlayerInfectionCollapse(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerInfectionCollapse")
{
...
cLuxPlayerCamDirEffects::cLuxPlayerCamDirEffects(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerCamDirEffects")
{
...
cLuxPlayerSpawnPS::cLuxPlayerSpawnPS(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerSpawnPS")
{
}
...
void cLuxPlayerSpawnPS::GenerateAllSpawnPos()
{
    ...
    for (; vLocalPos.z < mfRadius; vLocalPos.z += vStep.z)
    {
        ...
        for (; vLocalPos.x < mfRadius; vLocalPos.x += vStep.x)
        {
            ...
            cVector3f vFinalLocalPos = vLocalPos + cMath::RandRectVector3f(cVector3f(-fRandRange, 0, -fRandRange), cVector3f(fRandRange, 0, fRandRange));
...
cLuxPlayerHurt::cLuxPlayerHurt(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerHurt")
{
...
cLuxPlayerFlashback::cLuxPlayerFlashback(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerFlashback")
{
...
void cLuxPlayerFlashback::Start(const tString& asFlashbackFile, const tString& asCallback)
{
    ...
    // World sounds
    gpBase->mpEngine->GetSound()->GetSoundHandler()->FadeGlobalVolume(
          mfWorldSoundVolume, (1 - mfWorldSoundVolume) / 1.5f, eSoundEntryType_World, eLuxGlobalVolumeType_Flashback, false);
...
cLuxPlayerLookAt::cLuxPlayerLookAt(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerLookAt")
{
    Reset();
}
...
cLuxPlayerInfection::cLuxPlayerInfection(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerInfection")
{
...
void cLuxPlayerInfection::UpdateCheckEnemySeen(float afTimeStep)
{
    ...
    while (entIt.HasNext())
    {
        ...
        if (!pCritter->CausesInfectionIncrease())
            continue;
    ...
    while (it.HasNext())
    {
        ...
        if (pEnemy->IsActive() == false)
            continue;
        if (pEnemy->CausesInfectionIncrease() == false)
            continue;
...
void cLuxPlayerInfection::UpdateInfectionIncreaseSounds(float afTimeStep)
{
    ...
    if (mfInfectionIncreaseVolume > 1)
        mfInfectionIncreaseVolume = 1;
...
{
    if (mpPlayer->GetInfection() < mfInfectionVeryHighLimit)
        return;
...
cLuxPlayerLantern::cLuxPlayerLantern(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerLantern")
{
...
cLuxPlayerDeath::cLuxPlayerDeath(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "PlayerDeath")
{
    cGui* pGui = gpBase->mpEngine->GetGui();
...
void cLuxPlayerDeath::OnDraw(float afFrameTime)
{
    ...
    // gpBase->mpGameHudSet->DrawFont(kTranslate("Game", "DeathPress"),mpFont, cVector3f(400, 550, 6), 17, cColor(1,mFlashOscill.val,mFlashOscill.val,
    // mfTextAlpha2),eFontAlign_Center);
}
...
cLuxPlayerLean::cLuxPlayerLean(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "PlayerLean")
{
    mfMaxMovement = 0.5f;
...
cLuxPlayerHudEffect::cLuxPlayerHudEffect(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerHudEffect")
{
    cGui* pGui = gpBase->mpEngine->GetGui();
...
cLuxPlayerLightLevel::cLuxPlayerLightLevel(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerLightLevel")
{
...
cLuxPlayerIsMoving::cLuxPlayerIsMoving(cLuxPlayer* apPlayer)
    : iLuxPlayerHelper(apPlayer, "LuxPlayerIsMoving")
{
