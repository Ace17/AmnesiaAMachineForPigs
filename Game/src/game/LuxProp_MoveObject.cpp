...
cLuxPropLoader_MoveObject::cLuxPropLoader_MoveObject(const tString& asName)
...
cLuxProp_MoveObject::cLuxProp_MoveObject(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_MoveObject)
{
...
void cLuxProp_MoveObject::MoveToState(float afState, float afAcc, float afMaxSpeed, float afSlowdownDist, bool abResetSpeed)
{
    ...
    if (mMoveObjectType == eLuxMoveObjectType_Linear)
    {
        cVector3f vWanted = m_mtxClosedTransform.GetTranslation() * (1 - afState) + m_mtxOpenTransform.GetTranslation() * afState;
