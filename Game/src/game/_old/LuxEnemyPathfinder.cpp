...
cAINode* cLuxEnemyPathfinder::GetNodeAtPos(const cVector3f& avPos,
      float afMinDistance,
      float afMaxDistance,
      bool abGetClosest,
      bool abPosToNodeFreePathCheck,
      bool abEnemyToNodeFreePathCheck,
      cAINode* apSkipNode,
      int alFreePathRayNum,
      tAIFreePathFlag alFreePathFlags,
      bool abSkipUsedNodes)
{
    ...
    while (startNodeIt.HasNext())
    {
        ...
        if (abPosToNodeFreePathCheck && mpNodeContainer->FreePath(avPos, pNode->GetPosition(), alFreePathRayNum, alFreePathRayNum) == false)
        {
        ...
        ///////////////////////////////////
        // Check if there is a free path from pos to node
        if (abEnemyToNodeFreePathCheck && mpNodeContainer->FreePath(mpEnemy->mpCharBody->GetPosition(), pNode->GetPosition(), alFreePathRayNum, alFreePathRayNum) == false)
        {
