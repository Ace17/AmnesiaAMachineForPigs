...
{
    kSerializableClassInit(cLuxStateMessage_SaveData) public : int mlType;
    float mfTime;
    float mfCount;
...
{
    kSerializableClassInit(cLuxEnemyMover_SaveData) public : void FromMover(cLuxEnemyMover* apMover);
...
{
    kSerializableClassInit(cLuxEnemyPathfinder_SaveData) public : void FromPathfinder(cLuxEnemyPathfinder* apPathfinder);
...
{
    kSerializableClassInit(cLuxEnemyPatrolNode_SaveData) public : int mlNodeId;
    float mfWaitTime;
    tString msAnimation;
    bool mbLoopAnimation;
};
...
{
    kSerializableClassInit(iLuxEnemy_SaveData) public : virtual ~iLuxEnemy_SaveData();
...
enum eLuxEnemyState
{
    ...
    eLuxEnemyState_PatrolAnimation,
...
public:
    cEnemyAttackDamageData()
        ...
        , mlStrength(0)
        , mfHitSpeed(3)
        , mDamageType(eLuxDamageType_BloodSplat)
        , mWeaponHitType(eLuxWeaponHitType_Sword)
        , mbCheckPlayer(true)
        , mbCheckProps(true)
        , msHitSound("")
    {
...
public:
    ...
    void PlayAnim(const tString& asName,
          bool abLoop,
          float afFadeTime,
          bool abDependsOnSpeed = false,
    ...
    virtual bool CanSeePlayer() { return mbCanSeePlayer; }
    ...
    bool IsInvisible() { return mbInvisible; }
...
protected:
    ...
    void Attack(const cEnemyAttackSizeData& aSizeData, const cEnemyAttackDamageData& aDamageData);
