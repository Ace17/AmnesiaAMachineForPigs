...
void cLuxEnemy_ManPig::AILog(const tString& logMessage)
{
    ...
    sprintf(buff, "%2d:%2d.%2d %s.%c (%s): %s\n", now->tm_hour, now->tm_min, now->tm_sec, GetName().c_str(), this->mCurrentPose == eLuxEnemyPoseType_Quadruped ? 'q' : 'b',
          cString::To8Char(gsLuxEnemyStates[mCurrentState]).c_str(), logMessage.c_str());
    Log("*AI* %s", buff);
...
bool cLuxEnemy_ManPig::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage* apMessage)
{
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected)
          ...
          kLuxState(eLuxEnemyState_GoHome) kLuxOnEnter
    {
    ...
    ///////////////////////
    // Leave
    kLuxOnLeave { ChangeMoveType(eLuxEnemyMoveType_Normal); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
    {
        if (IsInPlayerFovAtFeetPos(mpCharBody->GetFeetPosition()) && // :MAARTEN: does this not risk the player seeing a teleporting pig?
              IsVisibleToPlayerAtFeetPos(mpCharBody->GetFeetPosition()) == false && DistToPlayer2D() > 6.0f)
        {
    ...
    kLuxOnLeave { FinishPatrolEndOfPath(false); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_BreakDoor) kLuxOnEnter AILOG("stopping pathfinder, playing breakdoor anim.");
    mpPathfinder->Stop();
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) if (PlayerIsDetected())
    {
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) if (mPreviousState == eLuxEnemyState_Hurt)
    {
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) if (mlTempVal == 1)
    {
        AILOG("received eLuxEnemyMessage_AnimationSpecialEvent and tempval == 1, doing damage.");

        Attack(mNormalAttackSize, mNormalAttackDamage);
    }
    else { AILOG("received eLuxEnemyMessage_AnimationSpecialEvent and tempval != 1, ignoring."); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) if (cMath::RandRectl(0, 1) == 0 && DistToPlayer() < mfNormalAttackDistance * 1.3f)
    {
...
void cLuxEnemy_ManPig::FinishPatrolEndOfPath(bool callPatrolUpdateNow)
{
    ...
    if (!mbPathReversed && IsAtLastPatrolNode() || mbPathReversed && IsAtFirstPatrolNode())
    {
        if (CheckEnemyAutoRemoval(mfAutoRemoveMinPlayerDistance))
        {
            AILOG("FinishPatrolEndOfPath, at last patrol node, not seen by player, distance to player > AutoRemoveMinPlayerDistance and AutoRemoveAtPathEnd is set, so "
                  "deactivating & calling OnAutoDisabled.");
            SetActive(false);
...
bool cLuxEnemy_ManPig::TrackTeleportBehindPlayer()
{
    ...
    for (int i = 0; i < lNumOfTries; ++i)
    {
        /////////////////////////////////////
        // Find a node to run to
        cAINode* pNode = mpPathfinder->GetNodeAtPos(GetPlayerFeetPos() + vPlayerBackward * fDistance, 0, fDistance * 0.5f, false, false, true, NULL);
        if (pNode == NULL)
        {
            return false;
        }
