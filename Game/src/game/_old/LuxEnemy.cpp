...
tWString gsLuxEnemyStates[eLuxEnemyState_LastEnum + 1] = { _W("Idle"), _W("GoHome"),

    _W("Wait"), _W("Patrol"), _W("Investigate"), _W("Alert"), _W("Search"), _W("Eat"),

    _W("Hurt"),

    _W("Hunt"), _W("HuntPause"), _W("HuntWander"),

    _W("Flee"), _W("Stalk"), _W("Track"),

    _W("AttackMeleeShort"), _W("AttackMeleeLong"), _W("AttackRange"), _W("BreakDoor"),
...
static tWString gsPoseStates[] = { _W("Biped"), _W("Quadruped"), _W("NULL") };

static tWString gsMoveStates[] = { _W("Backward"), _W("Stopped"), _W("Walking"), _W("Jogging"), _W("Running"),
...
iLuxEnemyLoader::iLuxEnemyLoader(const tString& asName)
...
void iLuxEnemyLoader::AfterLoad(cXmlElement* apRootElem, const cMatrixf& a_mtxTransform, cWorld* apWorld, cResourceVarsObject* apInstanceVars)
{
    ...
    if (apInstanceVars)
    {
        ...
        pEnemy->mfHallucinationEndDist = apInstanceVars->GetVarFloat("HallucinationEndDist", 0.0f);
        pEnemy->mbBlind = apInstanceVars->GetVarBool("Blind", false);
        pEnemy->mbInvisible = apInstanceVars->GetVarBool("Invisible", false);
...
iLuxEnemy::iLuxEnemy(const tString& asName, int alID, cLuxMap* apMap, eLuxEnemyType aEnemyType)
    : iLuxEntity(asName, alID, apMap, eLuxEntityType_Enemy)
{
...
void iLuxEnemy::SetupAfterLoad(cWorld* apWorld)
{
    ...
    if (mpMeshEntity)
        mpMeshEntity->SetUpdateAlwaysAnimation(true);
...
void iLuxEnemy::SendMessage(eLuxEnemyMessage aType, float afTime, bool abLocalScope, const cVector3f& avX, float afX, int alX)
{
    if (mbDisabled)
        return;
    if (TriggersDisabled() && aType > eLuxEnemyMessage_EndOfPath && aType != eLuxEnemyMessage_PlayerInRange && aType != eLuxEnemyMessage_PlayerOutOfRange &&
...
void iLuxEnemy::PlayAnim(const tString& asName,
      bool abLoop,
      float afFadeTime,
      bool abDependsOnSpeed,
      float afSpeedMul,
      ...
      bool abPlayTransition)
{
    ...
    if (mpCurrentAnimation != NULL && mpCurrentAnimation->GetName() == asName && mpCurrentAnimation->IsActive() && mpCurrentAnimation->IsOver() == false)
    {
        return;
    }
    ...
    if (abPlayTransition)
    {
        ...
        if (mlNextAnimationIndex >= 0)
            lPrevIndex = mlNextAnimationIndex;
    ...
    if (mpCurrentAnimation && mpCurrentAnimation != pNewAnim)
    {
        ...
        else
        {
            ...
            if (pNewAnim->IsFading() == false || abLoop == false)
...
void iLuxEnemy::FadeOutCurrentAnim(float afFadeTime)
{
    if (mpCurrentAnimation == NULL)
        return;
    if (mpCurrentAnimation->IsLooping() || (mlNextAnimationIndex >= 0 && mbNextAnimationLoop))
        return;
...
float iLuxEnemy::DrawDebug(cGuiSet* apSet, iFontData* apFont, float afStartY)
{
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  State: '%ls' MoveState: '%ls' Pose: '%ls'"), gsLuxEnemyStates[mCurrentState].c_str(),
          sMoveState.c_str(), gsPoseStates[mCurrentPose].c_str());
    afStartY += 14;
    ...
    apSet->DrawFont(
          apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  Anim: '%ls' pos: %0.2f / %0.2f Speed: %0.2f Loop: %d"), sAnim.c_str(), fPos, fMaxPos, fSpeed, bLoop);
    afStartY += 14;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  PlayerSeen: %d PlayerDetected: %d PlayerInRange: %d StuckAtDoor: %d FOVMul: %f"),
          mbCanSeePlayer ? 1 : 0, mbPlayerDetected ? 1 : 0, mbPlayerInRange ? 1 : 0, mbStuckAtDoor ? 1 : 0, mfFOVMul);
    afStartY += 14;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  Health: %f FOVMul: %f DistFromStart: %f StuckCounter: %f Speed: %f SpeedPercent: %f Vel: %f"),
          mfHealth, mfFOVMul, cMath::Vector3Dist(mpCharBody->GetFeetPosition(), mvStartPosition), mpMover->GetStuckCounter(), mpMover->GetMoveSpeed(),
          mpMover->GetWantedSpeedAmount());
    afStartY += 14;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("  DistToPlayer: %f PlayerDirAmount: %f CurrentPatrolNode: %d MoverSpeedMul: %f"), DistToPlayer(),
          GetPlayerMovementTowardEnemyAmount(), mlCurrentPatrolNode, mpMover->CalculateSpeedMul(1.0f / 60.0f));
    afStartY += 14;
...
void iLuxEnemy::AddTransitionAnimation(const tString& asMainAnim, const tString& asTransAnim, const tString& asPrevAnim, float afMinTime, float afMaxTime)
{
    ...
    if (pMainAnim == NULL)
    {
        Error("'%s': Could not find animation '%s' to add a transitional animation to.\n", msName.c_str(), asMainAnim.c_str());
        return;
    }
    ...
    if (lTransAnim < 0)
    {
        Error("'%s': Could not find animation '%s' to be used as a transitional animation\n", msName.c_str(), asTransAnim.c_str());
        return;
    }
    ...
    {
        Error("'%s': Could not find animation '%s' to be used as a previous transitional animation\n", msName.c_str(), asPrevAnim.c_str());
        return;
    }
...
void iLuxEnemy::UpdateAnimation(float afTimeStep)
{
    ...
    if (mpMover->GetOverideMoveState() && mpCurrentAnimation->GetPreviousTimePosition() <= mpCurrentAnimation->GetSpecialEventTime() &&
    ...
    if (mpCurrentAnimation->IsOver())
    {
        ...
        if (mlNextAnimationIndex >= 0)
        {
            ...
            if (pAnim)
            {
                PlayAnim(pAnim->GetName(), mbNextAnimationLoop, 0.3f, mbNextAnimationDependsOnSpeed, mfNextAnimSpeedMul, false, mbNextAnimOverideMoveState,
                      mbNextAnimUseMoveAnimWhenCurrentIsOver, pAnim->CanBlend(), false);
            }
            else
            {
...
void iLuxEnemy::UpdateCanSeePlayer(float afTimeStep)
{
    ...
    float fMinDist = mpCharBody->GetCurrentBody()->GetBoundingVolume()->GetRadius() + pPlayerBody->GetCurrentBody()->GetBoundingVolume()->GetRadius() + 0.05f;
...
void iLuxEnemy::Attack(const cEnemyAttackSizeData& aSizeData, const cEnemyAttackDamageData& aDamageData)
{
    bool bHit = gpBase->mpMapHelper->ShapeDamage(GetAttackShape(aSizeData.mlShapeIdx), GetDamageShapeMatrix(aSizeData.mvOffset), mpCharBody->GetPosition(), aDamageData.mfMinDamage,
          aDamageData.mfMaxDamage, aDamageData.mfForce, aDamageData.mfMaxImpulse, aDamageData.mlStrength, aDamageData.mfHitSpeed, aDamageData.mDamageType,
          aDamageData.mWeaponHitType, false, aDamageData.mbCheckPlayer, aDamageData.mbCheckProps, true);
    if (bHit)
    {
        mlAttackHitCounter++;
...
static const cVector2f gvPosAdds[] = { cVector2f(0, 0), cVector2f(0.75, 0), cVector2f(-0.75, 0), cVector2f(0, 1), cVector2f(0, -1), cVector2f(0.5, 0.5), cVector2f(-0.5, 0.5),
    cVector2f(0.25, 0), cVector2f(-0.25, 0) };
...
{
    cVector3f vPos = mpCharBody->GetPosition() + mpCharBody->GetRight() * avOffset.x + mpCharBody->GetUp() * avOffset.y + mpCharBody->GetForward() * avOffset.z;
...
kSerializeClassContainer(mvPatrolNodes, cLuxEnemyPatrolNode_SaveData, eSerializeType_Class) kSerializeVar(mlCurrentPatrolNode, eSerializeType_Int32)

kSerializeVar(mMeshEntity, eSerializeType_Class)
kSerializeClassContainer(mvPS, cEnginePS_SaveData, eSerializeType_Class) kSerializeClassContainer(mvBeams, cEngineBeam_SaveData, eSerializeType_Class)
      kSerializeClassContainer(mvLights, cEngineLight_SaveData, eSerializeType_Class) kSerializeClassContainer(mvBillboards, cEngineBillboard_SaveData, eSerializeType_Class)
