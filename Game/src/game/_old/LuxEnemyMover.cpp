...
cLuxEnemyMover::cLuxEnemyMover(iLuxEnemy* apEnemy, iCharacterBody* apCharBody)
{
    ...
    if (mvPrecalcSampleDirs.empty())
    {
        ...
        while (1)
        {
            ...
            if (vPartionCount[lPart] == lMaxPartCount)
                continue;
            ...
            if (lPartionsFull == (int)vPartionCount.size())
                break;
        }
...
void cLuxEnemyMover::TurnAwayFromPos(const cVector3f& avFeetPos)
{
    ...
    float fGoalAngle = -cMath::GetAngleFromPoints2D(cVector2f(vStartPos.x, vStartPos.z), cVector2f(avFeetPos.x, avFeetPos.z));
...
{
    bool bMoving = fabs(mpCharBody->GetMoveSpeed(eCharDir_Forward)) > 0.001f || fabs(mpCharBody->GetMoveSpeed(eCharDir_Right)) > 0.001f;
    ...
    if (mbWallAvoidActive && bMoving)
    {
        ...
        for (size_t i = 0; i < mvSampleRays.size(); ++i)
        {
            ...
            apFunctions->GetLowLevelGfx()->DrawLine(mpCharBody->GetPosition(), mpCharBody->GetPosition() + mvSampleRays[i] * mfWallAvoidRadius, col);
        }
...
void cLuxEnemyMover::ForceMoveState(eLuxEnemyMoveState aMoveState)
{
    ...
    case eLuxEnemyMoveState_Walking:
    {
        ...
        mpEnemy->PlayAnim(aMoveState == eLuxEnemyMoveState_Running ? mpEnemy->GetRunAnimationName() : mpEnemy->GetWalkAnimationName(), true, 0.0f, true,
              mpEnemy->mfMoveSpeedAnimMul, false, false);
        break;
    }
    }
...
void cLuxEnemyMover::UpdateStepEffects(float afTimeStep)
{
    ...
    for (int i = 0; i < pAnim->GetEventNum(); ++i)
    {
        ...
        if (pEvent->mfTime >= pAnim->GetPreviousTimePosition() && pEvent->mfTime < pAnim->GetTimePosition())
        {
            bStep = true;
            break;
        }
...
void cLuxEnemyMover::UpdateWallAvoidance(float afTimeStep)
{
    ...
    if (mbWallAvoidActive == false)
        return;

    ///////////////////////////////////////
    // Check if character is moving
    if (fabs(mpCharBody->GetMoveSpeed(eCharDir_Forward)) < 0.001f && fabs(mpCharBody->GetMoveSpeed(eCharDir_Right)) < 0.001f)
    {
        return;
    }
        ...
        for (size_t i = 0; i < mvSampleRays.size(); ++i)
        {
            ...
            if (mvSampleRaysCollide[i] == false)
            {
                ...
                while (vParitionUsageCount[lPart] == mlSampleMaxPartCount)
                {
                    ...
                    if (lAngleIdx >= (int)mvPrecalcSampleDirs.size())
                        lAngleIdx -= (int)mvPrecalcSampleDirs.size();
    ...
    for (size_t i = 0; i < mvSampleRays.size(); ++i)
    {
        if (mvSampleRaysCollide[i])
        {
            ...
            if (mvSampleRaysAmount[i] > 1.0f)
                mvSampleRaysAmount[i] = 1.0f;
    ...
    if (fTotalHits > 0)
    {
        if (vTotalSteer.SqrLength() > 1)
            vTotalSteer.Normalize();
