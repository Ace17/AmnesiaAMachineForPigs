...
cLuxEnemyLoader_Tesla::cLuxEnemyLoader_Tesla(const tString& asName)
    : cLuxEnemyLoader_ManPig(asName)
{
}
...
cLuxEnemy_Tesla::cLuxEnemy_Tesla(const tString& asName, int alID, cLuxMap* apMap)
    : cLuxEnemy_ManPig(asName, alID, apMap)
{
    mfPulseTimer = 0.0f;
    mpMouthLight = NULL;
    mpMouthBone = NULL;
...
void cLuxEnemy_Tesla::UpdateEnemySpecific(float afTimeStep)
{
    ...
    {
        CreateSpotLight(0, 0, 10, true, kPif / 3.0f, mLightColor);
    }
...
void cLuxEnemy_Tesla::UpdateLightList(tLightList& alstAddedLights, tLightList& alstRemovedLights)
{
    ...
    gpBase->mpMapHelper->GetLightsAtNode(
          pWorld->GetRenderableContainer(eWorldContainerType_Static)->GetRoot(), lstIntersectingLights, GetCharacterBody()->GetPosition(), mfDarkeningRadius, true, true);
    gpBase->mpMapHelper->GetLightsAtNode(
          pWorld->GetRenderableContainer(eWorldContainerType_Dynamic)->GetRoot(), lstIntersectingLights, GetCharacterBody()->GetPosition(), mfDarkeningRadius, true, true);

    tLightList lstStrobeIntersectingLights;

    gpBase->mpMapHelper->GetLightsAtNode(
          pWorld->GetRenderableContainer(eWorldContainerType_Static)->GetRoot(), lstStrobeIntersectingLights, GetCharacterBody()->GetPosition(), mfStrobeRadius, true, false);
    gpBase->mpMapHelper->GetLightsAtNode(
          pWorld->GetRenderableContainer(eWorldContainerType_Dynamic)->GetRoot(), lstStrobeIntersectingLights, GetCharacterBody()->GetPosition(), mfStrobeRadius, true, false);
