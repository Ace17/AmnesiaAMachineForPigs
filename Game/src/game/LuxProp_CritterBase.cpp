...
iLuxPropLoader_Critter::iLuxPropLoader_Critter(const tString asName)
...
iLuxProp_CritterBase::iLuxProp_CritterBase(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Critter)
{
    mvVel = 0;
    mvGravityVel = 0;
    mlAnimState = 0;
...
bool iLuxProp_CritterBase::Attack(const cVector3f& avDir)
{
    ...
    bool bHit = gpBase->mpMapHelper->ShapeDamage(mpDamageShape, GetAttackMatrix(avDir), mpBody->GetLocalPosition(), mvAttackDamageMinMax.x, mvAttackDamageMinMax.y, mvAttackForce.x,
          mvAttackForce.y, mlAttackStrength, 3, eLuxDamageType_BloodSplat, eLuxWeaponHitType_Sword, false, true, false, false, NULL);
    if (bHit)
    {
