...
cLuxAreaLoader_Infection::cLuxAreaLoader_Infection(const tString& asName)
...
cLuxArea_Infection::cLuxArea_Infection(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxArea(asName, alID, apMap, eLuxAreaType_Infection)
{
...
void cLuxArea_Infection::UpdateInfection(float afTimeStep)
{
    if (mbCollidesWithPlayer)
    {
        ...
        if (mfIncrementSpeed < 0 && (currentInfection + fIncrementThisFrame) <= mfInfectionLevelLowerBound)
        {
            ...
            if (fIncrementThisFrame > 0.0f)
                fIncrementThisFrame = 0.0f; // If the area is meant to heal up to a certain level, it can't damage if the player isn't at that level yet
        ...
        else if (mfIncrementSpeed > 0 && (currentInfection + fIncrementThisFrame) >= mfInfectionLevelUpperBound)
        {
            ...
            if (fIncrementThisFrame < 0.0f)
                fIncrementThisFrame = 0.0f; // If the area is meant to damage up to a certain level, it can't heal if the player is at a higher level
