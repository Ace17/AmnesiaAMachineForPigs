...
void cLuxJournalStateData::OnDraw(float afFrameTime)
{
    ...
    ///////////////////////
    // Note open
    if (mState == eLuxJournalState_OpenNote || mState == eLuxJournalState_OpenHint || mState == eLuxJournalState_OpenDiary || mState == eLuxJournalState_OpenNarratedDiary)
    {
...
void cLuxJournal::ExitPressed(bool abInstantExit)
{
    ...
    ////////////////////
    // Narrated diary open
    if (mCurrentState == eLuxJournalState_OpenNarratedDiary && mlCurrentNotePage < (int)mvPages.size() - 1)
    {
...
{
    if (mvHints.empty() == false)
        OpenHint(mvHints.back());
...
void cLuxJournal::OpenLastReadText()
{
    ...
    {
        if (mlLastReadTextEntry >= (int)mvHints.size())
            return;
...
int cLuxJournal::GetNoteListIndex(eLuxJournalState aState)
{
    if (aState == eLuxJournalState_Notes)
        ...
        return 1;
    else if (aState == eLuxJournalState_Hints)
        return 3;
    else
        return 2;
}
...
void cLuxJournal::CreateHintsGui()
{
    ...
    else
    {
        ...
        for (int i = (int)mvHints.size() - 1; i >= 0; --i)
        {
            ...
            pListPage->mvEntries.back().AddWidget(pLabel);
            if (pImage)
                pListPage->mvEntries.back().AddIcon(pImage, i);
...
void cLuxJournal::CreateOpenNoteGui()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        eLuxJournalState state = i == 0 ? eLuxJournalState_OpenNote
                                        : (i == 1 ? eLuxJournalState_OpenDiary : (i == 2 ? eLuxJournalState_OpenHint : eLuxJournalState_OpenNarratedDiary));
...
bool cLuxJournal::NoteBackClick(iWidget* apWidget, const cGuiMessageData& aData)
{
    if (aData.mMessage == eGuiMessage_UIArrowPress)
    {
        if (aData.mlVal == eUIArrow_Down)
        {
            ...
            if (UIListenerButtonPress(apWidget, data))
                return true;
        }
        else
            return false;
...
bool cLuxJournal::NoteArrowClick(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    else if (apWidget->GetUserValue() == 2)
    {
        ...
        if (mCurrentState == eLuxJournalState_Hints)
            mlLastReadHint = mvHints.size() - mlCurrentNoteListPage[GetNoteListIndex(mCurrentState)] * 12 - 1;
        if (mCurrentState == eLuxJournalState_Notes)
            mlLastReadDocument = mlCurrentNoteListPage[GetNoteListIndex(mCurrentState)] * 12;
    }
    // Backward
    else if (apWidget->GetUserValue() == 3)
    {
...
{
    if (aData.mMessage == eGuiMessage_UIArrowPress && aData.mlVal != eUIArrow_Down)
        return false;
...
#ifdef USE_GAMEPAD
#if USE_SDL2
        if (gpBase->mpInputHandler->IsGamepadPresent() && gpBase->mpInputHandler->GetGamepad()->ButtonIsDown(eGamepadButton_DpadDown) == false)
            break;
#else
        if (gpBase->mpInputHandler->IsGamepadPresent() && gpBase->mpInputHandler->GetGamepad()->HatIsInState(eGamepadHat_0, eGamepadHatState_Down) == false)
            break;
#endif
#endif
        if (lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenDiary || lType == eLuxJournalState_OpenHint || lType == eLuxJournalState_OpenNarratedDiary)
        {
            ExitPressed(false);
            return true;
        }
...
bool cLuxJournal::UIListenerButtonPress(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if (!(lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenDiary || lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenHint ||
