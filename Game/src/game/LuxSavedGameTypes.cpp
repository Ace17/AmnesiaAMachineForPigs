...
void cLuxEffectHandler_SaveData::FromEffectHandler(cLuxEffectHandler* apEffects)
{
    ...
    {
        if (i == eLuxGlobalVolumeType_Commentary)
            continue;
...
void cLuxEffectHandler_SaveData::ToEffectHandler(cLuxMap* apMap, cLuxEffectHandler* apEffects)
{
    ...
    if (msVoice_Name != "")
    {
        ...
        {
            apEffects->GetPlayVoice()->mpVoiceEntry = apEffects->GetPlayVoice()->mpSoundHandler->Play(
                  msVoice_Name, false, 1.0f, mvVoice_Position, mfVoice_MinDistance, mfVoice_MaxDistance, eSoundEntryType_Gui, false, true, 0, true);
            if (apEffects->GetPlayVoice()->mpVoiceEntry)
            {
                apEffects->GetPlayVoice()->mlVoiceEntryID = apEffects->GetPlayVoice()->mpVoiceEntry->GetId();
                apEffects->GetPlayVoice()->mpVoiceEntry->GetSoundChannel()->SetElapsedTime(mfVoice_ElapsedTime);
            ...
            if (msEffect_Name != "")
            {
                apEffects->GetPlayVoice()->mpEffectEntry = apEffects->GetPlayVoice()->mpSoundHandler->Play(
                      msEffect_Name, false, 1.0f, mvVoice_Position, mfVoice_MinDistance, mfVoice_MaxDistance, eSoundEntryType_Gui, false, true, 0, true);
                if (apEffects->GetPlayVoice()->mpEffectEntry)
                {
                    apEffects->GetPlayVoice()->mlEffectEntryID = apEffects->GetPlayVoice()->mpEffectEntry->GetId();
                    apEffects->GetPlayVoice()->mpEffectEntry->GetSoundChannel()->SetElapsedTime(mfVoice_ElapsedTime);
...
kBeginSerializeBase(cLuxEffectHandler_SoundMul_SaveData) kSerializeVar(mbActive, eSerializeType_Bool)
...
kBeginSerializeBase(cLuxEffectHandler_SaveData) kSerializeVar(mbFade_Active, eSerializeType_Bool)
...
kSerializeClassContainer(mvVoiceData, cLuxVoiceData, eSerializeType_Class) kSerializeClassContainer(mvTextQueue, cTextQueueEntry, eSerializeType_Class)
...
kBeginSerializeBase(cLuxMusicHandler_Music_SaveData) kSerializeVar(msFile, eSerializeType_String)
...
kBeginSerializeBase(cLuxMusicHandler_SaveData) kSerializeVar(msCurrentMusic, eSerializeType_String)
...
kSerializeClassContainer(mvMusic, cLuxMusicHandler_Music_SaveData, eSerializeType_Class) kSerializeVarContainer(mvAttackerIDs, eSerializeType_Int32)
      kSerializeVarContainer(mvSearcherIDs, eSerializeType_Int32) kEndSerialize()
...
kBeginSerializeBase(cLuxJournal_DiaryCont_SaveData) kSerializeVar(msType, eSerializeType_String)
kSerializeClassContainer(mvDiaries, cLuxDiary, eSerializeType_Class) kEndSerialize()
...
kBeginSerializeBase(cLuxJournal_SaveData) kSerializeVar(mlLastReadTextCat, eSerializeType_Int32)
...
kSerializeClassContainer(mvNotes, cLuxNote, eSerializeType_Class) kSerializeClassContainer(mvDiaryConts, cLuxJournal_DiaryCont_SaveData, eSerializeType_Class)
      kSerializeClassContainer(mvQuestNotes, cLuxQuestNote, eSerializeType_Class) kSerializeClassContainer(mvHints, cLuxHint, eSerializeType_Class) kEndSerialize()
...
kBeginSerializeBase(cLuxInventory_Item_SaveData) kSerializeVar(msName, eSerializeType_String)
...
void cLuxInventory_SaveData::ToInventory(cLuxMap* apMap, cLuxInventory* apInventory)
{
    ...
    for (size_t i = 0; i < mvItems.Size(); ++i)
    {
        ...
        cLuxInventory_Item* pItem = apInventory->AddItem(
              saveItem.msName, (eLuxItemType)saveItem.mlType, saveItem.msSubType, saveItem.msImageFile, saveItem.mfAmount, saveItem.msVal, saveItem.msExtraVal);
...
kBeginSerializeBase(cLuxInventory_SaveData) kSerializeVar(mbDisabled, eSerializeType_Bool)
kSerializeVar(mlEquippedHandItem, eSerializeType_Int32)
kSerializeClassContainer(mvItems, cLuxInventory_Item_SaveData, eSerializeType_Class) kSerializeClassContainer(mvCombineCallbacks, cLuxCombineItemsCallback, eSerializeType_Class)
      kEndSerialize()
...
kBeginSerializeBase(cLuxPlayerHands_SaveData) kSerializeVar(mlState, eSerializeType_Int32)
...
kBeginSerializeBase(cLuxSaveGame_SaveData) kSerializeVar(msMapFolder, eSerializeType_String)
