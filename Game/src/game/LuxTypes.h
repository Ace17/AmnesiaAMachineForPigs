...
{
    kSerializableClassInit(cLuxIdPair) public : int mlParentId;
    int mlChildId;
};
...
{
public:
    iLuxUpdateable(const tString& asName)
        : iUpdateable(asName)
    {
    }
...
{
public:
    iLuxPlayerHelper(cLuxPlayer* apPlayer, const tString& asName)
        : mpPlayer(apPlayer)
        , iLuxUpdateable(asName)
    {
    }
...
{
    kSerializableClassInit(cLuxHeadPosAdd) public : cLuxHeadPosAdd()
        : mvAdd(0)
        , mbMoving(false)
    {
    }
...
{
    kSerializableClassInit(cLuxEventTimer) public : tString msName;
    tString msFunction;
    float mfCount;
    bool mbDestroyMe;
};
...
{
    kSerializableClassInit(cLuxCombineItemsCallback) public : tString msName;
    tString msItemA;
    tString msItemB;
    tString msFunction;
    bool mbAutoDestroy;
...
{
    kSerializableClassInit(cLuxUseItemCallback) public : tString msName;
    tString msItem;
    tString msEntity;
    tString msFunction;
    bool mbAutoDestroy;
...
{
    kSerializableClassInit(cLuxNote) public : tString msNameEntry;
    tString msTextEntry;
...
{
    kSerializableClassInit(cLuxHint) public : cLuxHint()
        : mbHasBeenRead(true)
    {
    }
...
{
    kSerializableClassInit(cLuxDiary) public : tString msNameEntry;
    tString msTextEntry;
...
{
    kSerializableClassInit(cLuxQuestNote) public : bool mbActive;
    tString msName;
    tString msNameEntry;
    tString msTextEntry;
};
...
{
    kSerializableClassInit(cLuxScriptVar) public : cLuxScriptVar() {}
    cLuxScriptVar(const tString& asName)
        : msName(asName)
        , msVal("")
    {
    }
...
{
    kSerializableClassInit(cTextQueueEntry) public : tWString msTextEntry;
    float mfDelay;
};
...
{
    kSerializableClassInit(cLuxVoiceData) public : tWString msText;
    float mfTextDelay;
    tWString msText2;
    float mfText2Delay;
    tWString msText3;
...
{
    kSerializableClassInit(cLuxCollideCallback_SaveData) public : void FromCallback(cLuxCollideCallback* apCallback);
...
{
    kSerializableClassInit(cLuxProp_AttachedProp) public : cLuxProp_AttachedProp()
        : mpProp(NULL)
    {
    }
...
public:
    cLuxNode_Pos(const tString& asName)
        : msName(asName)
    {
    }
