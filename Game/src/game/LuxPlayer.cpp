...
void cLuxPlayer::OnLeaveContainer(const tString& asNewContainer)
{
    ...
    if (mState == eLuxPlayerState_InteractGrab || mState == eLuxPlayerState_InteractSlide || mState == eLuxPlayerState_InteractPush || mState == eLuxPlayerState_InteractWheel ||
...
void cLuxPlayer::GiveDamage(float afAmount, int alStrength, eLuxDamageType aType, bool abSpinHead, bool abLethal)
{
    ...
    if (mState == eLuxPlayerState_Ladder)
        ChangeState(eLuxPlayerState_Normal);
...
{
    if (abPressed && (mbJumpDisabled || mpInfectionCollapse->IsActive()))
        return;
...
{
    if (abPressed && (mbCrouchDisabled || mpInfectionCollapse->IsActive()))
        return;
...
void cLuxPlayer::FadePitchTo(float afX, float afSpeedMul, float afMaxSpeed)
{
    ...
    {
        if (afX > fPitchLimitMax)
            afX = fPitchLimitMax;
        if (afX < fPitchLimitMin)
            afX = fPitchLimitMin;
    }
...
void cLuxPlayer::UpdateCamera(float afTimeStep)
{
    ...
    if (mfAspectMul != mfAspectMulGoal)
    {
        ...
        if (cMath::Abs(mfAspectMulGoal - mfAspectMul) < 0.004f)
    ...
    if (mfRoll != mfRollGoal)
    {
        ...
        if (cMath::Abs(mfRollGoal - mfRoll) < 0.004f)
            mfRoll = mfRollGoal;
    ...
    if (mbFadingPitch)
    {
        ...
        if (fSpeed > mfPitchMaxSpeed)
            fSpeed = mfPitchMaxSpeed;
        if (fSpeed < -mfPitchMaxSpeed)
            fSpeed = -mfPitchMaxSpeed;
    ...
    if (mfLeanRoll != mfLeanRollGoal)
    {
        ...
        if (cMath::Abs(mfLeanRollGoal - mfLeanRoll) < 0.004f)
...
void cLuxPlayer::UpdateAvgMoveDir(float afTimeStep)
{
    ...
    if (mlstPrevMoveDirs.size() > 12)
        mlstPrevMoveDirs.pop_front();
