...
void cLuxMoveState_Normal_Callback::OnGravityCollide(iCharacterBody* apCharBody, iPhysicsBody* apBody, cCollideData* apCollideData)
{
    ...
    if (apCollideData->mvContactPoints[0].mvNormal.y > 0.001f && apCharBody->GetForceVelocity().y < -mpMoveState->mfMinHitGroundBounceSpeed)
    {
...
cLuxMoveState_Normal::cLuxMoveState_Normal(cLuxPlayer* apPlayer)
    : iLuxMoveState(apPlayer)
{
    ////////////////////////////
    // Create data
...
void cLuxMoveState_Normal::OnDraw(float afFrameTime)
{
    ...
    gpBase->mpGameDebugSet->DrawFont(
          gpBase->mpDefaultFont, cVector3f(5, 70, 0), 12, cColor(1, 1), _W("Crouch: %d Running: %d JumpCount: %f\n"), mbCrouching, mbRunning, mfJumpCount);
    fY += 13;
    gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 0), 12, cColor(1, 1), _W("Bob: %ls (%f). Bounce: %f\n"),
          cString::To16Char(mvHeadBob.ToString()).c_str(), cMath::ToDeg(cMath::Wrap(mfHeadBobCount, 0, k2Pif)), mfHeadGroundBounce);
}
...
void cLuxMoveState_Normal::SetCrouch(bool abActive)
{
    ...
    {
        cVector3f vPosTestAdd[5] = { cVector3f(0, 0.001f, 0), cVector3f(0.01f, 0.001f, 0), cVector3f(-0.01f, 0.001f, 0), cVector3f(0, 0.001f, 0.01f),
            cVector3f(0, 0.001f, -0.01f) };
...
{
    float fMoveMul = mpPlayer->GetInteractionMoveSpeedMul() * mpPlayer->GetScriptMoveSpeedMul() * mpPlayer->GetEventMoveSpeedMul() * mpPlayer->GetHurtMoveSpeedMul() *
          mpPlayer->GetInfectionCollapseSpeedMul() * mpPlayer->GetStaminaSpeedMul() * mpPlayer->GetInfectionSpeedMul();
...
bool cLuxMoveState_Normal::UpdateLedgeCheck(float afTimeStep)
{
    ...
    cVector3f vStartPos = pCharBody->GetFeetPosition() + cVector3f(0, pCharBody->GetSize().y - mfClimbLedgeCheckDistFromTop, 0) +
...
void cLuxMoveState_Normal::FootSound(eLuxFootSound aType)
{
    ...
    pMap->BroadcastEnemySoundMessage(
          mpPlayer->GetCharacterBody()->GetFeetPosition() + cVector3f(0, 0.1f, 0), extraData.mfAIVolume, extraData.mfMinDistance, extraData.mfMaxDistance, sSoundDataName);
}
