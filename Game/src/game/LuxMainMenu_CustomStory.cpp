...
cLuxMainMenu_CustomStory::cLuxMainMenu_CustomStory(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin)
...
void cLuxMainMenu_CustomStory::CreateGui()
{
    ...
    for (int i = 0; i < (int)vLabels.size(); ++i)
    {
        ...
        float fLabelLength = pButton->GetDefaultFontType()->GetLength(pButton->GetDefaultFontSize(), sLabel.c_str());
...
cLuxMainMenu_CustomStoryList::cLuxMainMenu_CustomStoryList(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin, cLuxMainMenu_CustomStory* apWindow)
    : iLuxMainMenuWindow(apGuiSet, apGuiSkin)
{
    mvWindowSize = cVector2f(600, 440);
...
bool cLuxMainMenu_CustomStoryList::PressOK(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if (mpLBStories->GetSelectedItem() < 0)
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("CustomStory", "NoValidStory"), kTranslate("Global", "OK"), _W(""), NULL, NULL);
    else
