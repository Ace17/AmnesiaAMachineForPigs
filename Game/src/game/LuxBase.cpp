...
#define kCurrentVersion_Minor 30
...
static unsigned char gv_main_init_str[27] = { 0x4B, 0x4A, 0xC1, 0xA5, 0x8, 0x40, 0x3A, 0xA4, 0x5C, 0x4D, 0x5B, 0x5C, 0x77, 0x45, 0x49, 0x41, 0x46, 0x77, 0x41, 0x46, 0x41, 0x5C,
    0x6, 0x4B, 0x4E, 0x4F, 0 };
static unsigned char gv_error_mess_str[29] = { 0x6C, 0x4A, 0x8F, 0xAD, 0xE, 0x53, 0x35, 0xB9, 0x4D, 0x5B, 0x5B, 0x8, 0x5F, 0x41, 0x5C, 0x40, 0x8, 0x5C, 0x40, 0x4D, 0x8, 0x4E, 0x41,
    0x44, 0x4D, 0x5B, 0x9, 0x9, 0 };
...
int GenerateDump(EXCEPTION_POINTERS* pExceptionPointers)
{
    ...
    sprintf(szFileName, "%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld.dmp", szVersion, stLocalTime.wYear, stLocalTime.wMonth, stLocalTime.wDay, stLocalTime.wHour, stLocalTime.wMinute,
    ...
    bMiniDumpSuccessful = MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hDumpFile, MiniDumpWithIndirectlyReferencedMemory, &ExpParam, NULL, NULL);
...
bool cLuxBase::StartGame(const tString& asFile, const tString& asFolder, const tString& asStartPos)
{
    ...
    if (sMapFile == "")
    {
        ...
        if (msCommandLineMapFile != "")
            sMapFile = msCommandLineMapFile;
...
bool cLuxBase::InitUserConfig()
{
    ...
    if (bDidLoadDefault)
    {
        mpUserKeyConfig->SetString("Attack", "Secondary", "Keyboard.LeftAlt");
    }
#endif
...
void cLuxBase::SaveConfig()
{
    ...
    if (mpUserConfig)
    {
        tString sUserConfigCopy;
        tString sUserKeysCopy;

        Log(" Saving user config.\n");
        mpUserConfig->Save(&sUserConfigCopy);
        if (mpUserKeyConfig)
            mpUserKeyConfig->Save(&sUserKeysCopy);
    }
}
...
// Demo output
    ...
    mpEngine->GetResources()->AddEntityLoader(hplNew(cLuxAnimatedPropLoader, ("AnimatedProp")));
    ...
    mpEngine->GetResources()->AddEntityLoader(hplNew(cLuxPropLoader_VoiceFlashbackTrigger, ("VoiceFlashbackTrigger")));
    mpEngine->GetResources()->AddEntityLoader(hplNew(cLuxPropLoader_PhoneBox, ("PhoneBox")));
    ...
    // mpEngine->GetResources()->AddEntityLoader(hplNew(cLuxEnemyLoader_ThomasPig, ("Enemy_ThomasPig")));
    // mpEngine->GetResources()->AddEntityLoader(hplNew(cLuxEnemyLoader_Tesla, ("Enemy_Tesla")));
...
#endif
    mpEngine->GetResources()->AddAreaLoader(hplNew(cLuxAreaLoader_Infection, ("Infection")));
...
void cLuxBase::SetCustomStory(cLuxCustomStorySettings* apCustomStory)
{
    ...
    if (mpCustomStory)
    {
        ...
        msProfileSavePath = cString::AddSlashAtEndW(msMainProfileSavePath + _W("custom")) + cString::AddSlashAtEndW(sStoryFolder);
...
bool cLuxBase::LoadLanguage(const tString& asName, bool abForceReload)
{
    ...
    */
    ...
    if (mpCustomStory && sGameFileName != mpCustomStory->msDefaultExtraLanguage)
        pResources->AddLanguageFile(cString::To8Char(mpCustomStory->msStoryRootFolder) + mpCustomStory->msExtraLangFilePrefix + mpCustomStory->msDefaultExtraLanguage, false);
