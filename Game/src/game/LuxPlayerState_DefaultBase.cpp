...
iLuxPlayerState_DefaultBase::iLuxPlayerState_DefaultBase(cLuxPlayer* apPlayer, eLuxPlayerState aType)
...
void iLuxPlayerState_DefaultBase::Update(float afTimeStep)
{
    ...
    gpBase->mpMapHelper->GetClosestEntity(pCam->GetPosition(), pCam->GetForward(), 20.0f, &mfFocusDistance, &mpBodyInFocus, &mpEntityInFocus);
...
{
    if (mpEntityInFocus && mfFocusDistance < mpEntityInFocus->GetMaxFocusDistance() && mpEntityInFocus->CanInteract(mpBodyInFocus) &&
...
{
    gpBase->mpEffectRenderer->ClearOutlineObjects();
    if (apEntity == NULL || apEntity->GetEntityType() != eLuxEntityType_Prop || mfFocusDistance > apEntity->GetMaxFocusDistance() ||
...
kBeginSerializeVirtual(iLuxPlayerState_DefaultBase_SaveData, iLuxPlayerState_SaveData) kEndSerialize()
