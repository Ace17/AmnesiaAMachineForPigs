...
private:
    float mfTimeSinceLastSound, mfStaminaTimer, mfSprintSlowdownStartTime, mfSprintExhaustionTime, mfExhaustionSpeedMultiplier, mfSprintRecoverySpeed;
};
...
public:
    ...
    void StopPlaying();
    void Start(const tString& asVoiceFile,
...
private:
    bool mbIsDelaying, mbIsPlaying, mbUseEffects, mbObstructMovement;
    float mfEffectFadeInTime, mfEffectFadeOutTime, mfSepiaAmount, mfLightFadeAmount, mfImageTrailAmount, mfBlurStartDistance, mfBlurAmount, mfFovMultiplier, mfMoveSpeedMultiplier,
...
public:
    cLuxFlashbackData() {}
    cLuxFlashbackData(const tString& asFile, const tString& asCallback)
        : msFile(asFile)
...
private:
    float mfFastMovementThreshold, mfSlowMovementThreshold, mfImmediatePlayerSpeed, mfAveragePlayerSpeed;
};
