...
kBeginSerializeBaseVirtual(iLuxInteractConnection_SaveData) kSerializeVar(msName, eSerializeType_String)
...
cLuxInteractConnection_Rope::cLuxInteractConnection_Rope(const tString& asName,
      iLuxProp* apProp,
      iPhysicsRope* apRope,
      float afSpeedMul,
      float afMinSpeed,
...
cLuxInteractConnection_MoveObject::cLuxInteractConnection_MoveObject(const tString& asName, iLuxProp* apProp, cLuxProp_MoveObject* apMoveObject, bool abInvert, int alStatesUsed)
    : iLuxInteractConnection(asName, apProp, abInvert, alStatesUsed)
{
...
{
    if (mbInteractionOnly == false || (mpMoveObject->IsMoving() == false && mbPropNeedsUpdate == false))
    {
        return;
    }
