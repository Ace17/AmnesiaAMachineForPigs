...
cLuxHandObject_Melee::cLuxHandObject_Melee(const tString& asName, cLuxPlayerHands* apHands)
...
void cLuxHandObject_Melee::UpdateCheckDamageEvent(float afTimeStep)
{
    if (mpHands->CheckAnimationEvent(mfRelativeDamageTime))
    {
        ...
        bool bHit = gpBase->mpMapHelper->ShapeDamage(mpCollideShape, mtxHands, pCam->GetPosition(), mfAttackMinDamage * fDamageMul, mfAttackMaxDamage * fDamageMul, mfAttackForce,
              mfMaxAttackForce, mlAttackStrength, mfAttackHitSpeed, eLuxDamageType_BloodSplat, eLuxWeaponHitType_Sword, true, false, true, false, NULL);
