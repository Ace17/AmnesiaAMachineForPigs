    ...
    if(sOldMatName == "soliddiffuse")
            {
                    ...
                    cMaterial* pCustomMat = hplNew( cMaterial, ("GrabTransCustom",	gpBase->mpEngine->GetGraphics(),
                                                                                                                                                    gpBase->mpEngine->GetResources(),
    pMatType) ); pCustomMat->SetDepthTest(true); pCustomMat->SetBlendMode(eMaterialBlendMode_Mul);
...
float cLuxPlayerState_InteractGrab::DrawDebug(cGuiSet* apSet, iFontData* apFont, float afStartY)
{
    ...
    cVector3f vRotError(cMath::GetAngleDistanceRad(vCurrentRot.x, vWantedRot.x), cMath::GetAngleDistanceRad(vCurrentRot.y, vWantedRot.y),
