...
cLuxEnemyLoader_WaterLurker::cLuxEnemyLoader_WaterLurker(const tString& asName)
...
cLuxEnemy_WaterLurker::cLuxEnemy_WaterLurker(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxEnemy(asName, alID, apMap, eLuxEnemyType_WaterLurker)
{
...
bool cLuxEnemy_WaterLurker::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage* apMessage)
{
    kLuxBeginStateMachine
          ...
          ////////////////////////////////
          // Default
          kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) ChangeState(eLuxEnemyState_Hunt);
    ...
    kLuxOnMessage(eLuxEnemyMessage_FoodInRange) cAINode* pNode = mpPathfinder->GetNodeAtPos(apMessage->mvCustomValue, 0, 1.5f, true, true, true, NULL);
    if (pNode)
    {
    ...
    ////////////////////////////////
    // Idle
    kLuxState(eLuxEnemyState_Idle) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Idle);

    kLuxOnUpdate if (PlayerIsDetected()) ChangeState(eLuxEnemyState_Hunt);
    ...
    ////////////////////////////////
    // Go Home
    kLuxState(eLuxEnemyState_GoHome) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Idle);
    ...
    ///////////////////////
    // Update
    kLuxOnUpdate if (mpMover->GetStuckCounter() > 0.9f)
    {
    ...
    ///////////////////////
    // Movement effect
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) SplashWater(eWaterLurkerSplash_Walk);
    ...
    kLuxState(eLuxEnemyState_Investigate) kLuxOnEnter gpBase->mpDebugHandler->AddMessage(_W("Sound Heard! Vol: ") + cString::ToStringW(mfTempVal), false);
    ...
    kLuxOnUpdate if (mpMover->GetStuckCounter() > 0.9f) { ChangeState(eLuxEnemyState_Idle); }
    ...
    kLuxState(eLuxEnemyState_Search) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Idle);
    ...
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) gpBase->mpDebugHandler->AddMessage(_W("to eat state!\n"), false);
    ...
    kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
          ...
          ////////////////////////////////
          // Eat
          kLuxState(eLuxEnemyState_Eat) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Idle);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) cLuxProp_Object* pFood = GetClosestFood(3.5f, mfPlayerDetectionHeight);
    if (pFood)
    {
    ...
    ////////////////////////////////
    // Move forward
    kLuxOnUpdate if (Dist2D(mvTempPos) > mpCharBody->GetSize().x * 0.75f) mpMover->MoveToPos(mvTempPos);

    ////////////////////////////////
    // Player detection, only hunt if player is really close.
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) if (DistToPlayer2D() < 2.0f) ChangeState(eLuxEnemyState_Hunt);
    else mbPlayerDetected = false;
    ...
    kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
          ...
          ///////////////////////
          // Enter
          kLuxOnEnter if (PlayerIsDetected() == false)
    {
        ChangeState(eLuxEnemyState_GoHome);
        mbCausesInfectionIncrease = false;
    }
    else if (mPreviousState == eLuxEnemyState_AttackMeleeShort) { ChangeState(eLuxEnemyState_HuntPause); }
    else
    {
    ...
    ///////////////////////
    // Movement effect
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) if (mpMover->GetWantedSpeedAmount() > 0.2f) SplashWater(eWaterLurkerSplash_Run);
    SendMessage(eLuxEnemyMessage_TimeOut_2, 0.6f, true);

    ///////////////////////
    // Update the player position
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) mpPathfinder->MoveTo(gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition());
    ...
    ///////////////////////
    // On update
    kLuxOnUpdate if (DistToPlayer() < mfNormalAttackDistance) { ChangeState(eLuxEnemyState_AttackMeleeShort); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
          ...
          ///////////////////////
          // Enter
          kLuxOnEnter mpPathfinder->Stop();
    SendMessage(eLuxEnemyMessage_TimeOut, mfAfterAttackPauseTime, true);

    kLuxOnMessage(eLuxEnemyMessage_TimeOut) ChangeState(eLuxEnemyState_Hunt);

    ////////////////////////
    // Overload global
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) ChangeState(eLuxEnemyState_Hunt);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
          ...
          ////////////////////////////////
          // Attack Short
          kLuxState(eLuxEnemyState_AttackMeleeShort) kLuxOnEnter SendMessage(eLuxEnemyMessage_TimeOut, 0.3f, true);

    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) ChangeState(mPreviousState);

    kLuxOnMessage(eLuxEnemyMessage_TimeOut) SplashWater(eWaterLurkerSplash_Attack);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_FoodInRange)
          ...
          ////////////////////////////////
          // Dead (just in case...)
          kLuxState(eLuxEnemyState_Dead) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Silent);
