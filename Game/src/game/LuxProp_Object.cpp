...
cLuxPropLoader_Object::cLuxPropLoader_Object(const tString& asName)
...
cLuxProp_Object::cLuxProp_Object(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Object)
{
    mbBroken = false;
    mfLifeLengthCount = 0;
    mlStuckState = 0;
...
void cLuxProp_Object::UpdateInfectionVision(float afTimeStep)
{
    ...
    if ((fInfection >= mfVisionMinInfection && mbInfectionVisionActive) || (fInfection < mfVisionMinInfection && mbInfectionVisionActive == false))
    {
        return;
    }
