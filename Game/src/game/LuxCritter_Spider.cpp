...
cLuxPropLoader_Critter_Spider::cLuxPropLoader_Critter_Spider(const tString& asName)
    : iLuxPropLoader_Critter(asName)
{
}
...
cLuxCritter_Spider::cLuxCritter_Spider(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp_CritterBase(asName, alID, apMap)
{
...
void cLuxCritter_Spider::UpdateVelocity(float afTimeStep)
{
    ...
    else if (mState == eLuxCritterState_Move)
    {
        ...
        if (mfCount <= 0)
        {
            ...
            if (gpBase->mpPlayer->IsDead() == false && fDistToPlayer < mvAttackPlayerDistMinMax.y && fDistToPlayer > mvAttackPlayerDistMinMax.x)
            {
