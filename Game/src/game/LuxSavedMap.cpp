...
void cLuxSavedMap::FromMap(cLuxMap* apMap)
{
    ...
    // Particle systems
    {
        ...
        while (psIt.HasNext())
        {
            ...
            if (pPS->IsSaved() && pPS->GetEntityParent() == NULL && pPS->GetParent() == NULL && (pPS->IsDying() == false || pPS->IsActive() == false && pPS->IsDead() == false))
            {
...
kBeginSerializeBase(cLuxSavedMap) kSerializeVar(msFileName, eSerializeType_String)
...
kSerializeClassContainer(mlstLights, cEngineLight_SaveData, eSerializeType_Class) kSerializeClassContainer(mlstRopes, cEngineRope_SaveData, eSerializeType_Class)
      kSerializeClassContainer(mlstSounds, cEngineSound_SaveData, eSerializeType_Class) kSerializeClassContainer(mlstPS, cEnginePS_SaveData, eSerializeType_Class)

            kSerializeClassContainer(mlstEntities, iLuxEntity_SaveData, eSerializeType_ClassPointer)

                  kSerializeClassContainer(mlstTimers, cLuxEventTimer, eSerializeType_Class) kSerializeClassContainer(mlstVars, cLuxScriptVar, eSerializeType_Class)
