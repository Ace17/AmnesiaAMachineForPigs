...
void cLuxMap::SetCheckPoint(const tString& asName, const tString& asStartPos, const tString& asCallback, bool abKeepPlayerInLimbo)
{
    ...
    gpBase->mpDebugHandler->AddMessage(_W("Setting check point ") + cString::To16Char(msCheckPointName) + _W(" Music: '") + cString::To16Char(msCheckPointMusic) + _W("'"), false);
}
...
void cLuxMap::BroadcastEnemyMessage(eLuxEnemyMessage aType,
      bool abHasPosition,
...
bool cLuxMap::CheckCollision(iLuxCollideCallbackContainer* apCollider1, iLuxCollideCallbackContainer* apCollider2)
{
    ...
    for (int body1 = 0; body1 < apCollider1->GetBodyNum(); ++body1)
        for (int body2 = 0; body2 < apCollider2->GetBodyNum(); ++body2)
        {
            ...
            if (mpPhysicsWorld->CheckShapeCollision(pBody1->GetShape(), pBody1->GetLocalMatrix(), pBody2->GetShape(), pBody2->GetLocalMatrix(), collideData, 1, false))
            {
                return true;
            }
        }
...
void cLuxMap::AddUseItemCallback(const tString& asName, const tString& asItem, const tString& asEntity, const tString& asFunction, bool abAutoCallback)
{
