...
void cLuxSaveHandler::LoadSaveGameData(cLuxSaveGame_SaveData* apSave)
{
    ...
    ///////////////////
    // Load new map
    if (pCurrentMap == NULL || msOldMapFolder != gpBase->mpMapHandler->GetMapFolder() || pCurrentMap->GetFileName() != apSave->mMap.msFileName)
    {
...
tWString cLuxSaveHandler::GetSaveName(const tWString& asPrefix)
{
    ...
    tWString sFileName = asPrefix + _W("_") + cString::To16Char(sMapName) + _W("_") + cString::ToStringW(currentDate.year) + _W("_") + cString::ToStringW(currentDate.month + 1) +
          _W("_") + cString::ToStringW(currentDate.month_day) + _W("_") + cString::ToStringW(currentDate.hours) + _W("_") + cString::ToStringW(currentDate.minutes) + _W("_") +
          cString::ToStringW(currentDate.seconds) + _W("_") + cString::ToStringW(mlSaveNameCount) + _W(".sav");
    mlSaveNameCount++;
