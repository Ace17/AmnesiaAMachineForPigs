...
cLuxPropLoader_Wheel::cLuxPropLoader_Wheel(const tString& asName)
...
cLuxProp_Wheel::cLuxProp_Wheel(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_Wheel)
{
...
{
    apSet->DrawFont(
          apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("WheelAngle: %f, Max: %f, Min: %f"), cMath::ToDeg(mfAngle), cMath::ToDeg(mfMaxLimit), cMath::ToDeg(mfMinLimit));
    afStartY += 13.0f;
...
void cLuxProp_Wheel::SetMaxJointAngle(float afMaxAngle)
{
    ...
    if (fDistToMax > kPif)
    { // TOo far away, no need to set limit.
