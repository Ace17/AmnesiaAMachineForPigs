...
eInputMenuEntryPos cLuxInputMenuEntry::GetKeyPos(eKey aKey)
{
    ...
    {
        const cSubActionWrapper& subAction = mvSubActions[i];
        if (subAction.mType == eInputDeviceType_Keyboard && subAction.mlInputId == aKey)
...
eInputMenuEntryPos cLuxInputMenuEntry::GetMouseButtonPos(eMouseButton aButton)
{
    ...
    {
        const cSubActionWrapper& subAction = mvSubActions[i];
        if (subAction.mType == eInputDeviceType_Mouse && subAction.mlInputId == aButton)
            return (eInputMenuEntryPos)i;
    }
...
eInputMenuEntryPos cLuxInputMenuEntry::GetGamepadButtonPos(eGamepadButton aButton)
{
    ...
    {
        const cSubActionWrapper& subAction = mvSubActions[i];
        if (subAction.mGamepadInputType == eGamepadInputType_Button && subAction.mlInputId == aButton)
            return (eInputMenuEntryPos)i;
    }
...
eInputMenuEntryPos cLuxInputMenuEntry::GetGamepadHatStatePos(eGamepadHat aHat, eGamepadHatState aState)
{
    ...
    {
        const cSubActionWrapper& subAction = mvSubActions[i];
        if (subAction.mGamepadInputType == eGamepadInputType_Hat && subAction.mlInputId == aHat && subAction.mfInputValue == aState)
            return (eInputMenuEntryPos)i;
    }
...
eInputMenuEntryPos cLuxInputMenuEntry::GetGamepadAxisPos(eGamepadAxis aAxis, eGamepadAxisRange aRange)
{
    ...
    {
        const cSubActionWrapper& subAction = mvSubActions[i];
        if (subAction.mGamepadInputType == eGamepadInputType_Axis && subAction.mlInputId == aAxis && subAction.mfInputValue == aRange)
            return (eInputMenuEntryPos)i;
    }
...
bool cLuxInputMenuEntry::InputEntryClick(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    {
        SaveSubAction(GetPosFromInput(apWidget), eInputDeviceType_Mouse, gpBase->mpEngine->GetGui()->TranslateGuiMouseToMouse((eGuiMouseButton)aData.mlVal));
    }
    return true;
}
...
bool cLuxInputMenuEntry::InputEntryKeyPress(iWidget* apWidget, const cGuiMessageData& aData)
{
    if (IsWaiting())
    {
        ...
        {
            SaveSubAction(GetPosFromInput(apWidget), eInputDeviceType_Keyboard, aData.mKeyPress.mKey);
        }
        else
        {
            UpdateEntry();
...
bool cLuxInputMenuEntry::InputEntryGamepadButtonPress(iWidget* apWidget, const cGuiMessageData& aData)
{
    if (IsWaiting())
    {
        ...
        SaveGamepadSubAction(GetPosFromInput(apWidget), aData.mGamepadInputData.mInputType, aData.mGamepadInputData.mlInputId, aData.mGamepadInputData.mfInputValue);
    }
...
cLuxMainMenu_KeyConfig::cLuxMainMenu_KeyConfig(cGuiSet* apGuiSet, cGuiSkin* apGuiSkin)
    : iLuxMainMenuWindow(apGuiSet, apGuiSkin)
{
...
{
    mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("KeyConfig", "LoadDefaultsMessage"), kTranslate("Global", "OK"), kTranslate("Global", "Cancel"),
...
bool cLuxMainMenu_KeyConfig::PressOK(iWidget* apWidget, const cGuiMessageData& aData)
{
    if (mbInvalidInputs)
    {
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("KeyConfig", "InvalidInputMessage"), kTranslate("Global", "OK"), kTranslate("Global", "Cancel"),
