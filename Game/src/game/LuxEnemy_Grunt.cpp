...
cLuxEnemyLoader_Grunt::cLuxEnemyLoader_Grunt(const tString& asName)
...
cLuxEnemy_Grunt::cLuxEnemy_Grunt(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxEnemy(asName, alID, apMap, eLuxEnemyType_Grunt)
{
    mfWaitTime = 0;
...
bool cLuxEnemy_Grunt::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage* apMessage)
{
    ...
    kLuxOnMessage(eLuxEnemyMessage_TakeHit) mfDamageCount += apMessage->mfCustomValue;
    ...
    ////////////////////////////////
    // Wait
    kLuxState(eLuxEnemyState_Wait) kLuxOnEnter if (mfWaitTime <= 0) SendMessage(eLuxEnemyMessage_TimeOut, cMath::RandRectf(1, 3), true);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) PlayAnim("IdleExtra" + cString::ToString(cMath::RandRectl(1, mlIdleExtraNum)), false, 0.3f);

    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) SendMessage(eLuxEnemyMessage_TimeOut_2, cMath::RandRectf(2, 6), true);
    ...
    ////////////////////////////////
    // Patrol
    kLuxState(eLuxEnemyState_Patrol) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Idle);
    SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);
    PatrolUpdateGoal();
    ...
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) PatrolEndOfPath();
...
// Alert
    ...
    kLuxState(eLuxEnemyState_Alert) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Alert);
    ...
    ////////////////////////////////
    // Search
    kLuxState(eLuxEnemyState_Search) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Alert);
    ...
    kLuxState(eLuxEnemyState_Hunt)
          ///////////////////////
          // Enter
          kLuxOnEnter if (mlAttackHitCounter >= 1)
    {
    ...
    //////////////////
    // When lost player give some extra time to catch up
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_3) if (PlayerIsDetected() == false)
    {
        gpBase->mpPlayer->RemoveTerrorEnemy(this);
        gpBase->mpMusicHandler->RemoveEnemy(eLuxEnemyMusic_Attack, this);
        ChangeState(eLuxEnemyState_Search);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          ///////////////////////
          // Enter
          kLuxOnEnter mpMover->ResetStuckCounter();
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          ///////////////////////
          // Enter
          kLuxOnEnter SendMessage(eLuxEnemyMessage_TimeOut_2, cMath::RandRectf(mfHuntPauseMinTime, mfHuntPauseMaxTime), true);
    mpPathfinder->Stop();
    ...
    else { PlayAnim("BreakDoor", false, 0.3f); }

    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) Attack(mNormalAttackSize, mBreakDoorAttackDamage, 1.0f);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          ////////////////////////////////
          // Attack Short
          kLuxState(eLuxEnemyState_AttackMeleeShort) kLuxOnEnter mpPathfinder->Stop();
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) if (mPreviousState == eLuxEnemyState_Hurt) ChangeState(eLuxEnemyState_Hunt);
    else ChangeState(mPreviousState);

    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) Attack(mNormalAttackSize, mNormalAttackDamage, 1.0f);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          ////////////////////////////////
          // Attack Launch
          kLuxState(eLuxEnemyState_AttackMeleeLong) kLuxOnEnter mpPathfinder->Stop();
    SetMoveSpeed(eLuxEnemyMoveSpeed_Run);
    mfForwardSpeed *= 1.5f;
    ...
    kLuxOnUpdate if (mlTempVal == 0) { mpMover->MoveToPos(gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition()); }

    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) mlTempVal = 1;
