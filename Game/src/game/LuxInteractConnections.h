...
{
    kSerializableClassInit(iLuxInteractConnection_SaveData) public : virtual ~iLuxInteractConnection_SaveData() {}
...
public:
    iLuxInteractConnection(const tString& asName, iLuxProp* apProp, bool abInvert, int alStatesUsed)
        : msName(asName)
        , mpProp(apProp)
        , mbInteractionOnly(true)
        , mbInvert(abInvert)
        , mlStateUsed(alStatesUsed)
    {
    }
...
{
    kSerializableClassInit(cLuxInteractConnection_Rope_SaveData) public : iLuxInteractConnection* CreateConnection(cLuxMap* apMap);
...
public:
    cLuxInteractConnection_Rope(const tString& asName,
          iLuxProp* apProp,
          iPhysicsRope* apRope,
          float afSpeedMul,
          float afMinSpeed,
...
{
    kSerializableClassInit(cLuxInteractConnection_MoveObject_SaveData) public : iLuxInteractConnection* CreateConnection(cLuxMap* apMap);
    void FromConnection(iLuxInteractConnection* apConnection);
