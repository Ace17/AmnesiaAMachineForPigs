    ...
    if (pEntity && pEntity->GetMeshEntity())
    {
        ...
        if (pMeshEntity)
        {
            ...
            for (int i = 0; i < pMeshEntity->GetSubMeshEntityNum(); ++i)
            {
                ...
                if (pMaterial && pMaterial->GetType()->IsTranslucent() == false && (mbCheckShadow == false || pSubMeshEnt->GetRenderFlagBit(eRenderableFlag_ShadowCaster)))
                {
                    bFoundSolid = true;
                    break;
                }
...
{
    if (apBody->GetCollideCharacter() == false || apBody->IsCharacter())
        return false;
    if (mbCheckDynamic == false && apBody->GetMass() > 0)
        return false;
...
bool cLuxMapHelper::ShapeDamage(iCollideShape* apShape,
      ...
      int alStrength,
      float afHitSpeed,
      eLuxDamageType aDamageType,
      eLuxWeaponHitType aWeaponHitType,
      bool abCheckEnemies,
      bool abCheckPlayer,
      bool abCheckProps,
      ...
      bool* apHitPlayer)
{
    ...
    for (size_t i = 0; i < vBodies.size(); ++i)
    {
        ...
        bool bCollide = pPhysicsWorld->CheckShapeCollision(apShape, a_mtxTransform, pBody->GetShape(), pBody->GetLocalMatrix(), collideData, 4, false);
        if (bCollide == false)
        {
            continue;
        }
...
bool cLuxMapHelper::GetClosestEntity(const cVector3f& avStart, const cVector3f& avDir, float afRayLength, float* afDistance, iPhysicsBody** apBody, iLuxEntity** apEntity)
{
    cLuxMap* pCurrentMap = gpBase->mpMapHandler->GetCurrentMap();
    if (pCurrentMap == NULL)
        return false;
...
bool cLuxMapHelper::GetClosestCharCollider(const cVector3f& avStart,
...
float cLuxMapHelper::GetLightLevelAtPos(const cVector3f& avPos, std::vector<iLight*>* apSkipLightsVec, float afRadiusAdd)
{
    ...
    for (; it != lstIntersectingLights.end(); ++it)
    {
        ...
        {
            // Check line of sight
            if (pLight->GetLightType() == eLightType_Spot && pLight->GetCastShadows() && CheckLineOfSight(pLight->GetWorldPosition(), avPos, true) == false)
            {
                continue;
            }
...
void cLuxMapHelper::GetLightsAtNode(iRenderableContainerNode* apNode, tLightList& alstLights, const cVector3f& avPos, float afRadius, bool includeInvisible, bool includeBoxLights)
{
    ...
    if (apNode->HasObjects())
    {
        ...
        {
            iRenderable* pObject = *it;
            if ((pObject->IsVisible() == false && !includeInvisible) || pObject->GetRenderType() != eRenderableType_Light)
                continue;
            ...
            {
            case eLightType_Box:
                bAdd = includeBoxLights &&
                      (cMath::CheckPointInBVIntersection(avPos, *pLight->GetBoundingVolume()) ||
