...
bool cLuxInventory_Slot::OnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if (pItem && pItem != mpInventory->GetPickedItem())
    {
        ...
        if (mfGlowAlpha > 0)
        {
            ...
            for (int i = 0; i < 2; ++i)
                mpInventory->mpGuiSet->DrawGfx(
                      pItem->GetImage(), vItemPos + cVector3f(0, 0, 0.1f), -1, cColor(fPulseAlpha * mfGlowAlpha * mpInventory->mfAlpha, 1), eGuiMaterial_Additive);
        }
        ...
        if (pType->HasCount())
        {
            ...
            mpInventory->mpGuiSet->DrawFont(mpInventory->mpFontDefault, vPos, mpInventory->mvLayout_FontSize_ItemCount, cColor(1, mpInventory->mfAlpha), eFontAlign_Center,
                  eGuiMaterial_FontNormal, _W("x%d"), pItem->GetCount());
        }
    }
...
void cLuxInventory::Update(float afTimeStep)
{
    ...
    if (gpBase->mpEngine->GetInput()->IsTriggerd(eLuxAction_LeftClick) == false && gpBase->mpEngine->GetInput()->IsTriggerd(eLuxAction_UIPrimary) == false)
    {
        mpPickedItem = NULL;
    }
    ...
    */
    ...
    if (mfMessageTextAlpha > 0)
    {
        if (msMessageText != _W(""))
        {
            ...
            mpGuiSet->DrawFont(msMessageText, mpFontDefault, mvLayout_PopUpMessageCenter, mvLayout_FontSize_Message, cColor(1, fAlpha), eFontAlign_Center);
    ...
    // Draw frame around text
    {
        ...
        cVector3f vTextFramePos = vHeaderCenter - cVector3f(mvLayout_ItemTextFrameSize.x, 0, 0) * 0.5f - cVector3f(mpFrameGenericCorners[0]->GetActiveSize()) -
              cVector3f(mvLayout_ItemTextFrameHPadding.x, mvLayout_ItemTextFrameVPadding.y, 0) + mvLayout_ItemTextFrameOffset;

        DrawFrame(mpFrameGenericCorners, mpFrameGenericBorders, vTextFramePos, mvLayout_ItemTextFrameSize);
    }
...
cLuxInventory_Item* cLuxInventory::AddItem(const tString& asName,
      eLuxItemType aType,
      const tString& asSubTypeName,
      const tString& asImageName,
      float afAmount,
      const tString& asVal,
      const tString& asExtraVal,
...
void cLuxInventory::AddCombineCallback(const tString& asName, const tString& asItemA, const tString& asItemB, const tString& asFunction, bool abAutoDestroy)
{
...
cLuxCombineItemsCallback* cLuxInventory::GetCombineCallback(const tString& asItemA, const tString& asItemB)
{
    ...
    for (; it != mlstCombineCallbacks.end(); ++it)
    {
        ...
        if ((pCombCallback->msItemA == asItemA && pCombCallback->msItemB == asItemB) || (pCombCallback->msItemA == asItemB && pCombCallback->msItemB == asItemA))
        {
...
void cLuxInventory::SetDescTextFromItem(cLuxInventory_Item* apItem)
{
    if (apItem)
    {
        ...
        SetDescText(kTranslate("Inventory", apItem->GetGameNameEntry()) + pItemType->GetDisplayedNameAdd(apItem), kTranslate("Inventory", apItem->GetGameDescEntry()));
    }
    else
    {
        SetDescText(_W(""), _W(""));
...
{
    return (apItemA->GetType() == aTypeA && apItemB->GetType() == aTypeB) || (apItemA->GetType() == aTypeB && apItemB->GetType() == aTypeA);
}
    ...
    */
    ...
    for (int y = 0; y < mlLayout_SlotsRows; ++y)
        for (int x = 0; x < mlLayout_SlotsColumns; ++x)
        {
            ...
            cVector3f vPos = cVector3f((float)x * (pImage->GetSize().x + vSlotSeparation.x), (float)y * (pImage->GetSize().y + vSlotSeparation.y), 0);
...
bool cLuxInventory::SanityOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    /////////////////////////////////////////
    // Draw frame
    DrawFrameAroundWidget(
          mpFrameSanityCorners, mpFrameSanityBorders, apWidget, mvLayout_SanityFrameHPadding, mvLayout_SanityFrameVPadding, mvLayout_SanityFrameOffset, mvLayout_SanityFrameSize);
...
bool cLuxInventory::HealthOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    // Draw frame
    DrawFrameAroundWidget(
          mpFrameHealthCorners, mpFrameHealthBorders, apWidget, mvLayout_HealthFrameHPadding, mvLayout_HealthFrameVPadding, mvLayout_HealthFrameOffset, mvLayout_HealthFrameSize);
...
bool cLuxInventory::TinderboxOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    mpGuiSet->DrawFont(mpFontDefault, vTinderboxCenter + cVector3f(0, -vCountFontSize.y * 0.5f, 1), vCountFontSize, cColor(1, mfAlpha), eFontAlign_Center, eGuiMaterial_FontNormal,
    ...
    DrawFrameAroundWidget(mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_TinderboxesFrameHPadding, mvLayout_TinderboxesFrameVPadding,
...
bool cLuxInventory::JournalOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    /////////////////////////////////////////
    // Draw frame
    DrawFrameAroundWidget(mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_JournalFrameHPadding, mvLayout_JournalFrameVPadding, mvLayout_JournalFrameOffset,
          mvLayout_JournalFrameSize);
...
bool cLuxInventory::ItemSlotsOnDraw(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    {
        DrawFrameAroundWidget(
              mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_SlotFrameHPadding, mvLayout_SlotFrameVPadding, mvLayout_SlotFrameOffset, mvLayout_SlotFrameSize);
    }

    if (mbLayout_DrawSlotsGrid)
    {
        // Get grid lines size
        cVector2f vHoriSize = cVector2f(apWidget->GetSize().x + mvLayout_SlotGridHExtension.x + mvLayout_SlotGridHExtension.y, mvLayout_SlotsSeparation.y);
        cVector2f vVertSize = cVector2f(mvLayout_SlotsSeparation.x, apWidget->GetSize().y + mvLayout_SlotGridVExtension.x + mvLayout_SlotGridVExtension.y);

        // Get grid lines separation
        cVector2f vLineSeparation = mvLayout_SlotSize + cVector2f(mvLayout_SlotsSeparation.x, mvLayout_SlotsSeparation.y);

        // Get grid start point
        cVector3f vPos = mvLayout_SlotsStart - cVector3f(mvLayout_SlotFrameHPadding.x, mvLayout_SlotFrameVPadding.x, 0) + mvLayout_SlotFrameOffset + mvLayout_SlotsGridOffset;
...
void cLuxInventory::DrawFrameAroundWidget(cGuiGfxElement** apGfxCorners,
      cGuiGfxElement** apGfxBorders,
      iWidget* apWidget,
      const cVector2f& avHPadding,
      const cVector2f& avVPadding,
      ...
      const cVector2f& avSize)
{
    ...
    cVector2f vSize = cVector2f(avSize.x == -1 ? vWidgetSize.x : avSize.x, avSize.y == -1 ? vWidgetSize.y : avSize.y);

    // Wrap frame around rectangle with added padding
    DrawFrame(apGfxCorners, apGfxBorders, vPosition - cVector3f(apGfxCorners[0]->GetActiveSize().x + avHPadding.x, apGfxCorners[0]->GetActiveSize().y + avVPadding.x, 0) + avOffset,
          vSize + cVector2f(avHPadding.x + avHPadding.y, avVPadding.x + avVPadding.y));
}

//-----------------------------------------------------------------------

void cLuxInventory::DrawFrame(cGuiGfxElement** apGfxCorners, cGuiGfxElement** apGfxBorders, const cVector3f& avPosition, const cVector2f& avSize)
{
    /////////////////////////////////////////
    // Draw frame
    ...
    // Left part
    {
        ...
        mpGuiSet->DrawGfx(apGfxCorners[0], vPos, -1, cColor(1, mfAlpha));

        // Draw Left border
        mpGuiSet->DrawGfx(apGfxBorders[3], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y, 0), cVector2f(apGfxCorners[0]->GetActiveSize().x, avSize.y), cColor(1, mfAlpha));

        // Draw Lower left corner
        mpGuiSet->DrawGfx(apGfxCorners[3], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y + avSize.y, 0), -1, cColor(1, mfAlpha));
    }
    ...
    {
        // Draw Upper border
        mpGuiSet->DrawGfx(apGfxBorders[0], vPos, cVector2f(avSize.x, apGfxCorners[0]->GetActiveSize().y), cColor(1, mfAlpha));

        // Draw Lower border
        mpGuiSet->DrawGfx(
              apGfxBorders[2], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y + avSize.y, 0), cVector2f(avSize.x, apGfxCorners[3]->GetActiveSize().y), cColor(1, mfAlpha));
    }
    ...
    // Right part
    {
        ...
        mpGuiSet->DrawGfx(apGfxCorners[1], vPos, -1, cColor(1, mfAlpha));

        // Draw Right border
        mpGuiSet->DrawGfx(apGfxBorders[1], vPos + cVector3f(0, apGfxCorners[1]->GetActiveSize().y, 0), cVector2f(apGfxCorners[1]->GetActiveSize().x, avSize.y), cColor(1, mfAlpha));

        // Draw Lower right corner
        mpGuiSet->DrawGfx(apGfxCorners[2], vPos + cVector3f(0, apGfxCorners[1]->GetActiveSize().y + avSize.y, 0), -1, cColor(1, mfAlpha));
    }
}
