...
cLuxPropLoader_SwingDoor::cLuxPropLoader_SwingDoor(const tString& asName)
...
cLuxProp_SwingDoor::cLuxProp_SwingDoor(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxProp(asName, alID, apMap, eLuxPropType_SwingDoor)
{
    mbClosed = false;
    mbLocked = false;
...
void cLuxProp_SwingDoor::SetOpenAmount(float afOpenAmount, float afDuration, bool abGoTowardsMaxAngle)
{
    ...
    if (!pHingeJoint || abGoTowardsMaxAngle && pHingeJoint->GetMaxAngle() == 0.0f || !abGoTowardsMaxAngle && pHingeJoint->GetMinAngle() == 0.0f)
    {
        return;
    }
    ...
    if (mfGoalOpenAmount == mfCurrentOpenAmount)
        return;
...
void cLuxProp_SwingDoor::UpdateOpening(float afTimeStep)
{
    if (mbIsUpdatingOpenAmount)
    {
        ...
        if ((mfOpenSpeed > 0 && newOpenAmount >= mfGoalOpenAmount) || (mfOpenSpeed < 0 && newOpenAmount <= mfGoalOpenAmount))
        {
