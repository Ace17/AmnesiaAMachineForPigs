...
cLuxEnemyLoader_ManPig::cLuxEnemyLoader_ManPig(const tString& asName)
    : iLuxEnemyLoader(asName)
{
}
...
cLuxEnemy_ManPig::cLuxEnemy_ManPig(const tString& asName, int alID, cLuxMap* apMap)
    : iLuxEnemy(asName, alID, apMap, eLuxEnemyType_ManPig)
{
    mfWaitTime = 0;
...
bool cLuxEnemy_ManPig::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage* apMessage)
{
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) ChangeState(eLuxEnemyState_Alert);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TakeHit)
          ...
          kLuxOnMessage(eLuxEnemyMessage_PlayerOutOfRange) SetActive(false);

    //------------------------------

    kLuxOnMessage(eLuxEnemyMessage_PlayerDead) ChangeState(eLuxEnemyState_Idle);

    //------------------------------

    kLuxOnMessage(eLuxEnemyMessage_HelpMe) ShowPlayerPosition();
    ...
    kLuxOnMessage(eLuxEnemyMessage_ChangePose)
          ...
          kLuxState(eLuxEnemyState_Idle) kLuxOnEnter mbPlayerInRange = false; // Reset so new checks are made in case player is near.
    SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);

    kLuxOnMessage(eLuxEnemyMessage_PlayerInRange) gpBase->mpDebugHandler->AddMessage(_W("Enemy ") + cString::To16Char(msName) + _W(" enabled!"), false);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected)
          ...
          kLuxState(eLuxEnemyState_GoHome) kLuxOnEnter mpPathfinder->MoveTo(mvStartPosition);
    SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);

    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) ChangeState(eLuxEnemyState_Idle);

    kLuxOnMessage(eLuxEnemyMessage_SoundHeard) if (apMessage->mfCustomValue > mfHearVolume)
    {
    ...
    kLuxState(eLuxEnemyState_Wait) kLuxOnEnter if (mfWaitTime <= 0)
    {
    ...
    else { SendMessage(eLuxEnemyMessage_TimeOut, mfWaitTime, true); }
    mfWaitTime = 0;
    ...
    kLuxOnLeave mpMover->SetOverideMoveState(false);

    //------------------------------

    kLuxOnMessage(eLuxEnemyMessage_TimeOut)

          if (mPreviousState == eLuxEnemyState_Search || mPreviousState == eLuxEnemyState_Stalk || mPreviousState == eLuxEnemyState_Track)
    {
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) int lRand = cMath::RandRectl(1, 2);
    PlayAnim("Idle" + GetCurrentPoseSuffix() + "Extra" + cString::ToString(lRand), false, 0.3f);

    //------------------------------

    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) SendMessage(eLuxEnemyMessage_TimeOut_2, cMath::RandRectf(4, 13), true);
    ...
    //------------------------------
    kLuxState(eLuxEnemyState_Patrol) kLuxOnEnter gpBase->mpMusicHandler->RemoveEnemy(eLuxEnemyMusic_Search, this);
    gpBase->mpMusicHandler->RemoveEnemy(eLuxEnemyMusic_Attack, this);
    ...
    kLuxOnUpdate if (mbStuckAtDoor) // && mpMap->DoorIsClosed(mlStuckDoorID))
    {
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) ChangeState(eLuxEnemyState_Wait);

    //////////////////////////
    // Looping animation shall end
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) mpMover->UseMoveStateAnimations();
    ChangeState(eLuxEnemyState_Wait);
    ...
    //------------------------------
    kLuxState(eLuxEnemyState_Investigate) kLuxOnEnter ChangeSoundState(eLuxEnemySoundState_Alert);
    SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);
    ...
    kLuxOnLeave mfFOVMul = 1.0f;
    FadeOutCurrentAnim(0.2f);
    SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);

    //------------------------------

    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);
    ...
    //////////////
    // Enter
    kLuxOnUpdate float fDistToPlayer = DistToPlayer();

    //////////////////////////////
    // Turn towards player
    if (mbThreatenOnAlert && PlayerIsDetected())
    {
        float fAngleDist = cMath::GetAngleDistanceRad(mpCharBody->GetYaw() + kPif, gpBase->mpPlayer->GetCharacterBody()->GetYaw());
    ...
    else
    {
        ...
        else
        {
            ...
            if (mbIsTelsa == true)
                fTerror *= 3;
            if ((fTerror >= 1 && (fDistToPlayer > mfAlertToHuntDistance || mfAlertRunTowardsCount > mfAlertRunTowardsToHuntLimit)) || fDistToPlayer < mfAlertToInstantHuntDistance)
            {
                gpBase->mpPlayer->SetTerror(1.0f);
    ...
    //////////////
    // Reach end of path
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) if (mbThreatenOnAlert == false)
    {
    ...
    //////////////
    // Update move to
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) if (PlayerIsDetected()) mpPathfinder->MoveTo(mvLastKnownPlayerPos);
    ...
    //////////////
    // Start Hunt
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) if (mbFleeFromPlayer) { ChangeState(eLuxEnemyState_Flee); }
    else
    {
        gpBase->mpPlayer->SetTerror(1.0f);
        ChangeState(eLuxEnemyState_Hunt);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_3) if (PlayerIsDetected() == false) ChangeState(eLuxEnemyState_Patrol);
    else SendMessage(eLuxEnemyMessage_TimeOut_3, 0.75f, true);

    //------------------------------

    //////////////
    // Takes a hit
    kLuxOnMessage(eLuxEnemyMessage_TakeHit) ChangeState(eLuxEnemyState_Hurt);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected)
          ...
          kLuxState(eLuxEnemyState_Search) kLuxOnEnter
          ForceTeslaSighting();
    ...
    kLuxOnLeave SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);
    ...
    // At node
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) mpPathfinder->Stop();
    ...
    // End of searching
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) ChangeState(eLuxEnemyState_Patrol);
    gpBase->mpMusicHandler->RemoveEnemy(eLuxEnemyMusic_Search, this);
    ...
    kLuxState(eLuxEnemyState_Flee)
          ///////////////////////
          // Enter
          kLuxOnEnter IncCurrentPatrolNode(true);
    ...
    ////////////////////////
    // Overload global
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          ///////////////////////
          // Enter
          kLuxOnEnter SetMoveSpeed(eLuxEnemyMoveSpeed_Walk);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) bool bSeenByPlayer = IsSeenByPlayer();
    ...
    if (fPlayerDistance < 4.5f || bSeenByPlayer || IsVisibleToPlayerAtFeetPos(mpPathfinder->GetNextGoalPos()) || IsVisibleToPlayerAtFeetPos(mpPathfinder->GetFinalGoalPos()))
    {
        if (StalkFindNode() == false)
        {
    ...
    ////////////////////////
    // End of current path
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) mfWaitTime = cMath::RandRectf(2, 4);
    ChangeState(eLuxEnemyState_Wait);
    //------------------------------
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
          ...
          if (mbIsTelsa == false && IsInPlayerFovAtFeetPos(mpCharBody->GetFeetPosition()) == false &&
    ...
    ////////////////////////
    // End of current path
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) if (TrackFindNode() == false)
    {
    ...
    ////////////////////////
    // End of current path
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) float fDistToPlayer = DistToPlayer();
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
          ...
          mpMap->BroadcastEnemyMessage(eLuxEnemyMessage_HelpMe, true, mpCharBody->GetPosition(), mfActivationDistance * 0.5f, 0, false, mpCharBody->GetFeetPosition());
    ...
    //////////////////
    // When lost player give some extra time to catch up
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_4) if (PlayerIsDetected() == false)
    {
        gpBase->mpPlayer->RemoveTerrorEnemy(this);
    ...
    else { SendMessage(eLuxEnemyMessage_TimeOut, 0.2f, true); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_EndOfPath) ChangeState(eLuxEnemyState_Hunt);
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
          ...
          if (DistToPlayer2D() > 2.0f) mpPathfinder->MoveTo(gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition());
    else mpMover->TurnToPos(gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition());
    ...
    ///////////////////////
    // Time out
    kLuxOnMessage(eLuxEnemyMessage_TimeOut_2) ChangeState(eLuxEnemyState_Alert);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_BreakDoor) kLuxOnEnter mpPathfinder->Stop();
    PlayAnim("Attack" + GetCurrentPoseSuffix() + cString::ToString(cMath::RandRectl(1, 3)), false, 0.3f);
    mfFOVMul = 4.0f;

    kLuxOnLeave mlAttackHitCounter = 0; // When returning from door breakage there should be no pause!
    mfFOVMul = 1.0f;
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) if (PlayerIsDetected()) { ChangeState(eLuxEnemyState_Alert); }
    ...
    else { PlayAnim("Attack" + GetCurrentPoseSuffix() + cString::ToString(cMath::RandRectl(1, 3)), false, 0.3f); }

    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) Attack(mNormalAttackSize, mBreakDoorAttackDamage, 20.0f);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_AttackMeleeShort) kLuxOnEnter
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver)
          ...
          if (mPreviousState == eLuxEnemyState_Hurt) ChangeState(eLuxEnemyState_Hunt);
    else ChangeState(mPreviousState);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_AttackMeleeLong) kLuxOnEnter
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) if (mlTempVal == 0)
    {
        mlTempVal = 1;
        PlayAnim("Charge" + GetCurrentPoseSuffix(), false, 0.3f, false);
    }

    kLuxOnMessage(eLuxEnemyMessage_AnimationSpecialEvent) if (mlTempVal == 1) { Attack(mNormalAttackSize, mNormalAttackDamage, mfDamageMul); }

    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) ChangeState(mPreviousState);
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_Hurt) kLuxOnEnter mpPathfinder->Stop();
    ...
    kLuxOnMessage(eLuxEnemyMessage_AnimationOver) if (PlayerIsDetected()) { ChangeState(eLuxEnemyState_Hunt); }
    else { ChangeState(eLuxEnemyState_Alert); }

    kLuxOnMessage(eLuxEnemyMessage_TimeOut) if (cMath::RandRectl(0, 1) == 0 && DistToPlayer() < mfNormalAttackDistance * 1.3f) { ChangeState(eLuxEnemyState_AttackMeleeShort); }
    ...
    kLuxOnMessage(eLuxEnemyMessage_PlayerDetected) kLuxOnMessage(eLuxEnemyMessage_HelpMe)
          ...
          kLuxState(eLuxEnemyState_Dead) kLuxOnEnter mpPathfinder->Stop();
...
void cLuxEnemy_ManPig::OnRenderSolidImplemented(cRendererCallbackFunctions* apFunctions)
{
    ...
    {
        pPhysicsWorld->RenderShapeDebugGeometry(GetAttackShape(0), GetDamageShapeMatrix(cVector3f(0, 0, 1)), apFunctions->GetLowLevelGfx(), cColor(1, 0, 0, 1));
    }
}
...
{
    if (mCurrentState == eLuxEnemyState_Idle || mCurrentState == eLuxEnemyState_Wait || mCurrentState == eLuxEnemyState_Patrol || mCurrentState == eLuxEnemyState_Investigate)
    {
        return 1.0f;
    }
...
bool cLuxEnemy_ManPig::FleeTryToFindSafeNode()
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        cAINode* pNode = mpPathfinder->GetNodeAtPos(mpCharBody->GetPosition() + vDirToPlayer * fLength * 0.2f, 0, fLength, false, false, true, NULL);
...
bool cLuxEnemy_ManPig::TrackTeleportBehindPlayer()
{
    ...
    for (int i = 0; i < lNumOfTries; ++i)
    {
        /////////////////////////////////////
        // Find a node to run to
        cAINode* pNode = mpPathfinder->GetNodeAtPos(GetPlayerFeetPos(), fDistance * 0.5f, fDistance, false, true, false, NULL);
        if (pNode == NULL)
        {
            return false;
        }
...
void cLuxEnemy_ManPig::UpdateTesla(float afTimeStep)
{
    ...
    // Special Notice Checks
    {
        ...
        if (mbTeslaEasyEscapeDisabled == false && mfTeslaSpecialNoticeCount < 0.35f && mbCanSeePlayer == false && gpBase->mpPlayer->GetHealth() < 75)
        {
            if (mCurrentState == eLuxEnemyState_Hunt || mCurrentState == eLuxEnemyState_HuntPause || mCurrentState == eLuxEnemyState_HuntWander)
            {
                mfWaitTime = 0.5f;
                gpBase->mpPlayer->RemoveTerrorEnemy(this);
                ChangeState(eLuxEnemyState_Wait);
